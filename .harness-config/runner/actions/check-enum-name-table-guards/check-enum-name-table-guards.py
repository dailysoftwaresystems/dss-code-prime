...
def repo_root():
    ...
    `here/../..` named `.harness-config/runner` the day this file moved out of `scripts/`
    (2026-09-18); a guard pointed at the wrong root finds nothing to refuse. The owner is
    loaded as a SIBLING; a missing owner exits 2, never 0.
    """
    here = os.path.dirname(os.path.realpath(__file__))
    path = os.path.join(os.path.dirname(here), 'owning-tree', 'owning-tree.py')
    if not os.path.isfile(path):
        print('check-enum-name-table-guards: cannot find %s -- this guard\'s tree is '
              'resolved there and nowhere else' % path, file=sys.stderr)
        sys.exit(2)
    ...
    try:
        ...
        print('check-enum-name-table-guards: %s' % exc, file=sys.stderr)
        sys.exit(2)
...
def main():
    ...
    # convention: this guard's ctest form was never self-tested.
    rc = check(repo_root())
    if rc != 0:
        return rc
    return self_test()
