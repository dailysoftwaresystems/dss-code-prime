...
a figure written in a source comment, so every such figure decays -- the general
form, learned from a CMake comment that quoted a corpus count its own test forbids: *a count in a comment is a measurement with no instrument
...
def self_repo():
    ...
    of `scripts/` (2026-09-18). `check-doc-census`'s self-test runs a COPY of this file
    under a synthetic root with `--repo`, and a copy has no sibling owner to load.
    """
    path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))),
                        "owning-tree", "owning-tree.py")
    if not os.path.isfile(path):
        print("source-census: cannot find %s -- this census's tree is resolved there and "
    ...
    try:
        ...
        print("source-census: %s" % exc, file=sys.stderr)
        raise SystemExit(EXIT_COLLAPSE)
...
DECLARATION = os.path.join(".harness-config", "runner", "actions", "check-doc-census",
                           "source-census.json")
