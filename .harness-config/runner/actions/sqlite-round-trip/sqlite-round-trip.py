...
#
# ─────────────────────────────────────────────────────────────────────────────
# WHY THIS EXISTS, and why it is not part of the sqlite driver (`build_and_test.py`)
#
# The harness driver answers "can THIS host build every leg, and run the ones
# it can reach?".  It is single-machine by construction (as its two retired
# predecessors, `build-and-test.{sh,ps1}`, were): a leg either
...
def read_sqlite_expectations(header: Path) -> "tuple[str, str]":
    ...
    Same two `#define`s the driver's smoke step reads out of the staged header
    (`sqlite_smoke.py`) -- but read from a COPY that travels inside the payload, which is
...
def leg_env(base: dict, payload: Path, m: dict, a: argparse.Namespace, subdir: str) -> dict:
    ...
    the same rule the driver's `sqlite_launch.py` keys its loader variable on, and the reason
