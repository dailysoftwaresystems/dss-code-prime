#!/usr/bin/env python3
"""Support subcommands for .harness-config/runner/actions/profile-compile/profile-compile.py — the cross-leg compile-time
profile.
...
Subcommands
  ...
  timed-gate  run a command under an in-process witness and time it on a MONOTONIC clock
...
# ceremony: the sqlite harness's PowerShell driver (retired on 2026-09-21 into the one
# Python driver, `real-examples/c/sqlite/build_and_test.py`) selected the NEWEST compiler
# under any build root regardless of type, which on a developer box is always
# `build/dbg`, while its bash twin built Release unconditionally. The resulting
...
#
# 1. THE WITNESS. It refuses to report success on an exit-0 that produced no evidence
#    of work, and captures rc DIRECTLY rather than after a pipe -- this tool's own
#    predecessor printed `PROFILE-LEG-OK vps-arm64 rc=1` over a compile that had died
#    before parsing a single file, which is precisely the self-authored success string
#    a witness exists to make unsayable.
#    ⓘ IN-PROCESS SINCE 2026-09-18. It used to be `scripts/run-gate/run-gate.sh`, which
#    the DssHarness migration deleted, and this subcommand then REFUSED on every leg.
#    The witness itself is three lines and lives here now; the gate script's OTHER two
#    refusals -- a tree that moved under the run, a build directory another live run
#    holds -- belong to the runner: `dssharness run profile-compile` fingerprints the
#    action's inputs (`requireInputsUnmoved`) and samples contention around the step.
#
...
def cmd_timed_gate(args):
    """Run `args.command`, its stdout and stderr both into `args.log`, and time it.

    rc != 0                   -> that rc, unchanged (the failure is the compiler's)
    rc == 0, witness present  -> 0
    rc == 0, witness ABSENT   -> 3, a REFUSAL: an exit 0 that left no evidence of work
    """
    if not args.command:
        die('timed-gate needs a command after `--`')
    try:
        witness = re.compile(args.witness)
    except re.error as exc:
        die('the witness %r is not a regular expression: %s' % (args.witness, exc))
    os.makedirs(os.path.dirname(os.path.abspath(args.log)), exist_ok=True)
    t0 = time.monotonic()
    try:
        with open(args.log, 'wb') as log:
            rc = subprocess.call(args.command, stdout=log, stderr=subprocess.STDOUT)
    except OSError as exc:
        die('could not start %r: %s' % (args.command[0], exc))
    elapsed = time.monotonic() - t0     # both reads in THIS process -- see note 2
    text = open(args.log, 'rb').read().decode('utf-8', 'replace')
    seen = bool(witness.search(text))
    print('TIMED-GATE elapsed=%.1fs rc=%d witness=%s' % (elapsed, rc, 'seen' if seen else 'ABSENT'))
    if rc != 0:
        return rc
    if not seen:
        print('timed-gate: REFUSED -- %r exited 0 and its log (%s) never printed the witness %r. '
              'An exit code is not evidence of work; the timing is withheld.'
              % (args.command[0], args.log, args.witness), file=sys.stderr)
        return 3
    return 0
...
def main():
    ...
    p = sub.add_parser('timed-gate', help='run under an in-process witness, timed monotonically')
