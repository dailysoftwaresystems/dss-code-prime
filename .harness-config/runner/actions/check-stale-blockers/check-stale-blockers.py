...
# directory -- the layout every program under `.harness-config/runner/actions/` shares.
sys.path.insert(0, os.path.join(os.path.dirname(HERE), "anchors"))

import anchors  # noqa: E402

# ★ THE ROOT IS THE OWNER'S ANSWER, NOT A COUNT OF `dirname` CALLS. This file used to
# derive it as two directories above its own, which named `.harness-config/runner` the
# moment the programs moved out of `scripts/` (2026-09-18) -- the depth-hardcoding defect
# `owning-tree` exists to end. `anchors.ROOT` is resolved through that owner from the
# sibling's own file, which sits in the same tree as this one.
ROOT = anchors.ROOT
...
def self_test():
    ...
    #   cosmetic. This directory is a CITATION ROOT (`anchors.citationRoots`), and
    #   `dssharness check-anchor-citations` refuses any `D-*` cited there with no
    #   registry row. A synthetic
