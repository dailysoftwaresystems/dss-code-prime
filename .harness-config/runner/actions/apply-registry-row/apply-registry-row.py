...
★★★ AND SINCE THE SAME DATE IT DOES NOT PLACE THE ROW ITSELF -- `.harness-config/runner/actions/anchors/anchors.py`
...
def repo_root():
    ...
    `.harness-config/runner/actions/anchors/anchors.py`, already walked up from ITS OWN file, so the two halves
    of one application could name two different trees. Both now answer by the same walk,
    owned by `.harness-config/runner/actions/owning-tree/owning-tree.py`.
    """
    ot = _owning_tree()
    try:
...
# `.harness-config/runner/actions/anchors/` had no writer -- including every fixture.
...
def _anchors():
    ...
    path = ANCHORS_PY
    if not os.path.isfile(path):
        raise Refused("cannot find .harness-config/runner/actions/anchors/anchors.py -- this tool VALIDATES a "
...
def self_test():
    ...
    # INPUT DATA to a parser test, not citations. ✔MEASURED: written
