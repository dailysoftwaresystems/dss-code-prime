...
pass (a guard that fails OPEN is the worst kind). Two floors are checked —
...
def _own_tree():
    ...
    Three `dirname` calls named `.harness-config/runner` the day this file moved out of
    ...
    if not os.path.isfile(path):
        print("check-export-macro-placement: cannot find %s -- this guard's tree is resolved "
    ...
    try:
        ...
        print("check-export-macro-placement: %s" % exc, file=sys.stderr)
        sys.exit(2)
