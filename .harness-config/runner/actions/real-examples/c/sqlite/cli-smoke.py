...
★ ONE IMPLEMENTATION. The driver's smoke step (`sqlite_smoke.py`) calls this
file, as both old drivers (`build-and-test.sh` and `build-and-test.ps1`) did. The
assertions could have been written twice — once in bash and once in PowerShell —
and that is exactly how "a capability in one driver and not the other" gets
manufactured; TF-C117 already established the answer (logic that can be shared
goes into the PYTHON, not duplicated into two shells). Fourteen assertions kept
in step by hand across two languages would have drifted on the first edit.
...
still non-zero, and still counted by the driver. Charging it to DSS was not
...
def a03_source_id_token_exact(r, ctx):
    ...
    (a staged tree mixing two sqlite vintages) applied to the finished binary
...
def main(argv=None):
    ...
    # with `--cli`, which the LA arms of `test_confound_scope.py` assert on by
    # substring (as the old `test-confound-scope.{sh,ps1}` did).
    ...
    # exact shape of a dash-leading launcher token misparsed as an option.
...
def self_test():
    ...
    try:
        ...
        #     and the JSON, exercised as the driver (`sqlite_smoke.py`) exercises them.
