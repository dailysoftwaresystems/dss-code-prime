#!/usr/bin/env python3
"""Stage the CONFIGURED headers a leg compiles against, PER TARGET — for the driver.
...
configuration and VERIFIES what it wrote.
  ...
  1. zlib's `zconf.h`   — the staged copy used to be PE-SHAPED for every leg
                          (one dir per `recipeTransform`, from `zconfGuards`)
  2. sqlite's `sqlite_cfg.h`
                        — a Mach-O leg inherited the deriving Linux host's configure probes
...
replaced the need for both: one implementation, now called by the driver
(`sqlite_build.py`), staging one zinc/ per `recipeTransform` from the guards each
leg DECLARES in legs.json.
...
opposite meanings. Passing only ONE of the two is a hard error. The driver
(`sqlite_build.py`) passes both, and REFUSES to build a leg without its staged
header; the reason the
...
# No bytecode beside the sibling: this directory is an action's, whose runs demand their inputs
# unmoved, and ✔MEASURED 2026-09-22 (helper p4s7) every run of this program wrote
# `__pycache__/harness_legs.*.pyc` into it until this line.
...
def stage_one(key, guards, zlib_h, zconf_h, dest_root):
    ...
    # Forward slashes on every host: this string is printed for the driver to put
    # straight into a manifest `includes` entry; DSS reads `C:/x/y` on every host,
    # and no shell can mis-read it the way the old bash driver mis-read `C:\x\y`.
...
def main(argv=None):
    ...
    # point of use because the driver (`sqlite_build.py`) refuses to continue when
    # no cfg stage was produced (`stage-zinc.py produced NO per-target sqlite_cfg.h`
    # → die) and poisons any leg whose stage is missing. So the guarantee — no leg is ever
    ...
    # the run-wide `die` the driver already has for a missing zlib.h
    # (`sqlite_stage.py`).
    ...
    if not want_cfg:
        sys.stdout.write(
            ...
            "./configure answers. The driver passes the pair and REFUSES to "
            "build without it.\n")
