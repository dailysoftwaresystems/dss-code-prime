...
A claim TRUE WHEN IT WAS TYPED and FALSE WHEN THE COMMIT LANDED -- the species
...
when it was typed and false when the commit landed (the species
`_INVENTORY_COMMENT` already names).
...
with exit 2, never a pass. An empty scan is a COLLAPSE (a guard that fails OPEN is
the worst kind).
...
INVENTORY_REL = os.path.join(ACTIONS_REL, "check-stale-refusal-citations",
                             "inventory.json")
...
def governed_files(root):
    ...
    # was not this tree's. The helper, and why, are in .harness-config/runner/actions/owning-tree/.
...
def repo_root():
    ...
    that chose it, and why git must agree are in `.harness-config/runner/actions/owning-tree/owning-tree.py`.
