...
def _own_tree():
    """The tree THIS FILE lives in, by `owning-tree`'s walk -- never a count of `dirname`s.

    Two `dirname` calls named `.harness-config/runner` the day this file moved out of
    `scripts/` (2026-09-18), and a census of `<that>/examples` counts a directory that does
    not exist. The owner is loaded as a SIBLING; a missing owner exits 2, never 0.
    """
    path = os.path.join(os.path.dirname(HERE), "owning-tree", "owning-tree.py")
    if not os.path.isfile(path):
        print("examples-census: cannot find %s -- this census's tree is resolved there and "
    ...
    try:
        ...
        print("examples-census: %s" % exc, file=sys.stderr)
        sys.exit(2)
...
def census(ms):
    ...
    for k in ("spec", "artifact", "runOn", "emulator", "exitCode",
              "expectedStdout", "dependsOn", "prebuiltLibraries",
              "loaderSearchPathVariable", "$comment"):
