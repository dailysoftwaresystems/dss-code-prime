...
def repo_root():
    ...
    directory to `.harness-config/runner/actions/burndown-queue/`); now the walk has ONE owner,
    `.harness-config/runner/actions/owning-tree/owning-tree.py`, so the two spellings cannot drift apart.
    """
    ot = _owning_tree()
    try:
...
# a harness row named for the reference differential oracle, filed in `-harness.md`, was printed as the
...
# ⇒ closed 2026-08-31: the burndown no longer bands a harness row into the production queue.
...
def selftest():
    ...
    # ⚠ THE FIXTURE ID BELOW IS ASSEMBLED FROM FRAGMENTS, AND THAT IS NOT DECORATION.
    # This directory is a citation root (`anchors.citationRoots`), so a plausible-looking
    # invented id written whole is an anchor cited in the tree with no row -- precisely the
    # leak `check-anchor-citations` exists to catch; the first draft of this self-test minted
    # two. It used to borrow a REAL row's id instead, and that row was retired with the
    # harness registry on 2026-09-16, which left the borrowed id dangling -- a fixture must
    # not depend on the registry it is not testing. The `D-BUILD-` head is what matters:
    # it is a harness-LOOKING prefix (`NS_HARNESS`).
    ...
    _HX = "D-" + "BUILD" + "-HARNESS-LOOKING-FIXTURE"
    b, _, _ = band_of("x#" + _HX, wrong, BUCKET_PRODUCTION)
    ...
    b2, _, _ = band_of("x#" + _HX, wrong, BUCKET_PLAN)
