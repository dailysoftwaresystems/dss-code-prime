...
A NEW wall-clock literal in a test used to be unguarded; this guard closes that.
...
pass (a guard that fails OPEN is the worst kind).
...
(the shape of a citation resolving via its own bug report). Two copies of a C
...
smaller inconsistency it happens to also produce (a collapse was once reported as
a stale allowlist path).
"""
...
def _own_tree():
    ...
    if not os.path.isfile(path):
        print("%s: cannot find %s -- this guard's tree is resolved there and nowhere "
              "else" % ('wall-clock-in-tests', path), file=sys.stderr)
        sys.exit(2)
    ...
    try:
        ...
        print("%s: %s" % ('wall-clock-in-tests', exc), file=sys.stderr)
        sys.exit(2)
...
# the laundering the inventory's DEBT rule forbids -- the
...
# not gets fixed.
# ⇒ The burn-down CLOSED with this dict empty. A guard existing is not the debt
#   being paid; this is.
INVENTORY = {}
