...
It closes a gap -- nothing ratcheted a NEW guard into reconfiguring its streams --
and generalises the instance that exposed it: a Python guard that DIED printing
tree text on a Windows pipe.
...
Every `.py` PRIMARY program of an action under `.harness-config/runner/actions/`
(where every program `scripts/` held moved on 2026-09-18), taken from
`check-scripts-index.scan()` -- IMPORTED, never re-implemented. That guard already
owns the definition of "what actions exist and which file is an action's primary
program"; a second copy here is how two instruments start disagreeing about the
same tree.
  * NOT "every `check-*` action". A name predicate would let a guard
...
The burn-down of those four is tracked by the inventory itself: the ratchet stays
red on any NEW entry until `unprotected` is empty, and it refuses a repaired
entry that lingers. ✔2026-09-18 (lane `mig`): `refresh_landing_log` repaired and
its entry deleted, which leaves two.
...
INVENTORY_REL = os.path.join(
    ".harness-config", "runner", "actions", "check-guard-output-encoding", "inventory.json")
...
def repo_root():
    ...
    walk in `.harness-config/runner/actions/owning-tree/owning-tree.py` keeps that property -- it does not count
...
def load_scripts_index(path=None):
    ...
    The alternative -- re-listing the actions here -- would be a second definition
    of what an action is, drifting away from the first the moment either changes.
    """
    if path is None:
    ...
    if not os.path.isfile(path):
        raise Collapse(
            "cannot find the action enumeration at %s.\n"
            "  This guard governs every Python PRIMARY program of an action, and the "
...
def subjects(root, index_path=None):
    """Every action's `.py` primary program, repo-relative, sorted."""
    ...
    try:
        ...
        raise Collapse(
            ...
            % exc)
    # ⓘ `primary` is None for an action that ships no program named for it (the sqlite
    # corpus harness's entry point is `build_and_test.py`, not a `sqlite.py`): such an
    # action has no primary for this guard to probe.
    return sorted(e.primary for e in entries if e.primary and e.primary.endswith(".py"))
...
def probe(root, rel_or_abs):
    ...
    try:
        try:
            # `-B`: the child IMPORTS the subject, and an import writes `__pycache__` beside
            # it unless told not to -- into an action directory whose steps declare
            # `requireInputsUnmoved` (✔MEASURED 2026-09-21: one directory per probed program).
            p = subprocess.run([sys.executable, "-B", "-c", _PROBE, path, report],
...
# have left the suite GREEN while asserting less -- a nonfatal guard's vacuous
# pass, reached by subtraction
...
def self_test():
    ...
    try:
        ...
        # ★ THE PROBE WRITES NO BYTECODE BESIDE WHAT IT IMPORTS (✔MEASURED 2026-09-21: before
        # `-B`, one run of this guard left a `__pycache__` beside every probed program, inside
        # action directories whose steps declare `requireInputsUnmoved`). Arms 0-5 probed
        # `box`; nothing may have appeared there.
        _arm(results, "20 NO-BYTECODE: the probes above left no __pycache__ beside the subjects",
             (os.path.isdir(os.path.join(box, "__pycache__")), ""), False)
        # ...and the negative is PRODUCIBLE: the probe's own program, run WITHOUT `-B` on a copy
        # of the same subject, does write one -- so arm 20 can fail.
        neg = os.path.join(box, "neg")
        os.makedirs(neg)
        shutil.copy2(good, os.path.join(neg, "protected.py"))
        env = dict(os.environ)
        env.pop("PYTHONDONTWRITEBYTECODE", None)
        subprocess.run([sys.executable, "-c", _PROBE, os.path.join(neg, "protected.py"),
                        os.path.join(neg, "report.json")], cwd=neg, capture_output=True,
                       stdin=subprocess.DEVNULL, env=env, timeout=PROBE_TIMEOUT_S)
        _arm(results, "20b NO-BYTECODE NEGATIVE: the same probe WITHOUT -B writes one",
             (os.path.isdir(os.path.join(neg, "__pycache__")), ""), True)
    finally:
