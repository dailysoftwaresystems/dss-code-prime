...
to occupy:
    ...
    ✓ a registry row id, spelled whole on one line
...
verb moves in both directions is not a ratchet, and `--write` once could RAISE a
ceiling. So:
...
is what exit 1 governs here. The species: a claim true when typed and false when
the commit landed.
...
INVENTORY_REL = os.path.join(".harness-config", "runner", "actions", "check-plan-citations",
                             "inventory.json")
...
# ★★ 2026-09-18: `scripts` AND `real-examples` BECAME ONE ROOT, AND IT HAD TO MOVE IN THE
# SAME CHANGE AS THEIR FILES. Every program from both now lives under DssHarness's
# actions directory (`real-examples/c/sqlite` as the group `real-examples/c/sqlite`
# inside it). `_walk` SKIPS a root that does not exist rather than refusing, so leaving
# the two old names here would have stopped governing all of those files without a word
# -- while the inventory's retargeted keys went stale and red for a reason nobody reading
# them could see. The root is the actions directory, not all of `.harness-config`: the
# rest of that directory is the tool's run logs and per-host connection data, which are
# generated or secret and are never documentation.
CODE_ROOTS = ("src", "tests", os.path.join(".harness-config", "runner", "actions"),
              "examples", "docs", "packaging", ".github", "integrated_tests")
...
def repo_root():
    ...
    `.harness-config/runner/actions/owning-tree/owning-tree.py`; this guard reads files only.
    """
    ot = _owning_tree()
    try:
...
def _walk(root, rel_roots, keep):
    ...
    for rel_root in rel_roots:
        ...
        for dirpath, dirnames, files in os.walk(base):
            ...
            # and would double-count every file it holds. `artifacts` is the other name
            # DssHarness reserves under every action for a run's KEPT output (`help
            # runners`: neither name may appear on an action's path), so nothing tracked
            # can live in one -- ✔MEASURED 2026-09-18, `git ls-files` holds no path with
            # an `artifacts/` segment -- and a run's output is not documentation.
            dirnames[:] = [d for d in dirnames
                           if d not in ("__pycache__", "worktrees", "build", "artifacts",
                                        ".git")]
            for f in files:
                if keep(f):
...
# ★ A COUNT IS REMEMBERED BY THE BYTES IT WAS COUNTED FROM, AND BY NOTHING ELSE.
# `count_text` is a pure function of a document's content -- the path only says where
# to read it -- so one census answers for every later census that reads the same bytes.
# ✔MEASURED 2026-09-23 (lane mig, part 5; Windows, direct run): the self-test re-runs
# the WHOLE census 33 times over its replica while each arm changes one or two files --
# 93.5 s of the guard's 99 s, one census 2.8 s, the real check 2.1 s -- which is why
# `plan_citations_guard` ran 107-272 s under ctest's load against a debug ceiling of
# 107 s. ⚠ KEYED BY A DIGEST OF THE BYTES, NEVER BY PATH OR mtime: a planted citation
# is new bytes, so it is counted afresh; a restored document is the old bytes, so it
# gets the old count, which is right by construction; a path or a timestamp could hand
# a changed document its old answer (two writes in one clock tick, a restore by rename).
# Per process: nothing is stored anywhere.
_COUNTS = {}


def count_in(path):
    """Positional citations in one document (its bytes' count, remembered by digest)."""
    with io.open(path, "rb") as fh:
        raw = fh.read()
    key = hashlib.sha256(raw).digest()
    n = _COUNTS.get(key)
    if n is None:
        n = _COUNTS[key] = count_text(raw.decode("utf-8", "replace"))
    return n


def count_text(text):
    """Positional citations in one document's decoded text."""
    n = 0
    ...
    # ⓘ Lines split as the file read split them: `newline=""` is universal-newline
    # splitting with the endings left in place, for a string as for a file.
...
def _files_under(top):
    """Every file under `top` (none when it does not exist)."""
    return [os.path.join(d, f) for d, _dirs, files in os.walk(top) for f in files]
...
def _pipe_arms(fact):
    ...
    try:
        ...
        # ⓘ `-B`: the child loads THIS file by path, and a by-path load writes bytecode
        # beside its source unless told not to -- into this action's own directory, on
        # every self-test (✔READ 2026-09-23: a `__pycache__` stood here, dated by a
        # self-test run). stdin at EOF: a guard's child never inherits ctest's.
        # ★ Arm 20c WITNESSES it without touching this directory: PYTHONPYCACHEPREFIX
        # moves any bytecode the child writes into the box, where it can be counted.
        env.pop("PYTHONDONTWRITEBYTECODE", None)
        prefix = os.path.join(box, "pycache-prefix")
        env["PYTHONPYCACHEPREFIX"] = prefix
        p = subprocess.run([sys.executable, "-B", "-c", driver], cwd=box, env=env,
        ...
        good &= fact("20c PIPE-CHILD-WRITES-NO-BYTECODE",
                     not written and c.returncode == 0 and any(
                         f.endswith(".pyc") for f in control),
                     "with -B: %d file(s); CONTROL without -B: rc=%d, %d .pyc"
                     % (len(written), c.returncode,
                        sum(1 for f in control if f.endswith(".pyc"))))
        return good
    finally:
...
def selftest(root):
    ...
    try:
        ...
        # ★ 2026-09-18: the sqlite harness moved under DssHarness's actions directory with
        # every former `scripts/` program, so this arm now also proves THAT root is read:
        # drop `.harness-config/runner/actions` from `CODE_ROOTS` and it reds.
        # ⓘ 2026-09-21: its subject is `sqlite_base.py`, the Python port of the shell/PowerShell
        # base helpers the arm used to plant into (no `.sh`/`.ps1` lives under the actions tree).
...
def main(argv):
    ...
    try:
        ...
        # session. Same shape as a nonfatal guard's vacuous pass.
