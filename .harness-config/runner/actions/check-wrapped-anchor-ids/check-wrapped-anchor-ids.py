...
An anchor id wrapped across a line break is invisible to every grep; this guard is
step 1 of making that impossible.
...
disagreeing about the same tree.
  ...
  * WHAT MEMBERSHIP IS FOR, **AS OF P55 (2026-09-03)**, AND IT IS NO LONGER THE
    ...
    a key, so membership was contributing no discrimination at all -- and it made the
    guard blind to a wrap of a row minted in the same cycle.
...
── FAIL-CLOSED ──────────────────────────────────────────────────────────────────
An empty scan is a COLLAPSE, not a pass (a guard that fails OPEN is the worst kind):
...
── NO `.ps1` TWIN ───────────────────────────────────────────────────────────────
A `.py` already runs on both hosts, and the operator ruled on 2026-08-19 that a
`.sh`/`.ps1` twin is a second implementation of something that was never split. Eight of this repository's
...
lines above claims it does -- the enum-name-table guard's ctest form was never
self-tested.
...
`_INVENTORY_COMMENT`: a claim true when typed and false when the commit landed).
...
INVENTORY_REL = os.path.join(ACTIONS_REL, "check-wrapped-anchor-ids", "inventory.json")
...
# ⓘ THE CLASS'S CANONICAL ROW (an id wrapped across a line break is invisible to
# every grep) is CLOSED, and it is recalled here deliberately, because the
# correction below is ABOUT that closure: it closed in P29 claiming *ZERO wrapped
# ids in the governed tree, and the
...
# ★★★ THE JOIN MUST NOT REQUIRE A **KNOWN** KEY, AND THAT IS THE WHOLE FIX FOR A
# GUARD THAT WAS BLIND TO A WRAP OF A ROW MINTED IN THE SAME CYCLE.
...
def governed_files(root):
    ...
    # ✔MEASURED 2026-09-15 (P66 lane rr). The helper, and why, are in .harness-config/runner/actions/owning-tree/.
...
def selftest():
    ...
    # registry guard's threshold widened to `{1,}` in P50 (the old one hid seventy
    # registered rows): a bare
    ...
    # one class that could not be found.
...
def repo_root():
    ...
    `.harness-config/runner/actions/owning-tree/owning-tree.py`. `reads_git`, because the scanned set is
