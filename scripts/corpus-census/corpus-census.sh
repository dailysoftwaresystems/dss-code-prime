...
# project before.
#
# `scripts/corpus-census/corpus-census.ps1` is the Windows sibling and calls the SAME .py.
...
CENSUS_PY="$HERE/corpus-census.py"

if [[ ! -f "$CENSUS_PY" ]]; then
  printf 'corpus-census: FATAL: %s is missing — this launcher has no census of its own.\n' \
         "$CENSUS_PY" >&2
  exit 2
fi
...
if [[ -z "$PY" ]]; then
  printf 'corpus-census: FATAL: no Python 3 interpreter found.\n' >&2
