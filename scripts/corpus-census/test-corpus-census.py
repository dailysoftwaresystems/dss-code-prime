...
def main(argv):
    ...
    try:
        ...
        copy = put(own, "scripts/corpus-census/corpus-census.py", copy_from=os.path.join(HERE, "corpus-census.py"))
        put(own, "scripts/owning-tree/owning-tree.py",
            copy_from=os.path.join(REAL_TREE, "scripts", "owning-tree", "owning-tree.py"))
        git(own, "init", "-q")
