...
# checked for it. `scripts/` held only the two anchor-registry twins, the
...
# diagnose. `scripts/check-line-endings/check-line-endings.sh` reached the same conclusion the hard way
...
if [[ "${_n_orphan}" -gt 0 ]]; then
    ...
    echo "      the ALLOWLIST in BOTH scripts/check-orphan-tests/check-orphan-tests.sh and"
    echo "      scripts/check-orphan-tests/check-orphan-tests.ps1, naming the reason AND the CMakeLists that"
