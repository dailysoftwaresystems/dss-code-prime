...
`lane-worktree add` (both twins) writes it with no paths; `seed` rewrites it with the
...
The rules the two verbs encode:
  ...
  * ALL OR NOTHING. `row/` is parsed strictly -- every entry is `<ANCHOR>.<status |
    ...
    byte-for-byte.
  * THROUGH THE SANCTIONED WRITER, CELLS BY FILE: `scripts/anchors/anchors.py write
...
root and `git_run` come from `scripts/owning-tree/owning-tree.py`.
...
# Where `land` keeps a removed lane's evidence by default. Ignored with the rest of
# `.worktrees/`, repo-relative, and never listed as a lane (it starts with a dot).
EVIDENCE_DIR = os.path.join(WORKTREES_DIR, ".evidence")
# The top-level directories a lane keeps evidence in -- the same two `lane-worktree`
# gates. Listed here only to MEASURE what `land` must find again at the destination.
...
def _owning_tree():
    """`scripts/owning-tree/owning-tree.py` -- the ONE owner of "which tree is this file in?" and
...
def repo_root(anchor=None):
    ...
    ★★ AND ONE OWNER ANSWERS IT FOR EVERY PYTHON SCRIPT: `scripts/owning-tree/owning-tree.py`
    walks up from `__file__` to the nearest directory holding `.plans/` and `scripts/`, and
...
def classify(root, wt, seed, settled=(), base=None):
    ...
    if base is None:
        base = lane_head(wt)
    candidates = sorted(q for q in set(changed_paths(wt)) if not is_lane_tree(q))
...
def load_anchors_module(root):
    """-> the TARGET tree's `scripts/anchors/anchors.py`, loaded as a module.
    ...
    """
    path = os.path.join(root, "scripts", "anchors", "anchors.py")
    if not os.path.isfile(path):
        die("no row writer at %s -- rows are written only through scripts/anchors/anchors.py."
            % path)
...
def _run_writer(root, plan, apply_it):
    ...
    """
    argv = [sys.executable, os.path.join(root, "scripts", "anchors", "anchors.py"), "write",
...
def evidence_digests(wt):
    """-> {worktree-relative path: md5} for every file under every evidence root."""
    found = {}
    for top_name in EVIDENCE_ROOTS:
        top = os.path.join(wt, top_name)
...
def default_evidence_destination(root, lane):
    ...
    first = os.path.join(root, EVIDENCE_DIR, "%s-%s" % (lane, stamp))
    candidate, n = first, 1
...
def lane_worktree_remove_argv(root, lane, destination, verified):
    ...
    evidence, DISCARD its uncommitted work, and remove its worktree -- through this host's
    twin, probed BY EXECUTION.
    ...
    reaches -- the twins refuse those with exit 8 too, and a fold cannot see them. `land` never
    builds it for such a lane: `lane_base` refuses first, a HEAD past the recorded base or, for a
    format-1 manifest, a HEAD holding such commits. Self-test arms (l8) and (l8b) pin that no
    removal is even built.

    ⓘ The `.ps1` on Windows, where `bash` may resolve to WSL's shim that cannot open a
    `C:/` path; the `.sh` everywhere else. Each candidate must hand back the subject's own
    usage line, so an interpreter that starts but cannot run the subject is not chosen.
    """
    ...
    lw = os.path.realpath(os.path.join(os.path.dirname(os.path.realpath(__file__)),
                                       "..", "lane-worktree"))
    if os.name == "nt":
        subject, usage = os.path.join(lw, "lane-worktree.ps1"), b"usage: lane-worktree.ps1"
        candidates = [[exe, "-NoProfile", "-NoLogo", "-ExecutionPolicy", "Bypass", "-File",
                       subject] for exe in ("pwsh", "powershell")]
        tail = ["remove", lane, "-Repo", root, "-DiscardWork", "-PreserveTo", destination]
    else:
        subject, usage = os.path.join(lw, "lane-worktree.sh"), b"usage: lane-worktree.sh"
        candidates = [["bash", subject]]
        tail = ["--repo", root, "remove", lane, "--discard-work", "--preserve-to", destination]
    if subject not in _LANE_WORKTREE:
        for base in candidates:
            try:
                probe = subprocess.run(base, capture_output=True,
                                       env=_owning_tree().git_environment())
            except OSError:
                continue
            if usage in probe.stdout + probe.stderr:
                _LANE_WORKTREE[subject] = base
                break
        else:
            die("no interpreter could run %s (tried %s, each BY EXECUTION against its own "
                "usage line) -- the worktree cannot be removed by its owner, so the landing "
                "stops here with the worktree kept."
                % (subject, ", ".join(c[0] for c in candidates)))
    return _LANE_WORKTREE[subject] + tail
...
def cmd_list(root):
    ...
    edir = os.path.join(root, EVIDENCE_DIR)
...
def self_test():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # whatever git environment the caller exported. [[D-SCRIPT-LANE-WORKTREE-REPO-ROOT-IS-CWD-KEYED]]
    ...
    with tempfile.TemporaryDirectory() as tmp2:
        ...
        def make_lane(repo, lane):
            git(repo, "worktree", "add", "--detach", "%s/%s" % (WORKTREES_DIR, lane), "HEAD")
        ...
        # real tree. The writer is a COPY of this tree's own `scripts/anchors/` together
        ...
        # .../scripts/owning-tree/owning-tree.py"). No lane could see it: `lf`'s own tree held
        # no `owning-tree` consumer, and the lanes that added them gated an older lane-fold.
        # D-TEST-LANE-FOLD-LANDING-FIXTURE-COPIES-THE-ROW-WRITER-WITHOUT-OWNING-TREE
        # ⚠ Fixture anchor ids are ASSEMBLED from fragments: `scripts/` is a scanned root
        # for `check-anchor-registry`, and an id written whole here would be a citation.
        here_tree = os.path.realpath(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", ".."))
        r3 = os.path.join(top, "land")
        for d in ("anchors", "check-anchor-balance", "burndown-queue", "owning-tree"):
            shutil.copytree(os.path.join(here_tree, "scripts", d),
                            os.path.join(r3, "scripts", d),
        ...
        # -- every lane's does -- unless told --discard-work / -DiscardWork. So the order must be:
        ...
        pin(kinds.count("remove-argv") == 1 and post is not None and events[at][1] is post
            ...
            and any(flag in built[0] for flag in ("--discard-work", "-DiscardWork")),
        ...
        pin(isinstance(rv_clean, list)
            and any(flag in rv_clean for flag in ("--discard-work", "-DiscardWork")),
