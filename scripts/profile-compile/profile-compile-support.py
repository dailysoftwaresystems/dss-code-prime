#!/usr/bin/env python3
"""Support subcommands for scripts/profile-compile/profile-compile.sh — the cross-leg compile-time
profile.
...
def cmd_timed_gate(args):
    # ★★ ASSEMBLED FROM COMPONENTS, WHICH IS WHY THE 2026-08-19 scripts/
    # MIGRATION MISSED IT: the path never spelled `tools/` anywhere, so a
    # repo-wide sweep over that string rewrote the diagnostic three lines
    # below and left the lookup pointing at a directory that no longer
    # exists -- `timed-gate` died on every leg while naming a file that was
    # present and correct. A path built from parts is invisible to a search
    # for the whole.
