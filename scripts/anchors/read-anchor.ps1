# read-anchor.ps1 -- print ONE anchor in full.
#
...
if (-not $py) {
    Write-Error "read-anchor: no python3, python or py on PATH. This is a launcher over $PSScriptRoot\anchors.py -- set \:PYTHON or install one."
    exit 3
}
& $py (Join-Path $PSScriptRoot 'anchors.py') read @args
exit $LASTEXITCODE
