# write-anchor.ps1 -- CREATE or REPLACE a whole row from its fields.
#
...
if (-not $py) {
    Write-Error "write-anchor: no python3, python or py on PATH. This is a launcher over $PSScriptRoot\anchors.py -- set \:PYTHON or install one."
    exit 3
}
& $py (Join-Path $PSScriptRoot 'anchors.py') write @args
exit $LASTEXITCODE
