# set-anchor.ps1 -- PATCH named fields on an EXISTING row (closing it MOVES it to the archive).
#
...
if (-not $py) {
    Write-Error "set-anchor: no python3, python or py on PATH. This is a launcher over $PSScriptRoot\anchors.py -- set \:PYTHON or install one."
    exit 3
}
& $py (Join-Path $PSScriptRoot 'anchors.py') set @args
exit $LASTEXITCODE
