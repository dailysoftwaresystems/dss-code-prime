#!/usr/bin/env bash
# set-anchor.sh -- PATCH named fields on an EXISTING row (closing it MOVES it to the archive).
#
...
if [ -z "${_py}" ]; then
    if command -v python3 >/dev/null 2>&1; then _py=python3
        ...
        echo "set-anchor: no python3 or python on PATH. This is a launcher over" >&2
        echo "  ${_dir}/anchors.py -- set PYTHON=<interpreter> or install one." >&2
        exit 3
    fi
fi
exec "${_py}" "${_dir}/anchors.py" set "$@"
