...
$censusPy = Join-Path $here 'corpus-census.py'

if (-not (Test-Path -LiteralPath $censusPy -PathType Leaf)) {
    Write-Error "corpus-census: FATAL: $censusPy is missing — this launcher has no census of its own."
    exit 2
}
...
if ($null -eq $chosen) {
    Write-Host 'corpus-census: FATAL: no Python 3 interpreter found.'
