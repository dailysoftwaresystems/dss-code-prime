// ML2 HIR→MIR lowering tests: end-to-end (parse c → semantic → HIR
...
TEST(MirLoweringC, StraightLineAddFunction) {
    auto L = lowerC("int add(int a, int b) { return a + b; }");
...
TEST(MirLoweringC, AtomicScalarAccessLowersToAtomicLoadStore) {
    auto L = lowerC(
        "int g;\n"
...
TEST(MirLoweringC, ComplexMultiplyEmitsCrossTermShape) {
    auto L = lowerC(
        "double _Complex ga;\n"
        "double _Complex gb;\n"
...
TEST(MirLoweringC, ComplexDivideEmitsSharedDenominatorShape) {
    auto L = lowerC(
        "double _Complex ga;\n"
        "double _Complex gb;\n"
...
TEST(MirLoweringC, ComplexElementConvertEmitsTwoComponentConverts) {
    {
        auto L = lowerC(
            "float _Complex w;\n"
...
TEST(MirLoweringC, ShippedAtomicIntTypedefCarriesAtomicBit) {
...
TEST(MirLoweringC, AtomicExplicitAccessorsFoldMemoryOrderIntoPayload) {
    auto L = lowerAtomicProgram(
        "#include <stdatomic.h>\n"
        "int main(void) {\n"
        "    atomic_int x;\n"
...
TEST(MirLoweringC, AtomicVolatileScalarLowersToAtomicOpsAlone) {
    auto L = lowerC(
...
TEST(MirLoweringC, AtomicExplicitNonConstOrderFallsBackToSeqCst) {
    auto L = lowerAtomicProgram(
        "#include <stdatomic.h>\n"
        "int run(int ord) {\n"
        "    atomic_int x;\n"
...
TEST(MirLoweringC, EnumeratorLowersToConstNotUnboundRef) {
    auto L = lowerC(
...
TEST(MirLoweringC, EnumParamArithmeticLowersClean) {
    auto L = lowerC(
        "enum Color { RED, GREEN, BLUE };\n"
...
TEST(MirLoweringC, ReturnLiteralProducesConst) {
    auto L = lowerC("int f() { return 42; }");
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.hir->ok);
    ASSERT_TRUE(L.mir.ok);
...
TEST(MirLoweringC, VoidFunctionWithEmptyBodyGetsImplicitReturn) {
    auto L = lowerC("void f() {}");
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.hir->ok);
    ASSERT_TRUE(L.mir.ok)
        << "MIR lowering: " << (L.mirReporter.all().empty()
...
TEST(MirLoweringC, GlobalWithLiteralInitFoldsToConstant) {
    auto L = lowerC("int g = 42;\n");
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.hir->ok);
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
...
TEST(MirLoweringC, GlobalWithoutInitIsZeroInit) {
    auto L = lowerC("int g;\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
...
TEST(MirLoweringC, FunctionReadingGlobalEmitsGlobalAddrThenLoad) {
    auto L = lowerC(
        "int g = 7;\n"
        "int read_g() { return g; }\n");
    ASSERT_TRUE(L.mir.ok)
...
TEST(MirLoweringC, GlobalWithBinaryOpOnLiteralsFoldsToConstant) {
    auto L = lowerC("int g = 1 + 2;\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
    ASSERT_EQ(m.moduleGlobalCount(), 1u);
...
TEST(MirLoweringC, ReturnBoolFromIntFnEmitsZExt) {
    auto L = lowerC(
...
TEST(MirLoweringC, BareCharToIntPromotionIsTargetSignednessAware) {
    ...
    {
        auto L = lowerC(kSrc, "arm64", "aapcs64");
        ASSERT_TRUE(L.mir.ok)
            << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
        Mir const& m = L.mir.mir;
        auto const sexts = collectOps(m, MirOpcode::SExt);
...
TEST(MirLoweringC, BareCharSignednessIsResolvedPerTargetAndFormat) {
    constexpr char kSrc[] = "int f(char c) { return c; }";
    ...
    for (Row const row : {
        ...
        auto L = lowerC(kSrc, row.target, row.cc, DataModel::Lp64,
...
TEST(MirLoweringC, IntToPtrExtendsSourceToPointerWidthBySignedness) {
    // signed narrower (int, 32-bit) → SExt to pointer width, then IntToPtr.
    {
        auto L = lowerC("void* f(int x) { return (void*)x; }", "x86_64",
                              "sysv_amd64");
        ASSERT_TRUE(L.mir.ok)
            << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
        Mir const& m = L.mir.mir;
    ...
    {
        auto L = lowerC("void* g(unsigned x) { return (void*)x; }", "x86_64",
                              "sysv_amd64");
        ASSERT_TRUE(L.mir.ok)
            << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
        Mir const& m = L.mir.mir;
    ...
    {
        auto L = lowerC("void* h(long long x) { return (void*)x; }", "x86_64",
                              "sysv_amd64");
        ASSERT_TRUE(L.mir.ok)
            << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
        Mir const& m = L.mir.mir;
...
TEST(MirLoweringC, GlobalWithTernaryInitFoldsToSelectedArm) {
    auto L = lowerC("int g = 1 ? 7 : 9;\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
    ASSERT_EQ(m.moduleGlobalCount(), 1u);
...
TEST(MirLoweringC, GlobalWithLogicalAndShortCircuitsPastDivByZero) {
    auto L = lowerC("int g = 0 && (1 / 0);\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
    ASSERT_EQ(m.moduleGlobalCount(), 1u);
...
TEST(MirLoweringC, GlobalWithOverflowingInitFoldsWithModularWrap) {
    ...
    // c, every `int g = N;` produces such a Cast (literal core →
...
TEST(MirLoweringC, GlobalWithCastedLiteralFoldsToConstant) {
...
TEST(MirLoweringC, GlobalWithUnaryNegLiteralFoldsToConstant) {
    auto L = lowerC("int g = -7;\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
    ASSERT_EQ(m.moduleGlobalCount(), 1u);
...
// runtime-init.
TEST(MirLoweringC, GlobalWithFloatArithmeticInitializerFoldsThroughCastToInt) {
    auto L = lowerC("int g = 1.7 + 2.5;\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
    ASSERT_EQ(m.moduleGlobalCount(), 1u);
...
TEST(MirLoweringC, BlockScopeStaticIntToPtrReachesClassifyGlobals) {
    auto L = lowerC(
...
TEST(MirLoweringC, FunctionWritingGlobalEmitsGlobalAddrThenStore) {
    auto L = lowerC(
        "int g;\n"
...
TEST(MirLoweringC, DirectCallLowersToMirCall) {
    auto L = lowerC(
...
TEST(MirLoweringC, TernaryLowersToDiamondPhi) {
    auto L = lowerC(
...
TEST(MirLoweringC, LogicalAndShortCircuitsWithPhi) {
    auto L = lowerC(
...
TEST(MirLoweringC, LogicalOrShortCircuitsWithPhi) {
    auto L = lowerC(
...
TEST(MirLoweringC, IfConditionWithLogicalOpsVerifiesClean) {
    auto L = lowerC(
        "int pick(int a) {\n"
...
// short-circuit join).
TEST(MirLoweringC, LogicalOpsAsValuesVerifyClean) {
    auto L = lowerC(
        "int and2(int a, int b) { return a && b; }\n"
        "int or2(int a, int b) { return a || b; }\n");
    ASSERT_TRUE(L.mir.ok);
...
//   `return x;` then seals).
TEST(MirLoweringC, IfElseDiamondWithReturnsInBothArms) {
    auto L = lowerC(
        "int abs(int x) {\n"
        "  if (x < 0) return -x;\n"
        "  return x;\n"
...
// the exit block.
TEST(MirLoweringC, WhileLoopWithEarlyReturn) {
    auto L = lowerC(
        "void loop(int n) {\n"
        "  while (n > 0) { return; }\n"
        "}\n");
...
TEST(MirLoweringC, IfBothArmsReturnSealsJoinAsUnreachable) {
    auto L = lowerC(
        "int sign(int x) {\n"
        "  if (x < 0) return -1;\n"
        "  else return 1;\n"
...
TEST(MirLoweringC, WhileLoopBodyEmitsBackEdgeToHeader) {
    auto L = lowerC(
        "void spin(int n) {\n"
        "  while (n > 0) { n = n - 1; }\n"
        "}\n");
...
TEST(MirLoweringC, DoWhileBodyReturnsElidesCondTest) {
    auto L = lowerC(
        "void f(int n) {\n"
...
TEST(MirLoweringC, DoWhileBodyFallsThroughLowersCondTest) {
    auto L = lowerC(
        "void f(int n) {\n"
        "  do { n = n; } while (n > 0);\n"
        "}\n");
...
TEST(MirLoweringC, ForLoopLowersWithUpdateOnBackEdge) {
    auto L = lowerC(
        "void f(int n) {\n"
...
TEST(MirLoweringC, ForLoopWithAssignmentUpdateLowers) {
    auto L = lowerC(
        "int f(int i) {\n"
        "  int acc = 0;\n"
        "  for (i = 9; i; i = i - 1) {\n"
...
// single desugar site.
TEST(MirLoweringC, ForLoopCompoundAndIncDecUpdatesLower) {
    auto L = lowerC(
        "int f(int n) {\n"
        "  int acc = 0;\n"
...
TEST(MirLoweringC, ForLoopWithBareExpressionInitLowers) {
    auto L = lowerC(
...
TEST(MirLoweringC, LogicalNotLowersToICmpEqZero) {
    auto L = lowerC("int isz(int x) { return !x; }\n");
    ASSERT_TRUE(L.mir.ok)
        << "MIR lowering: " << (L.mirReporter.all().empty()
            ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
...
TEST(MirLoweringC, UnaryNegationLowersToNeg) {
    auto L = lowerC("int neg(int x) { return -x; }\n");
    ASSERT_TRUE(L.mir.ok);
    Mir const& m = L.mir.mir;
    MirBlockId const entry = m.funcEntry(m.funcAt(0));
    // [arg, Neg, Return]
...
// c's current builtinTypes mapping `int → I32` (signed), all arith
// goes through the signed forms. This pins the type-driven opcode-pick
// path; cycle 2+ adds unsigned types and floats.
TEST(MirLoweringC, SignedDivisionLowersToSDiv) {
    auto L = lowerC("int q(int a, int b) { return a / b; }");
    ASSERT_TRUE(L.mir.ok);
    Mir const& m = L.mir.mir;
    MirBlockId const entry = m.funcEntry(m.funcAt(0));
...
TEST(MirLoweringCLinkage, StaticUnusedFunctionIsDceEliminated) {
    auto L = lowerC(
...
TEST(MirLoweringCLinkage, NonStaticUnusedFunctionSurvivesDce) {
    auto L = lowerC(
...
TEST(MirLoweringCLinkage, WeakAttributeThreadsToMirBinding) {
    auto L = lowerC(
...
// `MirFunc.noInline`. The two GNU attribute POSITIONS the c grammar
...
class MirLoweringCNoInlineForm
    : public ::testing::TestWithParam<NoInlineFormCase> {};

TEST_P(MirLoweringCNoInlineForm, ThreadsToMirFuncNoInline) {
    auto const& c = GetParam();
    SCOPED_TRACE(c.label);
    auto L = lowerC(c.source);
    ASSERT_FALSE(L.model.hasErrors())
        << (L.model.diagnostics().all().empty()
                ? "" : L.model.diagnostics().all()[0].actual);
    ASSERT_TRUE(L.hir->ok);
...
INSTANTIATE_TEST_SUITE_P(
    Forms, MirLoweringCNoInlineForm,
    ::testing::Values(
...
TEST(MirLoweringCLinkage, UnannotatedFunctionHasNoInlineClear) {
    auto L = lowerC(
        "static int helper(int k) { return k + 5; }\n"
        "int main() { return helper(37); }\n");
    ASSERT_FALSE(L.model.hasErrors());
...
TEST(MirLoweringCLinkage, NoInlineSurvivesShippedReleasePipeline) {
    auto L = lowerC(
        "static int __attribute__((noinline)) helper(int k) { return k + 5; }\n"
        "int main() { return helper(37); }\n");
    ASSERT_FALSE(L.model.hasErrors());
...
class MirLoweringCAlwaysInlineForm
    : public ::testing::TestWithParam<AlwaysInlineFormCase> {};

TEST_P(MirLoweringCAlwaysInlineForm, ThreadsToMirFuncAlwaysInline) {
    auto const& c = GetParam();
    SCOPED_TRACE(c.label);
    auto L = lowerC(c.source);
    ASSERT_FALSE(L.model.hasErrors())
...
INSTANTIATE_TEST_SUITE_P(
    Forms, MirLoweringCAlwaysInlineForm,
    ::testing::Values(
        // Mode-2: the attribute sits BETWEEN the type and the declarator.
...
TEST(MirLoweringCLinkage, UnannotatedFunctionHasAlwaysInlineClear) {
    auto L = lowerC(
        "static int helper(int k) { return k + 5; }\n"
        "int main() { return helper(37); }\n");
    ASSERT_FALSE(L.model.hasErrors());
...
TEST(MirLoweringCLinkage, AlwaysInlineBypassesThresholdInShippedRelease) {
    auto L = lowerC(overThresholdSource(/*annotate=*/true).c_str());
    ASSERT_FALSE(L.model.hasErrors())
        << (L.model.diagnostics().all().empty()
                ? "" : L.model.diagnostics().all()[0].actual);
    ASSERT_TRUE(L.mir.ok);
...
// `["optimize"]` row from `c.lang.json` and the source stops compiling at
...
TEST(MirLoweringCNoOptimize, PragmaOptimizeRegionThreadsToMirFuncNoOptimize) {
    auto L = lowerC(noOptimizeRegionSource(/*guard=*/true).c_str());
    ASSERT_FALSE(L.model.hasErrors())
        << (L.model.diagnostics().all().empty()
                ? "" : L.model.diagnostics().all()[0].actual);
    ASSERT_TRUE(L.mir.ok);
...
TEST(MirLoweringCNoOptimize, WithoutThePragmaNoFunctionCarriesTheFlag) {
    auto L = lowerC(noOptimizeRegionSource(/*guard=*/false).c_str());
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.mir.ok);
    Mir const& m = L.mir.mir;
...
TEST(MirLoweringCNoOptimize, FlagSurvivesTheShippedReleasePipeline) {
    auto L = lowerC(noOptimizeRegionSource(/*guard=*/true).c_str());
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.mir.ok);
    Mir& m = L.mir.mir;
...
TEST(MirLoweringCLinkage,
     OverThresholdCalleeWithoutAlwaysInlineKeepsItsCall) {
    auto L = lowerC(overThresholdSource(/*annotate=*/false).c_str());
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.mir.ok);
    Mir& m = L.mir.mir;
...
TEST(MirLoweringCLinkage, AlwaysInlineIsInertUnderShippedDebugPipeline) {
    auto L = lowerC(overThresholdSource(/*annotate=*/true).c_str());
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.mir.ok);
    Mir& m = L.mir.mir;
    ...
    EXPECT_EQ(countCalls(), 1u)
        << "under the SHIPPED debug pipeline (Identity only) always_inline is "
           ...
           "c.lang.json effects row in the SAME commit: it states this "
...
class MirLoweringCNoSanitizeThreadForm
...
TEST_P(MirLoweringCNoSanitizeThreadForm, ThreadsToMirTextNoSanitizeThread) {
    auto const& c = GetParam();
    SCOPED_TRACE(c.label);
    auto L = lowerC(c.source);
    ASSERT_FALSE(L.model.hasErrors())
    ...
    // `no_sanitize_thread` effects row from c.lang.json while leaving the two
...
INSTANTIATE_TEST_SUITE_P(
    Forms, MirLoweringCNoSanitizeThreadForm,
    ::testing::Values(
        // Mode-2: the attribute sits BETWEEN the type and the declarator
...
// case above.
TEST(MirLoweringCLinkage, UnannotatedFunctionHasNoSanitizeThreadClear) {
    auto L = lowerC(
        "static int helper(int k) { return k + 5; }\n"
        "int main() { return helper(37); }\n");
    ASSERT_FALSE(L.model.hasErrors());
...
TEST(MirLoweringCLinkage, NoSanitizeThreadOnDataObjectWarnsAndStaysInert) {
    auto L = lowerC(
...
TEST(MirLoweringCLinkage, NoSanitizeThreadSurvivesShippedReleasePipeline) {
    auto L = lowerC(kNoSanitizeThreadMultiBlockCalleeSource);
    ASSERT_FALSE(L.model.hasErrors())
        << (L.model.diagnostics().all().empty()
                ? "" : L.model.diagnostics().all()[0].actual);
    ASSERT_TRUE(L.mir.ok);
...
TEST(MirLoweringCLinkage, NoSanitizeThreadSurvivesTheInlinersOwnRebuild) {
    auto L = lowerC(kNoSanitizeThreadMultiBlockCalleeSource);
    ASSERT_FALSE(L.model.hasErrors())
        << (L.model.diagnostics().all().empty()
                ? "" : L.model.diagnostics().all()[0].actual);
...
TEST(MirLoweringCLinkage, ExternFunctionDefinitionHasExternalLinkage) {
    auto L = lowerC(
...
TEST(MirLoweringCLinkage, StaticNoreturnKeepsInternalLinkage) {
    for (char const* src : {
...
TEST(MirLoweringCLinkage, GnuDeprecatedDoesNotClobberStaticLinkage) {
    auto L = lowerC(
...
// c.lang.json and the bare-constexpr arm's binding flips Global → RED.
TEST(MirLoweringCLinkage, FileScopeConstexprGetsInternalLinkage) {
...
TEST(MirLoweringCThreadLocal, ThreadLocalGlobalLowersWithFlag) {
    auto L = lowerC(
...
TEST(MirLoweringCThreadLocal, ThreadLocalLinkageAndStaticComposition) {
    struct Arm {
        char const*   src;
...
TEST(MirLoweringC, VarDeclWithInitLowersToAllocaPlusStore) {
    auto L = lowerC("int f() { int x = 5; return x; }");
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.hir->ok);
    ASSERT_TRUE(L.mir.ok)
        << "MIR lowering: " << (L.mirReporter.all().empty()
...
TEST(MirLoweringC, AssignStmtLowersToStore) {
    auto L = lowerC(
...
TEST(MirLoweringC, AddressOfLocalReturnsAllocaDirectly) {
    auto L = lowerC(
...
TEST(MirLoweringC, AddressOfParamPromotesItToSlot) {
    auto L = lowerC(
...
TEST(MirLoweringC, ParamWithoutAddressOfStaysAsArg) {
    auto L = lowerC("int id(int x) { return x; }");
    ASSERT_TRUE(L.mir.ok);
    Mir const& m = L.mir.mir;
    MirBlockId const entry = m.funcEntry(m.funcAt(0));
...
TEST(MirLoweringC, AssignThroughDerefStoresIntoPointerWithoutExtraLoad) {
    auto L = lowerC("void f(int* p, int v) { *p = v; }");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
    MirBlockId const entry = m.funcEntry(m.funcAt(0));
...
TEST(MirLoweringC, MemberAccessReadEmitsGepThenLoad) {
    auto L = lowerC(
...
TEST(MirLoweringC, BitFieldReadExtractsAndWriteIsReadModifyWrite) {
    auto L = lowerC(
...
TEST(MirLoweringC, BitFieldMutationValueAndCompoundFormsAreReadModifyWrite) {
    auto L = lowerC(
...
TEST(MirLoweringC, WideUnitBitFieldOnLongLongBaseCompilesAndLowers) {
    auto L = lowerC(
...
// `typeBits > 64` guard, but `__int128` is not in the c's spelled
// type set, so the BASE-128 path is unreachable from source and pinned
// by the guard logic + comment rather than a corpus.)
TEST(MirLoweringC, WideUnitBitFieldWidthAboveBaseStaysRejected) {
    auto L = lowerC(
...
TEST(MirLoweringC, BitFieldStructInitializerPacksPerUnit) {
    auto L = lowerC(
...
TEST(MirLoweringC, BitFieldUnitZeroPrecedesOrdinaryFieldStoreInSharedUnit) {
    auto L = lowerC(
...
TEST(MirLoweringC, EnumTypedBitFieldResolvesAndLowers) {
    auto L = lowerC(
...
TEST(MirLoweringC, MixedWidthBitFieldsSharingOffsetEachZeroTheirUnit) {
    auto L = lowerC(
...
TEST(MirLoweringC, NonStringArrayDecaysToPointerNotFailLoud) {
    auto L = lowerC(
        "int g[2] = { 1, 2 };\n"
...
// is not dropped. The corpus witness (examples/c/array_decay_deref) proves
...
// ASSERT.
TEST(MirLoweringC, DerefOfArrayInCommaTernaryLowersWithLoadAndSideEffectStore) {
    auto L = lowerC(
...
TEST(MirLoweringC, MemberAccessAssignEmitsGepThenStore) {
    auto L = lowerC(
        "struct Point { int x; int y; };\n"
...
TEST(MirLoweringC, IndexOverIntPointerScalesIndexToByteOffset) {
    auto L = lowerC(
...
TEST(MirLoweringC, CharIndexIntegerPromotesToI32BeforeStrideMul) {
    auto L = lowerC("int f(int* a, char i) { return a[i]; }\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
...
// shape to the pointer path. MIR-tier pin (lowerC stops at MIR).
// RED-ON-DISABLE: keep the 3-op form / raw index → the size==2 + Mul-stride
// assertions fail.
TEST(MirLoweringC, IndexIntoIntArrayCollapsesToTwoOpGepAndScales) {
    auto L = lowerC(
...
TEST(MirLoweringC, IndexIntoCharArrayDoesNotScale) {
    auto L = lowerC(
...
TEST(MirLoweringC, ArrayLocalBraceInitStoresPerElementAtStride) {
    auto L = lowerC(
...
TEST(MirLoweringC, ArrayFieldBraceInitStoresPerElementAtStride) {
    auto L = lowerC(
...
TEST(MirLoweringC, ArrayLocalAllocaReservesFullByteSize) {
    auto L = lowerC("int g(void) { int a[4]; return a[0]; }\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
    MirBlockId const entry = m.funcEntry(m.funcAt(0));
...
TEST(MirLoweringC, IndexIntoZeroSizeElementArrayFailsLoud) {
    auto L = lowerC(
        "struct E { };\n"
...
TEST(MirLoweringC, AddressOfMemberAccessReturnsGepDirectly) {
    auto L = lowerC(
        "struct Point { int x; int y; };\n"
...
TEST(MirLoweringC, NestedMemberAccessComposesChainedGepOffsets) {
    auto L = lowerC(
...
TEST(MirLoweringC, UnionWithStructTypedFieldParsesAndAccesses) {
    auto L = lowerC(
        "typedef struct { int x; int y; } Inner;\n"
...
// compound-assign in c. `x = 5` as an rvalue is the canonical case:
// the AssignStmt becomes a SeqExpr whose tail loads the new value.
TEST(MirLoweringC, SeqExprLowersStmtsThenYieldsResult) {
    auto L = lowerC(
...
TEST(MirLoweringC, SwitchWithBreaksInEachArm) {
    auto L = lowerC(
        "void f(int x) {\n"
        "  switch (x) {\n"
        "    case 1: break;\n"
...
// exit].
TEST(MirLoweringC, SwitchFallthroughBranchesToNextArm) {
    auto L = lowerC(
        "void f(int x) {\n"
        "  switch (x) {\n"
...
TEST(MirLoweringC, ContinueInsideWhileBranchesToHeader) {
    auto L = lowerC(
        "void f(int n) {\n"
        "  while (n > 0) { continue; }\n"
        "}\n");
...
TEST(MirLoweringC, BreakInsideWhileBranchesToExit) {
    auto L = lowerC(
        "void f(int n) {\n"
        "  while (n > 0) { break; }\n"
        "}\n");
...
TEST(MirLoweringC, ContinueInsideDoWhileBranchesToCondTest) {
    auto L = lowerC(
        "void f(int n) {\n"
...
// discipline.
TEST(MirLoweringC, BreakInsideSwitchArmTargetsSwitchExit) {
    auto L = lowerC(
        "void f(int x) {\n"
        "  while (x > 0) {\n"
...
TEST(MirLoweringC, MultipleFunctionsEachGetIsolatedMirFunc) {
    auto L = lowerC(
...
TEST(MirLoweringC, ForwardReferenceCallResolvesViaPrePass) {
    auto L = lowerC(
...
TEST(MirLoweringC, VarDeclWithoutInitOnlyAllocas) {
    auto L = lowerC(
...
// `lowerC`-ordered. Order-preserving so consumers can assert
...
TEST(MirLoweringC, ByValueCompoundLiteralArgMaterializesSlotElementWise) {
    auto L = lowerC(
...
TEST(MirLoweringC, ByValueBitfieldCompoundLiteralArgPacksPerUnit) {
    auto L = lowerC(
...
TEST(MirLoweringC, DiscardedAggregateCompoundLiteralStatementRunsSideEffects) {
    auto L = lowerC(
        "struct P { int a; int b; };\n"
        "int h(void) { return 7; }\n"
...
TEST(MirLoweringC, ConstructAggregateAllConstFoldsToConst) {
    auto L = lowerC(
        "struct Point { int x; int y; };\n"
...
TEST(MirLoweringC, StructLocalRuntimeInitEmitsPerFieldStores) {
    auto L = lowerC(
        "struct Point { int x; int y; };\n"
...
// InsertValue.
TEST(MirLoweringC, UnionLocalRuntimeInitEmitsOneFieldStore) {
    auto L = lowerC(
...
TEST(MirLoweringC, ConstructAggregateUnionNonZeroVariantRuntimeOk) {
    auto L = lowerC(
        "union U { int i; char c; };\n"
...
TEST(MirLoweringC, ArrayLocalRuntimeInitStoresElementWise) {
    auto L = lowerC(
...
TEST(MirLoweringC, StructLocalInitStoresEachFieldAtItsOffset) {
    auto L = lowerC(
        "struct Point { int x; int y; };\n"
...
TEST(MirLoweringC, StructLocalNestedInitRecursesIntoSubSlot) {
    auto L = lowerC(
        "typedef struct { int x; int y; } Inner;\n"
        "typedef struct { Inner in; int z; } Outer;\n"
...
TEST(MirLoweringC, ConstructAggregateNestedAllConstFolds) {
    auto L = lowerC(
        "struct Inner { int v; };\n"
...
TEST(MirLoweringC, UnionCopyIsByteWiseNotFieldWise) {
    auto L = lowerC(
...
TEST(MirLoweringC, StructWithStructFieldCopyIsByteWise) {
    auto L = lowerC(
...
TEST(MirLoweringC, StructWithArrayFieldCopyIsByteWise) {
    auto L = lowerC(
...
TEST(MirLoweringC, SysVStructByValueParamReceivesOneRegisterPiece) {
    auto L = lowerC(
...
TEST(MirLoweringC, SysVStructByValueCallPassesOneRegisterPiece) {
    auto L = lowerC(
        "struct Pair { int x; int y; };\n"
        "int sum(struct Pair p) { return p.x + p.y; }\n"
...
TEST(MirLoweringC, SysVTwoEightbyteStructParamReceivesTwoRegisterPieces) {
    auto L = lowerC(
...
TEST(MirLoweringC, SysVByRefStructParamReceivesOnePointerArg) {
    auto L = lowerC(
...
TEST(MirLoweringC, SysVTwoEightbyteStructReturnEmitsMultiOperandReturn) {
    auto L = lowerC(
...
TEST(MirLoweringC, SysVByRefStructReturnUsesSretHiddenPointer) {
    auto L = lowerC(
...
TEST(MirLoweringC, SysVMixedClassStructReturnSplitsAcrossRegisterClasses) {
    auto L = lowerC(
...
TEST(MirLoweringC, StructReturningCallEmitsOneCallAndOneReturnPiece) {
    auto L = lowerC(
        "typedef struct { int x; int y; int z; } Tri;\n"
        "Tri mk(int a, int b, int c) { Tri t; t.x=a; t.y=b; t.z=c; return t; }\n"
...
// ML3 end-to-end: ML2-lowered MIR for a representative c
...
TEST(MirLoweringC, Ml3VerifierAcceptsRealLoweredMir) {
    Lowered L = lowerC(
...
TEST(MirLoweringC, Ml4TextFormatRoundTripsRealMir) {
    Lowered L = lowerC(
        "int add(int x, int y) { return x + y; }\n"
        "int factorial(int n) {\n"
        "  if (n <= 1) return 1;\n"
...
// `1.7 + 2.5;` (a bare expression statement — c has no float
...
TEST(MirLoweringC, BodyF64LiteralPromotesToAnonymousGlobalPlusLoad) {
    auto L = lowerC("int f() { 1.7 + 2.5; return 4; }\n");
    ASSERT_FALSE(L.model.hasErrors())
        << "semantic phase: " << (L.model.diagnostics().all().empty()
...
TEST(MirLoweringC, IdenticalStringLiteralsShareOneRodataGlobal) {
    auto L = lowerC(
        "int f() {\n"
        "    const char *a;\n"
        "    const char *b;\n"
...
// exit).
TEST(MirLoweringC, FuncNameReadsShareOneRodataGlobal) {
    auto L = lowerC(
...
TEST(MirLoweringC, FuncNameStaticInitializerSharesTheBodyReadGlobal) {
    auto L = lowerC(
        "int main() {\n"
...
TEST(MirLoweringC, MixedSignCompareLowersUnsignedWithExplicitCast) {
    auto L = lowerC(
...
// program links + runs in `examples/c/shipped_tcl_wideint` (the RUN witness).
TEST(MirLoweringC, DirectCallIntPointeeArgRealizesAsPtrBitcast) {
...
TEST(MirLoweringC, CharPlusIntPromotesToI32WithSExt) {
    auto L = lowerC("int f(char c) { return c + 1; }");
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.mir.ok);
...
TEST(MirLoweringC, ShortPlusShortPromotesToI32) {
    auto L = lowerC("int f(short a, short b) { return a + b; }");
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.mir.ok);
    EXPECT_EQ(firstOpTypeKind(L, MirOpcode::Add), TypeKind::I32);
...
TEST(MirLoweringC, MixedI32U64AddComputesU64) {
    auto L = lowerC(
...
TEST(MirLoweringC, BareIntIfConditionLowersAsICmpNeNotTrunc) {
    auto L = lowerC("int main() { if (2) return 42; return 7; }");
...
TEST(MirLoweringC, BareIntWhileConditionLowersAsICmpNeNotTrunc) {
    auto L = lowerC(
        "int f(int n) { while (n) { n = n - 1; } return n; }");
    expectTruthinessNe(L, "while");
}

TEST(MirLoweringC, BareIntDoWhileConditionLowersAsICmpNeNotTrunc) {
    auto L = lowerC(
        "int f(int n) { do { n = n - 1; } while (n); return n; }");
    expectTruthinessNe(L, "do-while");
}

TEST(MirLoweringC, BareIntForConditionLowersAsICmpNeNotTrunc) {
...
TEST(MirLoweringC, BareIntTernaryConditionLowersAsICmpNeNotTrunc) {
    auto L = lowerC("int f(int c) { return c ? 1 : 2; }");
    expectTruthinessNe(L, "ternary");
}

TEST(MirLoweringC, CallResultIfConditionLowersAsICmpNeNotTrunc) {
    auto L = lowerC(
        "int two() { return 2; }\n"
...
TEST(MirLoweringC, BoolIfConditionKeepsExactlyOneComparison) {
    auto L = lowerC(
...
TEST(MirLoweringC, FloatIfConditionLowersAsFCmpUneNotFpToUi) {
    auto L = lowerC(
...
TEST(MirLoweringC, CharIfConditionIntegerPromotesToI32BeforeICmpNe) {
    auto L = lowerC(
...
// the language admits integer-zero null-pointer constants (c
...
TEST(MirLoweringC, PointerIfConditionLowersAsICmpNeAgainstNullConstant) {
    auto L = lowerC(
...
TEST(MirLoweringC, ShiftResultIsPromotedLeftOperand) {
    auto L = lowerC(
...
TEST(MirLoweringC, BoolKeywordLiteralsCarryFixedValues) {
    auto L = lowerC(
        "bool t() { return true; }\n"
        "bool f() { return false; }\n");
    ASSERT_FALSE(L.model.hasErrors());
...
//
// END-TO-END through the REAL c attribute (not hand-built MIR —
...
// `"visibility:hidden"` row from c.lang.json's linkageSpecifiers
// and hidden_unused stays Default -> retained -> this test goes RED.
TEST(MirLoweringC, HiddenVisibilityUnusedFunctionIsDceEliminated) {
    auto L = lowerC(
...
// Independently pinned at the HIR tier (HirLoweringC.VisibilityDefault*),
// including the two-rows-are-independent perturbation lever.
TEST(MirLoweringC, VisibilityDefaultAttributeAppliesDefaultVisibility) {
    auto L = lowerC(
...
TEST(MirLoweringC, Aapcs64StructSretUsesReadIndirectResultAndFlagsCall) {
    auto L = lowerC(
        "typedef struct { long a; long b; long c; } Big;\n"
...
TEST(MirLoweringC, ByValueAggregateTernaryMaterializesCommonSlotUnderDiamond) {
    // `pick` is a runtime PARAMETER (keeps the condition runtime so neither arm
    // folds away — c has no bare prototypes; a param is the clean lever).
    auto L = lowerC(
        "struct P { int a; int b; };\n"
        "int take(struct P p) { return p.a + p.b; }\n"
...
TEST(MirLoweringC, ByValueBitfieldAggregateTernaryPacksPerUnitUnderDiamond) {
    auto L = lowerC(
        "struct B { unsigned a : 3; unsigned b : 5; };\n"
        "int take(struct B v) { return (int)v.a + (int)v.b; }\n"
        "int run(int pick, int a) {\n"
...
TEST(MirLoweringC, AggregateSeqExprByValueLowersWithSideEffectAndSlot) {
    auto L = lowerC(
        "struct P { int a; int b; };\n"
...
TEST(MirLoweringC, ByValueAggregateTernaryNamedLvalueArmsCopyIntoCommonSlot) {
    auto L = lowerC(
        "struct P { int a; int b; };\n"
        "int take(struct P p) { return p.a + p.b; }\n"
        "int run(int pick, int a) {\n"
...
TEST(MirLoweringC, DiscardedAggregateTernaryNamedArmsRoutesThroughSlotNoPhi) {
    auto L = lowerC(
        "struct P { int a; int b; };\n"
...
TEST(MirLoweringC, ForUpdateAggregateTernaryRoutesThroughSlotNoPhi) {
    auto L = lowerC(
        "struct P { int a; int b; };\n"
        "int sink(int x) { return x; }\n"
...
TEST(MirLoweringC, SizeofDerefArrayFoldsElementNotWholeArray) {
    auto L = lowerC("int g(void) { int a[10]; return (int)sizeof(*a); }\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
...
TEST(MirLoweringC, SizeofDerefTwoDimArrayFoldsRowNotWholeArray) {
    auto L = lowerC("int g(void) { int m[3][4]; return (int)sizeof(*m); }\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
...
TEST(MirLoweringC, VaStartEmitsFourFieldStoresPlusFrameAddrs) {
    auto L = lowerC(
        "int sum(int n, ...) {\n"
        "  va_list ap;\n"
        "  va_start(ap, n);\n"
...
TEST(MirLoweringC, VaArgIntLowersToRegVsOverflowDiamond) {
    auto L = lowerC(
        "int sum(int n, ...) {\n"
        "  va_list ap;\n"
        "  va_start(ap, n);\n"
...
TEST(MirLoweringC, Win64VaArgIntLowersLinearNotDiamond) {
    char const* src =
        "int sum(int n, ...) {\n"
        "  va_list ap;\n"
        "  va_start(ap, n);\n"
    ...
    auto S = lowerC(src, "x86_64", "sysv_amd64");
...
TEST(MirLoweringC, VaArgStructMixedLowersToAtomicRegisterGather) {
    auto L = lowerC(
...
TEST(MirLoweringC, VaArgStructOver16BDispatchesToOverflowArm) {
    auto L = lowerC(
        "struct Big { long a; long b; long c; };\n"   // 24B → MEMORY class
        "long sum(int n, ...) {\n"
        "  va_list ap;\n"
...
TEST(MirLoweringC, Aapcs64FieldOffsetHelperSelfTest) {
    auto L = lowerC(
        "int sum(int n, ...) {\n"
        "  va_list ap;\n"
        "  va_start(ap, n);\n"
...
TEST(MirLoweringC, Win64VaArgStructLeq8ReturnsSlotAddr) {
    auto L = lowerC(
...
TEST(MirLoweringC, Win64VaArgStructGt8DerefsPointer) {
    auto L = lowerC(
...
TEST(MirLoweringC, Win64StructByValueVarargCallLeq8OneSlot) {
    auto L = lowerC(
...
TEST(MirLoweringC, Win64StructByValueVarargCallGt8OnePointer) {
    auto L = lowerC(
        "struct Big { long a; long b; long c; };\n"  // 24B → ByReference (hidden ptr)
        "int sink(int n, ...) { return n; }\n"   // DEFINED so the callee symbol binds (D-CSUBSET-FN-PROTOTYPE: a bare proto no longer emits a spurious FnSig global)
        "int main(void) {\n"
...
TEST(MirLoweringC, VaStartFixedParamsOverflowToStackSucceeds) {
...
TEST(MirLoweringC, Aapcs64VaStartFixedParamsOverflowToStackSucceeds) {
...
TEST(MirLoweringC, Aapcs64VaStartFixedDoubleParamsOverflowToStackSucceeds) {
    auto L = lowerC(
...
TEST(MirLoweringC, Aapcs64VaStartMixedClassFixedStackOverflow) {
    auto L = lowerC(
...
TEST(MirLoweringC, VaStartFixedStructStraddleOverflowSucceeds) {
    auto L = lowerC(
...
TEST(MirLoweringC, Aapcs64VaStartFixedHfaStraddleOverflowSucceeds) {
    auto L = lowerC(
...
TEST(MirLoweringC, CallerNonVariadicStraddlingAggregateUsesStackCarrier) {
    auto L = lowerC(
        "struct S16 { long x; long y; };\n"
...
TEST(MirLoweringC, VariadicCallWithStructStampsFixedOperandCount) {
...
TEST(MirLoweringC, VariadicCallSretHiddenArgCountedInFixedOperandCount) {
    using namespace dss::call_payload;
    auto L = lowerC(
...
TEST(MirLoweringC, VariadicFnFixedStructParamThreadsBothClassCursors) {
    auto L = lowerC(
        "struct Pt { long a; double b; };\n"
...
TEST(MirLoweringC, VariadicCallStructOver16BByValueUsesCarrier) {
    auto L = lowerC(
        "struct Big { long a; long b; long c; };\n"   // 24B → MEMORY class
...
TEST(MirLoweringC, VariadicFnFixedStructOver16BParamThreadsHiddenPtrCursor) {
    auto L = lowerC(
        "struct Big { long a; long b; long c; };\n"   // 24B → MEMORY class
...
TEST(MirLoweringC, VariadicCallStructRegisterExhaustionSplitRoutesToCarrier) {
    auto L = lowerC(
...
TEST(MirLoweringC, DiscardedAggregateVaArgRoutesByAddressNoAggregatePhi) {
    auto L = lowerC(
        "struct Pt { long a; double b; };\n"
        "long sum(int n, ...) {\n"
        "  va_list ap;\n"
...
TEST(MirLoweringC, VaArgTwoSameClassStructThreadsCursorThroughAdd) {
    auto L = lowerC(
        "struct LL { long a; long b; };\n"
        "long sumLL(int n, ...) {\n"
        "  va_list ap;\n"
...
TEST(MirLoweringC, Aapcs64VaArgIntLowersToDualCursorDiamondSignExtend) {
    auto L = lowerC(
        "int sum(int n, ...) {\n"
        "  va_list ap;\n"
        "  va_start(ap, n);\n"
...
TEST(MirLoweringC, Aapcs64VaStartInitsNegativeGrOffs) {
    auto L = lowerC(
        "int sum(int n, ...) {\n"
        "  va_list ap;\n"
        "  va_start(ap, n);\n"
...
TEST(MirLoweringC, Aapcs64VaArgStructGathersViaDualCursor) {
    auto L = lowerC(
...
TEST(MirLoweringC, Aapcs64StructByValueVarargCallPlacesInRegisters) {
    auto L = lowerC(
        "struct Pt { int a; int b; };\n"
        "int sink(int n, ...) { return n; }\n"   // DEFINED so the callee symbol binds (D-CSUBSET-FN-PROTOTYPE: a bare proto no longer emits a spurious FnSig global)
        "int main(void) {\n"
...
TEST(MirLoweringC, Aapcs64VaArgHfaFromVrNotGr) {
    auto L = lowerC(
...
TEST(MirLoweringC, Aapcs64VaArgNonHfaFromGrNotVr) {
    auto L = lowerC(
...
TEST(MirLoweringC, Aapcs64VaArgByRefUsesChainedLoad) {
    auto L = lowerC(
        "struct Big { long a; long b; long c; };\n"   // 24B → ByReference (hidden ptr)
        "long pick(int n, ...) {\n"
        "  va_list ap;\n"
...
TEST(MirLoweringC, Aapcs64VaArgByRefRegArmDerefsGrTop) {
    auto L = lowerC(
        "struct Big { long a; long b; long c; };\n"   // 24B → ByReference (hidden ptr)
        "long pick(int n, ...) {\n"
        "  va_list ap;\n"
...
TEST(MirLoweringC, Aapcs64CallerHfaVarargInRegisters) {
    auto L = lowerC(
        "struct HFA { double a; double b; };\n"
        "int sink(int n, ...) { return n; }\n"   // DEFINED so the callee symbol binds (D-CSUBSET-FN-PROTOTYPE: a bare proto no longer emits a spurious FnSig global)
        "int main(void) {\n"
...
TEST(MirLoweringC, Aapcs64CallerNonHfaVarargExhaustsGpr) {
    auto L = lowerC(
        "struct LL { long a; long b; };\n"
...
TEST(MirLoweringC, Aapcs64CallerByRefVarargIsPositionedPointerNotCarrier) {
    auto L = lowerC(
        "struct Big { long a; long b; long c; };\n"  // 24B → ByReference (hidden ptr)
        "int sink(int n, ...) { return n; }\n"   // DEFINED so the callee symbol binds (D-CSUBSET-FN-PROTOTYPE: a bare proto no longer emits a spurious FnSig global)
        "int main(void) {\n"
...
TEST(MirLoweringC, AppleVaArgStructSmallByValueLinear) {
    auto L = lowerC(
...
TEST(MirLoweringC, AppleVaArgStructLargeByRefLinear) {
    auto L = lowerC(
        "struct Big { long a; long b; long c; };\n"   // 24B → ByReference (hidden ptr)
        "long pick(int n, ...) {\n"
        "  va_list ap;\n"
...
TEST(MirLoweringC, AppleVaArgHfa16BBumpsBySixteenNotEight) {
    auto L = lowerC(
...
TEST(MirLoweringC, AppleVaArgIntLowersLinearOverflowBase) {
    auto L = lowerC(
        "int sum(int n, ...) {\n"
        "  va_list ap;\n"
        "  va_start(ap, n);\n"
...
TEST(MirLoweringC, ComputedGotoIndirectBrSuccessorsAreAllAddressTakenBlocks) {
...
TEST(MirLoweringC, ComputedGotoBlockAddressTargetsAreAddressTaken) {
...
TEST(MirLoweringC, IterativeDeepBinaryChainLowersFlatAndByteIdentical) {
...
TEST(MirLoweringC, IterativeDeepUnaryChainLowersFlatAndByteIdentical) {
...
TEST(MirLoweringC, IterativeDeepIndexAddressChainLowersFlatAndByteIdentical) {
...
TEST(MirLoweringC, IterativeDeepTernaryChainLowersFlatAndByteIdentical) {
...
TEST(MirLoweringC, IterativeDeepLogicalChainLowersFlatAndByteIdentical) {
...
TEST(MirLoweringC, IterativeDeepCallChainLowersFlatAndByteIdentical) {
...
TEST(MirLoweringC, IterativeDeepIfNestLowersFlatAndByteIdentical) {
...
// miscompile FIX (the end-to-end runtime witness is examples/c/core_incdec,
...
TEST(MirLoweringC, PointerIncrementScalesByElementSizeViaGep) {
    auto L = lowerC("void f(int* p) { p++; }\n");
    ASSERT_TRUE(L.mir.ok)
        << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
    MirBlockId const entry = m.funcEntry(m.funcAt(0));
...
// adds. The same `volatileMap` is threaded by `lowerC` above.
// ─────────────────────────────────────────────────────────────────────────────
...
TEST(MirLoweringCVolatile, LocalObjectLoadAndStoreFlagged) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, VolatileMemberLoadFlaggedNonVolatileSiblingNot) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, VolatileGlobalLoadAndStoreFlagged) {
    auto L = lowerC(
        "volatile int gv;\n"
...
TEST(MirLoweringCVolatile, VolatilePointerObjectStoreFlagged) {
    auto L = lowerC(
        "int g;\n"
...
TEST(MirLoweringCVolatile, NonVolatileBaselineHasNoVolatileFlags) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, PointerToVolatilePointeeDerefIsVolatile) {
...
TEST(MirLoweringCVolatile, VolatileLocalLoadSurvivesMem2Reg) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, VolatileAddressTakenParamInitStoreFlagged) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, NonVolatileAddressTakenParamInitStoreNotFlagged) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, WholeVolatileAggregateAssignCopyFlagged) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, NonVolatileAggregateAssignCopyNotFlagged) {
    auto L = lowerC(
        "struct S { int x; int y; };\n"
        "struct S b;\n"
...
TEST(MirLoweringCVolatile, StructMemberPointeeVolatileNowCompilesAndDerefVolatile) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, UnionMemberPointeeVolatileNowCompiles) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, StructMemberDoublePointeeVolatileInnermostOnly) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, StructMemberPointeeVolatileListEverySlotVolatile) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, VolatileTypedefCarriesThroughType) {
    {
        auto L = lowerC(
...
TEST(MirLoweringCVolatile, ConstPointeeStaysNonVolatile) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, SizeofVolatileEqualsSizeofUnqualified) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, VolatileArrayElementIndexIsVolatile) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, ContainerVolatileMemberReadThroughPtr) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, ContainerVolatileMemberWriteThroughPtr) {
    auto L = lowerC(
        "struct S { int a; int b; };\n"
...
TEST(MirLoweringCVolatile, ContainerVolatileMemberOfLocal) {
    auto L = lowerC(
        "struct S { int a; int b; };\n"
...
TEST(MirLoweringCVolatile, ContainerVolatileIndexOfArray) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, ContainerVolatileNestedMemberPropagates) {
    auto L = lowerC(
        "struct Inner { int x; };\n"
...
TEST(MirLoweringCVolatile, ContainerVolatileCompoundAssignMember) {
    auto L = lowerC(
        "struct C { double s; };\n"
...
TEST(MirLoweringCVolatile, ContainerVolatileByValueArg) {
    auto L = lowerC(
...
TEST(MirLoweringCVolatile, ContainerVolatileStructReturn) {
    auto L = lowerC(
        "struct S { long a; long b; long c; };\n"
...
TEST(MirLoweringCVolatile, ContainerVolatileBraceInit) {
    auto L = lowerC(
        "struct S { int a; int b; };\n"
...
TEST(MirLoweringCVolatile, ContainerVolatileIndexThenMember) {
    auto L = lowerC(
        "struct S { int a; };\n"
...
TEST(MirLoweringCVolatile, TwoVolatileStoresToDifferentGlobalsKeepRelativeOrder) {
    auto L = lowerC(
        "volatile int g1, g2;\n"
...
TEST(MirLoweringCVolatile, VolatileStoreThroughPointerParamSurvivesDce) {
    auto L = lowerC(
...
// frame slot.
TEST(MirLoweringC, C35ValueOfIncompleteFailsLoud) {
    auto L = lowerC(
        "struct S;\n"
...
TEST(MirLoweringC, C35MemberOfIncompletePointerFailsLoud) {
    auto L = lowerC(
        "struct S;\n"
...
TEST(MirLoweringC, C35SizeofOfIncompleteFailsLoud) {
    auto L = lowerC(
        "struct S;\n"
...
TEST(MirLoweringC, C35OpaqueHandleViaPointerLowersClean) {
    auto L = lowerC(
        "struct Stmt;\n"
...
TEST(MirLoweringC, ArrayRvalueValueReadDecaysNeverLoads) {
    auto L = lowerC(
...
TEST(MirLoweringC, SehTryExceptLowersToRegionSkeleton) {
    auto L = lowerC(
...
TEST(MirLoweringC, SehSurvivesFullReleasePipeline) {
    auto L = lowerC(
        "int g;\n"
        "static int helper(int x) { return x + 1; }\n"   // an inline candidate
...
TEST(MirLoweringC, SehFunctionKeepsAllocasUnpromoted) {
    auto L = lowerC(
        "int f(int *p) { int rc = 0; __try { rc = *p; } "
...
TEST(MirLoweringC, SehInsideLoopStaysVerifierGreen) {
    auto L = lowerC(
...
TEST(MirLoweringC, WideBitIntMulDivModLowersAtC3) {
...
TEST(MirLoweringC, NarrowBitIntFloatConversionLowersGreen) {
    for (char const* src : {
...
TEST(MirLoweringC, WideBitIntEasyOpsLowerGreen) {
    auto L = lowerC(
...
// the one pinned here.
TEST(MirLoweringC, WideIntNarrowingCarriesDeclaredTypeId) {
...
TEST(MirLoweringC, WideIntNarrowingToEnumAndFromBitIntCarriesDeclaredTypeId) {
    ...
    for (Row const r : {
        ...
        Lowered L = lowerC(std::string{r.src});
        ASSERT_FALSE(L.model.hasErrors())
            << "\n" << r.what << "\n"
            << (L.model.diagnostics().all().empty() ? "" : L.model.diagnostics().all()[0].actual);
        ASSERT_TRUE(L.hir->ok)
...
TEST(MirLoweringC, IncDecOnWideIntegerLowersGreenForEverySpelling) {
...
TEST(MirLoweringC, IncDecOnUnsignedWideIntegerZeroFillsItsUpperLimbs) {
    ...
    auto build = [&](char const* decl) {
        return lowerC(std::string{"int main(void) { "} + decl
                            + " x = 41; x++; return (int)x; }\n");
    };
...
TEST(MirLoweringC, VlaLocalLowersToRuntimeOperandAllocaAtDeclPoint) {
    auto L = lowerC(
        "int main(void) {\n"
        "  int n;\n"
        "  n = 4;\n"
...
TEST(MirLoweringC, SizeofOfVlaLoadsDeclFrozenSizeSlot) {
    auto L = lowerC(
        "int main(void) {\n"
        "  int n;\n"
        "  n = 4;\n"
...
TEST(MirLoweringC, VlaTypedefFreezesSizeAtTypeDeclStatement) {
    auto L = lowerC(
        "int main(void) {\n"
        "  int n;\n"
        "  n = 3;\n"
...
TEST(MirLoweringC, VlaTypedefObjectAllocaLoadsFrozenSizeNotReLoweredN) {
    auto L = lowerC(
        "int main(void) {\n"
        "  int n;\n"
        "  n = 3;\n"
...
TEST(MirLoweringC, MultiDimVlaLowersWithRuntimeStrideSlots) {
    auto L = lowerC(
        "int main(void) {\n"
...
TEST(MirLoweringC, FixedMultiDimArrayDoesNotRouteToVlaPath) {
    auto L = lowerC(
        "int main(void) {\n"
        "  int b[5][5];\n"
        "  b[1][0] = 7;\n"
...
TEST(MirLoweringC, RowSizeofOfMultiDimVlaLoadsStrideSlot) {
    auto L = lowerC(
        "int main(void) {\n"
        "  volatile int vn = 3, vm = 5;\n"
        "  int n = vn, m = vm;\n"
...
TEST(MirLoweringC, VlaInLoopBodyEmitsBlockScopeTeardown) {
    auto L = lowerC(
        "int main(void) {\n"
        "  volatile int vn = 4;\n"
        "  int n = vn;\n"
...
TEST(MirLoweringC, VlaBreakContinueGotoEmitStackRestore) {
...
TEST(MirLoweringC, NonVlaLoopBodyEmitsNoStackTeardown) {
    auto L = lowerC(
        "int main(void) {\n"
        "  int i;\n"
        "  for (i = 0; i < 3; i = i + 1) {\n"
...
TEST(MirLoweringC, NestedGotoRestoreTargetsShallowestExitedFrame) {
    auto L = lowerC(
        "int main(void) {\n"
        "  volatile int vn = 4;\n"
        "  int n = vn;\n"
...
TEST(MirLoweringC, ForInitVlaEmitsForScopeTeardownAtLoopExitOnly) {
    auto L = lowerC(
        "int main(void) {\n"
        "  volatile int vn = 4;\n"
        "  int n = vn;\n"
...
TEST(MirLoweringC, ForInitAndBodyVlaTornDownAtDistinctPoints) {
    auto L = lowerC(
        "int main(void) {\n"
        "  volatile int vn = 4;\n"
        "  int n = vn;\n"
...
TEST(MirLoweringC, PtrToVlaLocalSubscriptScalesByDeclSiteStrideSlot) {
    auto L = lowerC(
        "int main(void) {\n"
        "  volatile int vn = 4;\n"
        "  int n = vn;\n"
...
TEST(MirLoweringC, PtrToVlaPointerArithSubscriptFailsLoud) {
    auto L = lowerC(
        "int main(void) {\n"
        "  volatile int vn = 3;\n"
        "  int n = vn;\n"
...
TEST(MirLoweringC, ParamPtrToVlaSubscriptScalesByPrologueStrideSlot) {
    auto L = lowerC(
...
TEST(MirLoweringC, BitCountBuiltinsLowerToDedicatedMirOps) {
    auto L = lowerC(
...
// zero extra engine code: the six c.lang.json rows differ only in their
// param/result cores.
//
...
// c.lang.json (or delete the `case BuiltinLowering::Bswap:` arm in
// hir_to_mir) → nBswap drops below 6 AND nCall rises above 0.
TEST(MirLoweringC, ByteSwapBuiltinsAllRouteToOneMirBswapNeverACall) {
    auto L = lowerC(
...
// rule `asmStmt` — it lives in `asm.lang.json`, NOT in `c.lang.json`, and
// C inherits it through `languageReferences`; this recipe said c until
...
TEST(MirLoweringC, InlineAsmEmptyTemplateLowersToCompilerBarrier) {
    auto L = lowerC("void f(void){ __asm__ volatile(\"\"); }");
...
TEST(MirLoweringC, InlineAsmEmptyTemplateNoVolatileLowersToCompilerBarrier) {
    auto L = lowerC("void f(void){ __asm__(\"\"); }");
    ASSERT_TRUE(L.mir.ok) << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
    auto ids = stdbitAllEntryInsts(m);
    EXPECT_EQ(stdbitCountOp(m, ids, MirOpcode::CompilerBarrier), 1);
...
TEST(MirLoweringC, InlineAsmEmptySectionFormsEachLowerToOneCompilerBarrier) {
...
// production never gets here. THIS TIER HAS NO SUCH GUARD: `lowerC` at the
...
TEST(MirLoweringC, AsmPayloadHirLoweringCannotREPRESENTIsRefusedNotDropped) {
    ...
    // ...and lowering ran ANYWAY (lowerC has no `hasErrors()` bail, on
...
TEST(MirLoweringC, InlineAsmExtendedSurfaceIsCarriedNotReconstructed) {
    struct Row {
        char const* src;
        bool        extended;
        char const* why;
...
TEST(MirLoweringC, ReadWriteAsmOperandCarriesItsReadHalfTiedToItsOutput) {
    auto L = lowerC(
...
TEST(MirLoweringC, AsmOutputStoreBackGoesThroughTheScalarFunnel) {
...
// c.lang.json to `"barrier"` (or drop the addInst in hir_to_mir's
...
TEST(MirLoweringC, SyncSynchronizeLowersToAtomicFenceSeqCst) {
    auto L = lowerC("void f(void){ __sync_synchronize(); }");
    // The builtin is ALWAYS-injected (c.lang.json builtinFunctions), so an
...
TEST(MirLoweringC, StdbitLeadingZerosComposesClz) {
    auto L = lowerC(
...
TEST(MirLoweringC, StdbitCountOnesComposesPopcount) {
    auto L = lowerC(
...
TEST(MirLoweringC, StdbitBitWidthComposesClzSub) {
    auto L = lowerC(
...
TEST(MirLoweringC, StdbitHasSingleBitIsPopcountEqOneBool) {
    auto L = lowerC(
...
TEST(MirLoweringC, StdbitFirstLeadingOneComposesClzSelect) {
    auto L = lowerC(
...
TEST(MirLoweringC, StdbitBitCeilComposesClzShlAndMask) {
    auto L = lowerC(
...
TEST(MirLoweringC, StdbitBitFloorShiftAmountIsClampedNotBareMinusOne) {
    auto L = lowerC(
...
TEST(MirLoweringC, StdbitBitCeilShiftAmountIsClampedNotBareBitWidth) {
    auto L = lowerC(
        "unsigned f(unsigned int x){ return __builtin_stdc_bit_ceil_ui(x); }");
    ASSERT_TRUE(L.mir.ok) << (L.mirReporter.all().empty() ? "" : L.mirReporter.all()[0].actual);
    Mir const& m = L.mir.mir;
...
// Lower a program that `#include <setjmp.h>` through the FULL c pipeline with a
...
TEST(MirLoweringC, SetjmpCallCarriesReturnsTwiceFlagAndDecaysEnv) {
...
TEST(MirLoweringC, LongjmpNoreturnTerminatesBlockInUnreachable) {
    auto L = lowerSetjmpProgram(kSetjmpRoundTripSrc);
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.hir->ok);
    ASSERT_TRUE(L.mir.ok)
...
TEST(MirLoweringC, SameRepresentationAssignEmitsNoCastOrBitcast) {
...
TEST(MirLoweringC, VolatileLongDoubleArithmeticEmitsNoExtraCast) {
...
TEST(MirLoweringC, SameRepresentationIntegerReturnEmitsNoCast) {
    ...
    auto L = lowerC("long f(long *p) { *p = 42; return 0; }",
                          "x86_64", "sysv_amd64", DataModel::Llp64);
    ASSERT_FALSE(L.model.hasErrors());
    ASSERT_TRUE(L.hir->ok);
...
TEST(MirLoweringC, InlineFunctionTypedParamKeepsItsArgSlot) {
    auto L = lowerC(
...
TEST(MirLoweringC, InlineAsmOutputOnAParameterGivesThatParameterStorage) {
    auto L = lowerC(
...
TEST(MirLoweringC, InlineAsmMarksEveryOutputParameterAddressable) {
    auto L = lowerC(
