...
TEST(DiagnosticCode, DependsOnBandContinuationAfterTheSuppressSlot) {
    ...
    // to 8. `.harness-config/runner/actions/check-diagnostic-codes/check-diagnostic-codes.py` reads the ENUM itself and fails on
...
//
// The scan carries a FLOOR (same reasoning as the per-root floors of
// .harness-config/runner/actions/check-anchor-registry/check-anchor-registry.py):
// a scan that discovers nothing would otherwise pass while checking nothing.
