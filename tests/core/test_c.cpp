...
// End-to-end tests driving the shipped c.lang.json through the
...
TEST(CEndToEnd, TopLevelVarDeclWithIntInitializer) {
    auto h = tokenizeShipped("c", "int x = 5;");
...
TEST(CEndToEnd, FunctionWithIfReturnInsideBlock) {
    auto h = tokenizeShipped("c", "int main(void) { if (x) { return x; } }");
    ASSERT_NE(h.schema, nullptr);
...
TEST(CEndToEnd, OperatorTableMatchesCPrecedence) {
    auto h = tokenizeShipped("c", "");
    ASSERT_NE(h.schema, nullptr);
...
// migrating c's `expression` rule to use the `expr` shape and
// wiring the Pratt walker into the parser. The precedence-correct
// shape is now pinned end-to-end in
// `tests/analysis/syntactic/test_parser_c_pratt.cpp`, where it
...
// for the design call.
TEST(CEndToEnd, SwitchStmtParsesAllArmKinds) {
    auto h = tokenizeShipped("c",
