...
- **1,907 internal tests, 100% green on FOUR legs** — Windows x86_64, Linux x86_64 (WSL2), Linux arm64 (native), macOS arm64 — backed by a test corpus nearly as large as the engine itself (~143,000 lines). ⓘ The three non-Windows legs run **1,885** of those: the 22 repository-hygiene guards are root-host-only by construction, and `1885 = 1907 − 22` holding on all three is the cross-leg check that they tested the same tree.
