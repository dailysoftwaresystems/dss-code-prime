...
The result is a **hermetic, auditable toolchain**. DSS writes its own machine code and its own PE, ELF, and Mach-O executables directly — no `as`, no `ld`, no `lld`, no `llvm-mc`. Every byte between your source and the running binary is produced by **349,081 lines of code you can read** (254,889 of `.cpp` and 94,192 of `.hpp` under `src/`, ✔measured 2026-09-04), not by an opaque, multi-million-line dependency you can only trust.

## What works today

DSS Code Prime already compiles and runs **real, unmodified, production software**:

- **SQLite — compiled from the complete upstream source tree, and passing SQLite's own test suite.** Not just the amalgamation: **189 translation units** of unmodified upstream source go through a single `--project` manifest to build SQLite's own `testfixture`, which then runs **SQLite's own unit corpus**. At the `full` tier that is **~1.06 million assertions per run**: Linux x86_64 **7 failures / 1,061,830**, Linux arm64 **12 / 1,060,828**, Windows x86_64 **0 / 979,736**. Every residual failure is a known **non-DSS confound backed by a matched control** — a GCC-built reference `testfixture`, containing no DSS-compiled code, fails it identically on the same machine. **Nothing in the SQLite tree is patched and no test file is excluded to get there.** ⚠ **Read the numerator, not the denominator.** Those three totals were each taken against a *different* upstream revision of SQLite: the harness re-clones upstream on every run, so the corpus it executes is whatever SQLite shipped that day and the denominator moves with it — which is why the three differ. It is not a fixed yardstick and we do not offer it as one; the load-bearing figure is the **numerator**, the count of *DSS-attributable* failures, and that is **zero** in all three. A total quoted without the upstream commit it ran against is not comparable to any other; the most recent run we have pinned is **2026-09-14**, SQLite upstream `7936c5107f`, DSS `9cd8a227`–`f0a52d3a` (no `src/` change between them, so one compiler) — the `veryquick` tier driven from **all four hosts**, **0 poisoned on every one**: Windows `5 / 394,817` + `8 / 394,821` + `3 / 394,440`, WSL `5 / 394,817` + `7 / 394,821` + `3 / 392,936`, the arm64 VPS `3 / 394,795` + `3 / 394,799`, and macOS — the only host on earth that can EXECUTE the two Mach-O legs — `3 / 394,609` + `3 / 394,605`. **Every one of those numerators is a named non-DSS confound**, each earned per leg against a control rather than pattern-matched across legs. ⚠ The WSL `pe64` figure carries `[NOT FULL COVERAGE: 3 unit group(s) NOT REACHED]` and the report says so on its own line: three test FILES abort under `wine` for reasons measured to be upstream's — one of them a probe that calls `deleteWin32Symlink` outside its own `catch` — and an excused abort still means those tests did not run. Those denominators are a smaller corpus tier than the `full`-tier figures above, which is precisely the point. **The `sqlite3` CLI is built from full upstream source as well — 103 translation units, not the amalgamation** — on **all five OS × ISA targets** (Linux x86_64 + arm64, Windows x86_64, macOS arm64 + x86_64) with **zero special flags**: `sqlite3 --version` → 3.54.0 and a `CREATE` / `INSERT` / `SELECT` round-trip returns the correct result. ✔MEASURED 2026-09-14 across two hosts — the 14-assertion smoke gate is **14/14 on every one of the five**: Windows drives `elf64-x86_64`, `elf64-arm64` and `pe64-x86_64`, and a Mac drives `macho64-arm64` natively plus `macho64-x86_64` **under Rosetta** (`arch -x86_64`), which is what makes the Intel Mach-O leg observable at all rather than merely built. The single-file amalgamation is compiled too, as a separate and much faster probe — it is an *additional* check, never the thing standing in for the real build.
- **Every target builds on every host — the build is not a host question.** All five SQLite legs (`elf64-x86_64`, `elf64-arm64`, `pe64-x86_64`, `macho64-arm64`, `macho64-x86_64`) are built **unconditionally on whichever host is driving**: DSS selects its target from config, so a Windows box builds the Mach-O legs and a Linux box builds the PE leg. A build can still FAIL, but the outcome is always OBSERVED and reported by name — never inferred from the host's identity. ⚠ **RUNNING the artifact is the one real host question, and we do not blur the two.** A leg executes natively when the host matches, otherwise through a *declared* launcher (qemu cross-arch, Wine cross-OS), otherwise not at all — and "not at all" is a **named verdict**, never a silent skip. ⓘ The honest edge: there is **no off-Mac emulator for a Darwin arm64 executable**, so a real Mac is the only way to observe that leg run; and some declared launchers (the WSL cross-OS entry, Wine) are declarations of how a host *would* run it, labelled as such rather than as measurements.
...
- **2,054 internal tests, 100% green on FOUR legs** — Windows x86_64, Linux x86_64 (WSL2), Linux arm64 (native), macOS arm64 — backed by a test corpus **larger than the engine itself**: 428,765 lines under `tests/` and `integrated_tests/` against 349,081 under `src/` (✔measured 2026-09-04). ⓘ **Windows and WSL2 run all 2,054, the two REMOTE legs run 2,028**: the 26 repository-hygiene guards check the source tree, which is byte-identical everywhere, so a second physical host re-checking it learns nothing while paying the suite's most expensive entries. `2028 = 2054 − 26` holding on both remote legs is the cross-leg check that they tested the same tree. ⚠ The subtrahend is a **dated inventory, not a constant** — it moves whenever a guard is added (it was 18 at P36 and 24 at P58). Re-derive it from `ctest -N -L repo-guard`, never from this line.
...
| **Real-world corpus** | [`.harness-config/runner/actions/real-examples/`](.harness-config/runner/actions/real-examples/) — DssHarness actions, the registry of **known real-world repositories DSS compiles from unmodified upstream source and then runs their own test suites against**. Today: [`c/sqlite`](.harness-config/runner/actions/real-examples/c/sqlite) — full source (189 TUs), ~1.06 M unit assertions per `full`-tier run. Each entry is an action (`dssharness run sqlite`) shipping `build-and-test.sh` + `build-and-test.ps1`, which clone upstream, build, run the project's own suite and classify every failure against a reference build |
