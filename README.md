...
- **2,025 internal tests, 100% green on FOUR legs** — Windows x86_64, Linux x86_64 (WSL2), Linux arm64 (native), macOS arm64 — backed by a test corpus nearly as large as the engine itself (~143,000 lines). ⓘ **Windows and WSL2 run all 2,025, the two REMOTE legs run 2,001**: the 24 repository-hygiene guards check the source tree, which is byte-identical everywhere, so a second physical host re-checking it learns nothing while paying the suite's most expensive entries. `2001 = 2025 − 24` holding on both remote legs is the cross-leg check that they tested the same tree.
