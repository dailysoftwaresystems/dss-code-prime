...
| Predecessors  | ⏳ [`08.5-substrate-prep-plan`](./08.5-substrate-prep-plan%20-%20tbd.md) (core type lattice). 🟡 [`09-hir-plan`](./09-hir-plan%20-%20tbd.md) (`ExternFunction` / `ExternGlobal` nodes — HR1–HR7 ✅ 2026-05-26..27: the declarations landed at HR4 and the `FfiMetadata` / `HirFfiMap` side-table FFI ingestion populates is in place HR4–HR5, round-trip-serialized by HR7's `.dsshir` `@ffi(...)`; HR8–HR11 pending). |
