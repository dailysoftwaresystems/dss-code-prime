...
| ~~FF2~~ | C header mode + parser reuse | ✅ **CLOSED 2026-06-01** (post-fold heavy). `src/ffi/c_header_parser.{hpp,cpp}` exposes `readCHeader[FromText,Shipped](...) -> std::expected<vector<ImportSurface>, HeaderReadError>` — thin orchestration over the c-subset frontend (tokenize + parse + semantic + lower-to-HIR), walking the resulting HIR module's `moduleDecls` and emitting one `ImportSurface` per `ExternFunction` / `ExternGlobal`. Typedef is absorbed into the type system (no row); function body / non-extern global / `#include` (ImportGroup) / future HirKinds each fail loud with a distinct F_* code. 8 closed-enum `HeaderReadErrorKind` variants (FileOpenFailed / HeaderParseFailed / HeaderHasFunctionBody / HeaderHasNonExternDecl / EmptyImportLibrary / GrammarLoadFailed / HeaderHasUnsupportedTopLevel / InternalInvariant) map 1:1 to 8 new F_* codes at `0x5008-0x500E` + the reserved `F_HeaderHasExternInitializer` (`0x500F`, awaiting D-FF2-3 cross-tier fix). Underlying c-subset diagnostics (P_*/L_*/S_*/H_*) + ConfigDiagnostics (C_*) from a failed grammar load both drain to the caller's reporter alongside the FF2-layer F_* verdict — wrapper carries verdict, cause stays visible. Source span attribution on every FF2-layer rejection via `HirSourceMap::tryGet`. `slurpFile` checks `in.bad()` after open to catch mid-read I/O truncation (silent-failure C2 fold). `forwardConfigDiagnostics` hoisted to `grammar_schema.hpp` (silent-failure C1 fold). `findShippedFfiHeader` + `readCHeaderShipped` wrap the cwd-walk-up for `src/dss-config/ffi-headers/<library>/<name>.h` lookup (architect Item B fold). Shipped `libc/{stdio,stdlib}.h` cover puts/putchar/malloc/free/exit (the minimum surface the c-subset grammar accepts today — varargs/size_t await grammar extensions). 23 tests + 131/131 ctest. Anchored sub-rows: **D-FF2-1** cSignature reshape (drop or `optional<FnSigTypeId>`; trigger FF3 ABI catalog cycle), **D-FF2-2** `HeaderReadError` carries SourceLocation (trigger first field report needing wrapper-diag span), **D-FF2-3** `extern int x = 5;` initializer detection (trigger next cycle touching cst_to_hir.cpp::lowerExternDecl OR semantic_analyzer's externDecl handling — proper fix is cross-tier; F_HeaderHasExternInitializer code reserved), **D-FF1-NEST** split `src/ffi/binary_readers/` subdirectory (trigger second binary-reader TU lands — FF1-PE or FF1-MachO), **D-FF5-INGESTION-SOURCE** `IngestionSource` variant lives in `ingest.hpp` (trigger FF5 session), **D-FF6-HEADER-DIR-READER** `readCHeaderDirectory` for multi-file libraries (trigger first library needing > 3 header files). |
