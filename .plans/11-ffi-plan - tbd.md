...
| ~~FF2~~ | C header mode + parser reuse | ✅ **CLOSED 2026-06-01.** `src/ffi/c_header_parser.{hpp,cpp}` exposes `readCHeader[FromText,Shipped](...) -> expected<vector<ImportSurface>, HeaderReadError>`. Reuses the c-subset frontend; rejects function bodies / non-extern globals / `#include` / unknown HirKinds each with a distinct F_* code. 9 `HeaderReadErrorKind` variants ↔ 9 F_* codes (0x5008-0x500F) via closed-table dispatch. Shipped `src/dss-config/ffi-headers/libc/{stdio,stdlib}.h`. **D-FF2-2 ✅ CLOSED 2026-06-01** (post-fold #7, commit `6a6eec8`): `HeaderReadError` gains `HirSourceLoc at` field (no optional<> — `HirSourceLoc{}` is documented absent sentinel); F2 silent-failure fold threads first reported Error span into the wrap for lowering-failure path. **D-FF2-3 ✅ CLOSED 2026-06-01** (post-fold #7): new `H_ExternHasInitializer` (0xF00A) rejects `extern int x = 5;` / `= y;` / `= {0,1};` / `= {};` via shape-based varDeclTail walker; `arraySuffix` added to externDecl row. Detailed fold history in `memory/project_ff2_*` + `memory/project_tracks_a_b_post_fold_7_2026_06_01.md`. **Anchors**: D-FF2-4 unsuppressable-error class (trigger: codebase-wide audit of severity-gated codes); D-FF2-5 extern array-type plumbing audit (trigger: first downstream symbol-table site for array externs); D-FF2-6 HeaderReadError → std::variant (trigger: 9th kind with divergent payload); D-FF2-7 `H_ContradictoryDeclarationModifier` umbrella (trigger: 2nd contradictory-modifier kind); D-FF2-3-FN-INIT `extern fn = expr` reject (trigger: grammar permitting it); D-FF2-2-INVARIANT at-pin for InternalInvariant arms (trigger: fuzz harness); D-FF1-NEST split `src/ffi/binary_readers/` (trigger: 2nd binary-reader TU). |
...
| ~~FF5~~ | `ingest()` + `HirAttribute<FfiMetadata>` | ✅ **CLOSED 2026-06-01** (post-fold #5 parallel-tracks cycle). `src/ffi/ingest.{hpp,cpp}` exposes `ingest(sources, externs, target, format, ffiMap, reporter) -> HirIngestResult`. Composes FF1 binary readers + FF2 header parser + FF3 ABI catalog + FF4 mangling at one boundary: reads each `IngestionSource` (variant over BinaryLibrarySource / CHeaderSource / CHeaderDirSource — closes D-FF5-INGESTION-SOURCE), aggregates ImportSurface rows, FF4-unapplies binary-reader rows to recover canonical names, matches against caller-supplied `ExternDeclRef[]` (HirNodeId + canonicalName, decoupled from SemanticModel), FF4-applies to produce linker-visible mangledName, populates HirFfiMap. `readCHeaderDirectory` closes D-FF6-HEADER-DIR-READER (alphabetical *.h enumeration). FF3 cross-validation gates the (target, format) pair. **Hand-off half** (HIR → MIR via `collectExterns` pre-pass) landed earlier at LK6 cycle 2d. ELF-only at this cycle; FF1-PE/MachO triggers transparently extend FF5. **Out of scope**: end-to-end libc smoke test (FF6 — needs real binary on disk + runtime execution). 8 tests in `tests/ffi/test_ingest.cpp`. |
