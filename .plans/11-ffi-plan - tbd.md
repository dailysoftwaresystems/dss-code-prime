...
| FF1-PE | PE export-table reader                     | Parse `.edata` (export directory) and `.idata` (import-address-table) from `.dll` / `.lib` archives. Output uniform `ImportSurface` rows. Mirrors FF1-ELF's discipline (format-blind dispatch + F_* diagnostics + closed-enum SymbolKind/Visibility/Linkage). Trigger: first Windows c-subset corpus needing extern resolution against `msvcrt.dll` / `kernel32.dll`. **✅ `.edata` export reader SHIPPED v1 2026-06-01, HARDENED to production 2026-07-16 (c159, roadmap B1):** full Export Address Table walk (ordinal→EAT RVA→address), fn/data KIND classification by section executability (`SymbolKind::Function` for executable-section RVAs, `SymbolKind::Object` for data-section RVAs — closes `D-FF1-PE-OBJECT-EXPORTS`), and FORWARDER detection (EAT RVA in the export-directory span → new `SymbolKind::Forwarder` + `ImportSurface::forwardTarget` "DLL.Symbol"; kernel32 is forwarder-heavy). Fail-loud on truncation extended to the EAT/ordinal-table/forwarder-string reads. dumpbin-cross-checked on the REAL msvcrt.dll + kernel32.dll (see §5 acceptance). `.idata` IMPORT reader + linker extern-bind wiring stay OPEN (`D-FF1-PE-READER`). | First PE corpus that needs FFI-resolved externs. |
...
- [ ] ELF / PE / Mach-O / ar readers produce `ImportSurface` matching `nm` / `dumpbin` / `objdump` oracles for libc / libSystem / msvcrt across all 6 (OS × arch) targets. (Compound — box flips when all four readers land.)
    - [x] ✅ **ELF** done 2026-06-01 (`.dynsym`/`.dynstr`; STT_FUNC/OBJECT/TLS kind mapping).
    - [x] ✅ **PE** done 2026-07-16 (**c159**, roadmap B1): the `.edata` export reader HARDENED to production — full Export Address Table walk + fn/data kind classification (by section executability) + forwarder detection. **dumpbin-verified against the REAL msvcrt.dll** (1330 exports; puts/printf/fopen=Function; `_iob`/`_fmode`/`_environ`/`__argc`=Object — the exact criterion this line names) **AND kernel32.dll** (211 forwarders, count + targets exact vs `dumpbin /exports`, e.g. HeapAlloc→NTDLL.RtlAllocateHeap) **AND DSS's own c152-emitted `.dll`** (round-trip: functions→Function @ .text RVAs, data→Object @ .data RVA, dumpbin-exact). The `.idata` import half stays open (`D-FF1-PE-READER`).
    - [ ] ⏳ **Mach-O** — export reader is names-only (`D-FF1-MACHO-READER`, roadmap B2).
    - [ ] ⏳ **ar** (`.a`/`.lib` archive members) — not started (`D-FF1-AR-READER`, roadmap B3).
