...
| | |
|---|---|
| Status        | 🟡 **In flight.** PR0 ✅ shipped. PR1 ✅ shipped + review-fixed. PR2a ✅ shipped + review-fixed. PR2b ✅ shipped + review-fixed. PR3 ✅ shipped + review-fixed (`scopeRequire` object with `anyOf`/`forbid`/`topMustBe`/`outermost`; legacy `validScopes` loads as `anyOf` shorthand; new `C_ConflictingField`/`C_UnknownScopeName` diagnostic codes; `C_RedundantScopeRequire` warnings on `topMustBe + anyOf`, `forbid ∩ anyOf`, `forbid ∋ topMustBe/outermost`, `topMustBe == outermost`, empty arrays, oversize lists; `static_assert` pins on trivial-copyability of `ScopeMatch` + `LexemeMeaning`). 378 ctest cases across 22 suites, 100% pass. PR4 (Checkpoint) is the critical-path heavy next. |
