...
| | |
|---|---|
| Status        | 🟡 **In flight.** PR0 ✅ shipped. PR1 ✅ shipped + review-fixed. PR2a ✅ shipped + review-fixed. PR2b ✅ shipped (contextual keywords + reservedWordPolicy; TreeBuilder now walks SchemaCursor on open/close/pushToken; demotion via `P_ContextualKeywordResolution` info-level diagnostic). 343 ctest cases across 20 suites, 100% pass. PR3 (scopeRequire) is the next item; PR4 (Checkpoint) is the critical-path heavy. |
...
| PR2b | ✅ shipped | `LexemeMeaning.contextual` flag; `keywords[].contextual: true` per-entry; top-level `reservedWordPolicy: "strict" | "contextual"` (default Strict); `GrammarSchema::reservedWordPolicy()` accessor. `contextual` rejected on token entries. `TreeBuilder` now owns a `SchemaCursor` + parallel stack: `open()` enters the rule, RAII close leaves it, `pushToken` advances after demotion check. `pushToken` consults `expectedSet(cursor_)` before resolving a contextual keyword — demotes to Identifier when not expected and emits info-level `P_ContextualKeywordResolution`. Conservative fallback: invalid cursor → keyword stays. 14 new tests; `toy.lang.json` + `c-subset.lang.json` reload unchanged. |
