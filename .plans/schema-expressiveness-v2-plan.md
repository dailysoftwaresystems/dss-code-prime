...
| PR6  | ✅ shipped | _initial commit pending review_. New `string_style.{hpp,cpp}` ships `EscapeKind` enum (`None`/`Char`/`DoubledDelimiter`) and `StringStyle` POD with owned `endsAt`/`tagPattern` strings + bools (`endsAtLongestMatch`, `hasMatchedDelimiterTag`, `multiline`) + `escapeChar`. `LexemeMeaning::stringStyleIdx` is an `optional<uint32_t>` pool index into `GrammarSchemaData::stringStyles`; LexemeMeaning stays trivially copyable. `GrammarSchema::stringStyle(meaning)` returns the entry or nullptr; aborts on out-of-range index (strong-id-style contract). Loader validates the full surface: `escapeKind` required string; `escapeChar` required iff `Char`; `endsAt` required + non-empty; `endsAtLongestMatch`/`multiline` optional bools; `delimiterTag: "matched"` enables tag capture with default `tagPattern: "[A-Za-z0-9_]{0,16}"`; `tagPattern` regex compiled at load via `std::regex` (regex_error → `C_InvalidStringStyle`). Keywords reject `stringStyle` (`C_ConflictingField`) — word-shaped tokens can't open delimited strings. New code: `C_InvalidStringStyle`. 21 tests covering all four escape-kind/longest-match/delimiterTag variants, every error path, multi-style pool indexing, keyword rejection, backward-compat smoke. |
| PR7  | ✅ shipped | _initial commit pending review_. Authored `src/source-config/languages/tsql-subset.lang.json` exercising the full v2 feature surface: `reservedWordPolicy: "contextual"`, operator precedence table (infix + prefix `-`), `[bracket]` identifiers via stringStyle (doubled-delimiter), `'…'` and `N'…'` strings (doubled-delimiter), three lexer modes (`bracket-id`, `single-string`, `unicode-string`), `qualifiedName` shape for three-part naming, SELECT/INSERT/UPDATE/DELETE/CREATE statements. 19 tests cover every loader pin AND end-to-end TreeBuilder parse for every statement shape AND contextual-keyword demotion in name position. Empirical proof that v2's feature set is sufficient for a non-trivial real grammar. Explicitly out of scope (per plan §7): `GO`, `sys.*`, CLR types, stored-proc control flow, window clauses — residual gaps logged in `v2-gap-catalog.md`. |
