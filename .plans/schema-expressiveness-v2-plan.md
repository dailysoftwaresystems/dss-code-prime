...
| | |
|---|---|
| Status        | 🟡 **In flight.** PR0 ✅ shipped. PR1 ✅ shipped + review-fixed. PR2a ✅ shipped + review-fixed. PR2b ✅ shipped + review-fixed. PR3 ✅ shipped + review-fixed. PR4 ✅ shipped (`TreeBuilder::Checkpoint` RAII with rolls-back-by-default destructor; `DiagnosticReporter::Snapshot` + `truncateTo`; `Frame::children` refactored to shared `pendingChildren_` staging vector with per-frame `pendingStart` offset; loader plumbs `speculative: true` + `lookahead` on alt shapes; `P_MaxSpeculationDepth` / `P_UncommittedCheckpoint` / `P_BacktrackFailed` codes). 395 ctest cases across 23 suites, 100% pass. PR5 (lexer modes) next. |
...
| PR4  | ✅ shipped | _initial commit pending review_. `TreeBuilder::Checkpoint` (move-only RAII with rolls-back-by-default destructor); `checkpoint()` / `commit(Checkpoint&&)` / `rollback(Checkpoint&&)` snapshot/restore arena, child-index, pendingChildren staging, open-frame stack, scope stack, schema cursor + stack, cookie counter, closedCookies set, cursor-desync latch, AND the diagnostic reporter's `(all_, perCode_, recent_, hitCap_)`. `DiagnosticReporter::Snapshot` + `snapshotForRollback()` / `truncateTo()`. `Frame::children` refactored from per-frame `vector<NodeId>` to shared `pendingChildren_` staging vector with per-frame `pendingStart` offset; rollback truncates ONE vector. `BuilderConfig::maxSpeculationDepth` (default 64) gates the checkpoint stack — one-shot `P_MaxSpeculationDepth` on overflow with no-op guards thereafter. Loader plumbs `speculative: true` + `lookahead: N` (default 8) on alt shapes; cursor walker doesn't consume them yet — exposed via `GrammarSchema::isSpeculativeAlt(cursor)` / `lookahead(cursor)` for the future parser. 17 new tests covering rollback round-trip per axis, commit-keeps-diagnostics, hitCap latch clearing, nested commit-then-rollback / rollback-then-commit, watchdog one-shot, uncommitted-checkpoint warning, loader plumbing, 1000-token perf smoke. |
