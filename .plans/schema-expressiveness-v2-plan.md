...
| | |
|---|---|
| Status        | 🟡 **In flight.** PR0 ✅ shipped. PR1 ✅ shipped + review-fixed. PR2a ✅ shipped + review-fixed. PR2b ✅ shipped + review-fixed. PR3 ✅ shipped + review-fixed. PR4 ✅ shipped + review-fixed (`TreeBuilder::Checkpoint` RAII; full state snapshot incl. reporter `recent_` deque + `maxSpeculationDepthReached_`; `Frame::children` → shared `pendingChildren_`; `checkpointStack_` is `vector<pair<id, snap>>` with linear-search-by-id (robust against commit-then-rollback interleaving); loader emits diagnostics for malformed `speculative`/`lookahead`; speculative alts exempt from ambiguity-detect; `forceReport` bypass for forgotten-commit warnings; new `C_RedundantField` code; static_assert on Checkpoint move-semantics). 411 ctest cases across 23 suites, 100% pass. PR5 (lexer modes) next. |
