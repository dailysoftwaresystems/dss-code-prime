...
| | |
|---|---|
| Status        | 🟡 **In flight.** PR0 ✅ shipped. PR1 ✅ shipped + review-fixed. PR2a ✅ shipped + review-fixed. PR2b ✅ shipped + review-fixed (contextual keywords + reservedWordPolicy; TreeBuilder now walks SchemaCursor on open/close/pushToken with AltChoice routing in `open()`; demotion via `P_ContextualKeywordResolution` info-level diagnostic; new `P_SchemaCursorDesync` one-shot diagnostic surfaces cursor off-track events; loader rejects `kind:"Identifier"+contextual:true`). 349 ctest cases across 21 suites, 100% pass. PR3 (scopeRequire) is the next item; PR4 (Checkpoint) is the critical-path heavy. |
