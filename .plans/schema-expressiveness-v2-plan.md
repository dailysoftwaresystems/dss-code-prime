...
| | |
|---|---|
| Status        | 🟡 **In flight.** PR0 ✅ shipped. PR1 ✅ shipped + review-fixed. PR2a ✅ shipped + review-fixed. PR2b ✅ shipped + review-fixed. PR3 ✅ shipped (`scopeRequire` object with `anyOf`/`forbid`/`topMustBe`/`outermost`; legacy `validScopes` loads as `anyOf` shorthand; `C_RedundantScopeRequire` warning on `topMustBe + anyOf`). 364 ctest cases across 22 suites, 100% pass. PR4 (Checkpoint) is the critical-path heavy next. |
...
| PR3  | ✅ shipped | _initial commit pending review_. `ScopeMatch { anyOf, forbid, topMustBe, outermost }` on every `LexemeMeaning`. Loader accepts `scopeRequire` object on per-token meanings; legacy flat `validScopes: [...]` loads as `scopeRequire.anyOf` (backward compat). Loader rejects co-existence of `validScopes` and `scopeRequire` (`C_MissingField`); warns on redundant `topMustBe + anyOf` (`C_RedundantScopeRequire`). Builder checks `forbid → topMustBe → outermost → anyOf` in `pushToken`'s candidate filter; first failure rejects the meaning. 15 tests; `toy.lang.json` + `c-subset.lang.json` reload unchanged. |
