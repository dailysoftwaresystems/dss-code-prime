...
| | |
|---|---|
| Status        | 🟡 **In flight.** PR0 ✅ shipped. PR1 ✅ shipped + review-fixed. PR2a ✅ shipped + review-fixed. PR2b ✅ shipped + review-fixed. PR3 ✅ shipped + review-fixed. PR4 ✅ shipped + review-fixed. PR5 ✅ shipped (`lexerModes` field; `LexerModeStack` runtime with push/pop/replace/snapshot/restore; `modeOp`/`modeArg` on `LexemeMeaning`; `LexerModeId` strong id; `C_UnknownLexerMode` code; synthesized "main" mode for v1 configs). 435 ctest cases across 24 suites, 100% pass. PR6 (stringStyle) next. |
...
| PR5  | ✅ shipped | _initial commit pending review_. New `lexer_mode.{hpp,cpp}` ships `ModeOp` enum (`None`/`PushMode`/`PopMode`/`ReplaceMode`), `LexerMode` POD (name/id/defaultToken), and `LexerModeStack` runtime with `push`/`pop`/`replaceTop`/`apply` + opaque `Snapshot` for future Checkpoint integration. `LexerModeId` strong id (zero is sentinel; ids dense 1..N). `LexemeMeaning` gains `modeOp` + `modeArg` (resolved to `LexerModeId` at load time). Loader registers `lexerModes` in Pass 1 (names → ids) so token meanings can resolve forward-referenced `modeArg`s; Pass 2 parses each mode's `defaultToken` and populates `lexerModeTokens` with main-mode-inherits-top-level. Auto-synthesizes "main" mode for v1 configs so consumers query `lexerModes()` uniformly. New `C_UnknownLexerMode` code. `modeArg` on `popMode` → `C_RedundantField` warning. `modeArg` without `modeOp` → `C_ConflictingField` error. Per-mode inline `tokens: { ... }` parsing deferred to a follow-up (most real configs put modeOp on top-level tokens). 24 tests covering stack stub-driver (push/pop/replace/snapshot/restore/nested), loader registration, modeOp parsing, all four error paths, cyclic mode references load cleanly, shipped configs still load. |
