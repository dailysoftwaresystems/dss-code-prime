...
| | |
|---|---|
| Status        | 🟡 **In flight.** PR0 ✅ shipped (c-subset config, loader version-window bump, gap catalog, 6 new tests). PR1 ✅ shipped + review-fixed (OperatorTable with key-packing guard, loader `operators` section with duplicate-detect, `expr` shape kind with allowlist, shape-kind mutual exclusion, c-subset operator table). 302 ctest cases across 18 suites, 100% pass. PR2a (schema cursor) is the next critical-path item. |
