...
> Follow-up to [`01-tree-node-model-plan - ok.md`](./01-tree-node-model-plan - ok.md) (v1 sub-plan).
>
...
| | |
|---|---|
| Status        | ✅ **Done.** PR0 ✅ shipped. PR1 ✅ shipped + review-fixed. PR2a ✅ shipped + review-fixed. PR2b ✅ shipped + review-fixed. PR3 ✅ shipped + review-fixed. PR4 ✅ shipped + review-fixed. PR5 ✅ shipped + 2 review rounds (`lexerModes` + `LexerModeStack`; fatal-on-empty discipline; Snapshot owner stamp uses per-instance monotonic id (defeats address-recycling false-pass); `lexerModes()` hides sentinel; keywords reject modeOp; near-miss case-folded duplicate-mode warning; inline-tokens deferral warning; orphan `defaultToken`-only mode warning; `lookupLexemeInMode` aborts on invalid id with id-value in message; `LexerMode::make` factory used in both passes). PR6 ✅ shipped + review-fixed (`stringStyle` descriptor; `LexemeMeaning::stringStyleId` is a proper `StringStyleId` strong id; new `SchemaId` strong id stamps every LexemeMeaning so cross-schema misuse aborts loudly; `escapeChar` with non-Char kind + `tagPattern` without delimiterTag both fail-fast; longest-match + 1-char endsAt warns; keywords reject `stringStyle`; `hasMatchedDelimiterTag` dropped — `tagPattern.empty()` is the dynamic-tag signal; new `C_InvalidStringStyle` code). PR7 ✅ shipped + review-fixed (tsql-subset.lang.json stress test). PR8 ✅ shipped (cross-plan close-out: parent + v1 sub-plan flipped to ✅ done; v2 additions documented across `docs/language-config-spec.md` §9 and `docs/tree-model.md` §6; v2-gap-catalog finalized). **509 ctest cases across 26 suites, 100% pass.** v2 is complete; the path opens to the languages-onboarding plan. |
| Trigger       | ✅ v1 complete; PR0 and PR1 shipped. |
| Predecessors  | ✅ v1 T1–T12 (`01-tree-node-model-plan - ok.md`). ⚠️ "minimal end-to-end pipeline (tokenize → parse → IR → emit)" — **NOT done**. T10 ships an E2E test that drives `TreeBuilder` via hand-constructed token streams; no real lexer/parser/IR exists yet. PR0/PR1 inherit this constraint (see §0.2). |
...
|---|---|---|
| PR0 | ✅ shipped | `4aef654` (initial) + `3aca464` (review followup). c-subset.lang.json, loader [1..2], v2-gap-catalog - tbd.md, 6 tests. |
...
| PR2a | ✅ shipped | `b571cf3` (initial) + `1d400a9` (review followup). New `compiled_shape.hpp` (Position class with factories, CompiledRule). `SlotKind` lives in `dss::`. Loader computes per-rule FIRST/NULLABLE via fixed-point with iteration cap, builds per-rule flat Position table (tie-the-knot for repeats), runs ambiguity detection (`C_AmbiguousAlternatives` on overlapping alt FIRSTs), then computes nullableTail. SchemaCursor is `(RuleId, posId)` at 8 bytes, trivially-copyable. Full cursor API + drop of `expectedAt`. Loader hardening: empty array bodies + zero-shape-kind objects load-error; iteration caps on fixed points. c-subset's `topLevel` refactored to disambiguate. 27 cursor tests + ambiguity + multi-level descent + span-stability + ordering pins. |
| PR2b | ✅ shipped + review-fixed | `03c6f22` (initial) + `9765791` (review followup). `LexemeMeaning.contextual` flag; `keywords[].contextual: true` per-entry; top-level `reservedWordPolicy: "strict" | "contextual"` (default Strict); `GrammarSchema::reservedWordPolicy()` accessor. `contextual` rejected on token entries AND on built-in `Identifier` kind. `TreeBuilder` owns a `SchemaCursor` + parallel stack: `open()` enters the rule (routing through any `AltChoice` to a `RuleLeaf(rule)` so `leaveRule` stays valid), RAII close leaves it, `pushToken` advances after demotion check (and skips advance for `EmptySpace` tokens). `pushToken` consults `expectedSet(cursor_)` before resolving a contextual keyword — demotes to Identifier when not expected and emits info-level `P_ContextualKeywordResolution`. Conservative fallback: invalid cursor → keyword stays. New `P_SchemaCursorDesync` info diagnostic fires one-shot on the first valid→invalid cursor transition. New `GrammarSchema::routeToRuleLeaf(parentCur, rule)`. Defensive `*Fatal` in `pushToken` for contextual→Identifier identity. 19 contextual-keyword tests; `toy.lang.json` + `c-subset.lang.json` reload unchanged. |
| PR3  | ✅ shipped + review-fixed | `901ad62` (initial) + `55db3df` (review followup). `ScopeMatch { anyOf, forbid, topMustBe, outermost }` on every `LexemeMeaning`. Loader accepts `scopeRequire` object on per-token meanings; legacy flat `validScopes: [...]` loads as `scopeRequire.anyOf` (backward compat). New diagnostic codes: `C_ConflictingField` (dual-field conflict, wrong-type field), `C_UnknownScopeName` (replaces overloaded `C_UnclosableScope` for unknown scope names in config). `C_RedundantScopeRequire` warnings expanded: `topMustBe + anyOf`, `forbid ∩ anyOf`, `forbid` containing `topMustBe`/`outermost`, `topMustBe == outermost`, explicit empty arrays, oversize lists (>32). Builder checks `forbid → topMustBe → outermost → anyOf` in `pushToken`'s candidate filter. `static_assert` pins trivial-copyability of `ScopeMatch` + `LexemeMeaning` (hot-path copies). 29 tests; `toy.lang.json` + `c-subset.lang.json` reload with empty-scopeRequire pinning. |
| PR4  | ✅ shipped + review-fixed | `9b1ca2c` (initial) + `40d8576` (review followup). `TreeBuilder::Checkpoint` (move-only RAII with rolls-back-by-default destructor); `checkpoint()` / `commit(Checkpoint&&)` / `rollback(Checkpoint&&)` snapshot/restore arena, child-index, pendingChildren, open-frame stack, scope stack, schema cursor + stack, cookie counter, closedCookies set, cursor-desync latch, watchdog latch, AND the diagnostic reporter's `(all_, perCode_, recent_, hitCap_)` — `recent_` is captured by full-deque copy (size-only would lose pop_front evictions). `DiagnosticReporter::Snapshot` is friend-encapsulated (private fields). `checkpointStack_` is `vector<pair<id, snap>>`; lookup is linear-search-by-id, robust against commit-inner-then-rollback-outer interleaving that breaks any index-arithmetic scheme. `Frame::children` refactored from per-frame `vector<NodeId>` to shared `pendingChildren_` staging vector with per-frame `pendingStart` offset; rollback truncates ONE vector. `BuilderConfig::maxSpeculationDepth` (default 64) gates the stack — `P_MaxSpeculationDepth` one-shot (snapshotted and restored on rollback) with no-op guards thereafter; `kNoOpCheckpointId` named constant for the sentinel; dtor short-circuits on it. Loader emits `C_ConflictingField` for malformed `speculative`/`lookahead` and `C_RedundantField` for `lookahead` without `speculative: true`; speculative alts are exempt from ambiguity-detect (overlapping FIRST sets are the whole point). `commitToId_`/`rollbackToId_` emit `P_BuilderInvariant` on stale ids or outer-commit-while-inner-pending. `forceReport_` bypasses the reporter cap for the `P_UncommittedCheckpoint` dtor warning so the forgotten-commit signal isn't silenced by a capped pre-checkpoint reporter. 33 tests covering rollback round-trip per axis (incl. perCode/dedup), nested interleavings, the C4 inner-commit-outer-rollback bug, Checkpoint move semantics, watchdog latch reset, loader diagnostics, speculative ambiguity exemption, 1000-token perf smoke. |
| PR5  | ✅ shipped + review-fixed | `10641af` (initial) + `0507a7c` + `d53d627` (two review rounds). New `lexer_mode.{hpp,cpp}` ships `ModeOp` enum, `LexerMode` POD with factory, and `LexerModeStack` runtime — `pop`/`replaceTop`/`top` are fatal on empty (matches strong-id discipline); lenient `tryPop()`/`topOrInvalid()` for callers that need it; `clear()` for tokenizer hard-reset. `Snapshot` carries owner-stack identity stamp; cross-stack `restore` aborts. `LexerModeId` strong id (zero = sentinel; dense 1..N). `LexemeMeaning` gains `modeOp` + `modeArg`. Loader: Pass 1 registers modes with case-folded near-miss detection (warning on `"String-body"` vs `"string-body"`); Pass 2 populates per-mode tables. `lexerModes()` returns `subspan(1)` to hide the internal sentinel. `lookupLexemeInMode` aborts on invalid id (matches `lexerMode(id)`'s contract). Loader rejects `modeOp`/`modeArg` on keywords (`C_ConflictingField` — soft-keyword demotion ≠ mode switching). Inline `tokens: {...}` parsing deferred; loader emits `C_RedundantField` warning so authors aren't silently surprised. Diagnostics: `C_UnknownLexerMode` (unresolved modeArg); `C_ConflictingField` (wrong type / unknown modeOp / missing modeArg on push-replace / keyword carrying modeOp / case-folded duplicate / non-string `tokens` value); `C_RedundantField` (modeArg-with-popMode / inline-tokens deferral / lookahead-without-speculative); `C_MissingField` (malformed defaultToken). 41 tests covering stack stub-driver (incl. death tests for fatal-on-empty + cross-stack restore), `clear()`/`tryPop()`/`topOrInvalid()`, snapshot of empty stack, loader registration, all four modeOps incl. replaceMode, malformed defaultToken/tokens, inline-tokens deferral warning, non-main tokens:"default" inheritance, sentinel hiding, keyword rejection, case-fold duplicate warning, lookupLexemeInMode death on invalid id. |
| PR6  | ✅ shipped + review-fixed | `283fbb2` (initial) + `3285eed` (review followup). New `string_style.{hpp,cpp}` ships `EscapeKind` enum (`None`/`Char`/`DoubledDelimiter`) and `StringStyle` POD with owned `endsAt`/`tagPattern` strings + bools (`endsAtLongestMatch`, `multiline`) + `escapeChar`. `LexemeMeaning::stringStyleId` is a `DSS_STRONG_ID(StringStyleId)` pool index into `GrammarSchemaData::stringStyles`. New `SchemaId` strong id stamped onto every LexemeMeaning so cross-schema lookups abort loudly. `GrammarSchema::stringStyle(meaning)` returns the entry or nullptr; aborts on cross-schema misuse or out-of-range id. Loader fail-fasts on `escapeChar` with non-Char kind, `tagPattern` without `delimiterTag: "matched"`, missing/empty `endsAt`, multi-byte `escapeChar`, unknown `escapeKind`, non-`"matched"` `delimiterTag`. `delimiterTag: "matched"` enables dynamic-tag capture with default `tagPattern: "[A-Za-z0-9_]{0,16}"`; non-empty `tagPattern` IS the dynamic-tag signal (no separate bool). Regex compile catches `std::regex_error` then `std::exception` (covers `bad_alloc`). Longest-match + 1-char endsAt warns. Keywords reject `stringStyle` (`C_ConflictingField`). New code: `C_InvalidStringStyle`. 28 tests covering every escape-kind/longest-match/delimiterTag variant + every error path + pool stress (50 styles) + death tests for OOB and cross-schema. |
| PR7  | ✅ shipped + review-fixed | `f2ba763` (initial) + `0c06d05` (review followup). Authored `src/dss-config/sources/tsql-subset.lang.json` exercising the full v2 feature surface: `reservedWordPolicy: "contextual"`, operator precedence table (infix + prefix `-`), `[bracket]` identifiers via stringStyle (doubled-delimiter), `'…'` and `N'…'` strings (doubled-delimiter), three lexer modes (`bracket-id`, `single-string`, `unicode-string`), `qualifiedName` shape for three-part naming, SELECT/INSERT/UPDATE/DELETE/CREATE statements. 23 tests cover every loader pin AND end-to-end TreeBuilder parse with full `prettyPrint` tree-shape goldens AND contextual demotion verified at the leaf-kind level AND multi-statement parsing AND operator-precedence data consumption AND NOT NULL clause. Empirical proof v2's feature set is sufficient for a non-trivial real grammar. Explicitly out of scope (per plan §7): `GO`, `sys.*`, CLR types, stored-proc control flow, window clauses — residual gaps logged in `v2-gap-catalog - tbd.md` §6. |
| PR8  | ✅ shipped | `8f0125d`. Parent `00-compiler-implementation-plan - tbd.md` v2 status flipped from 🟡 in flight to ✅ done; `01-tree-node-model-plan - ok.md` test count updated to 509/26. This file's §0 status flipped to ✅ done; final landing entries recorded. `v2-gap-catalog - tbd.md` §6 marked PR0–PR8 all shipped. `docs/language-config-spec.md` §9 "v2 additions" verified complete (operators, expr shape, reservedWordPolicy + contextual keywords, scopeRequire, speculative alt, lexerModes + modeOp, stringStyle); `docs/tree-model.md` §6 diagnostic table covers all v2 parse-time `P_*` codes (the v2 `C_*` load-time codes are catalogued in `docs/language-config-spec.md` §9.9). No code changes. |
...
### PR2a deviations from this plan

- **Caller-managed descent stack, per-rule cursor** instead of an inline-stack auto-descending cursor. The cursor is `(RuleId, posId)` and stays trivially-copyable at 16 bytes; nesting is unlimited because the caller owns the stack. Rationale: zero-limit, more composable (auto-descent can be implemented on top; the reverse can't), and matches how real grammar walkers work (parser owns the descent stack).
- **`advance` is per-rule.** It only handles `TokenLeaf` and `AltChoice` slots. For `RuleLeaf` slots the caller explicitly calls `enterRule(ruleId)` (descend) and later `leaveRule(parentCur)` (skip past the slot once the child is done OR if the rule is nullable and the caller wants to skip). Rationale: more primitive operation; auto-descent would conflate observation points the future parser needs for error recovery and speculative checkpointing.
- **`expectedAt` dropped entirely.** The plan §5.2 only required `expectedSet` (token-id form); the legacy string-form `expectedAt` was deleted in this PR rather than maintained in parallel. Diagnostic rendering already populates `ParseDiagnostic::expected` directly from `schemaTokens().name(id)` so no production caller broke.
- **`expr` shape kind is treated as its `atom` reference at the cursor layer.** PR2a's cursor walks `expr` as if it were a plain rule reference to `atom`. Operator climbing is the parser's job (will consult `OperatorTable` at parse time), not the cursor's.
...
### PR5 deviations from this plan

- **Per-mode inline `tokens: { ... }` parsing deferred to a follow-up.** Plan §5.5's `string-body` example declares per-mode tokens inline; PR5 ships the registration and `defaultToken` parsing but only `tokens: "default"` (inherit-from-top-level) is wired. Inline objects emit a `C_RedundantField` warning at load time so authors aren't silently surprised by empty mode tables. Follow-up will extract the top-level tokens-parsing block into a reusable function and call it for per-mode inline tokens too. Trigger: when a real v2-target config needs `string-body`-style mode-local lexeme entries (likely PR7's tsql-subset stress or the C# onboarding).
- **`LexerModeStack::pop`/`replaceTop`/`top` are fatal on empty** (`*Fatal` pattern, not silent no-ops) to match the project's strong-id "invalid state is a bug" discipline. Lenient callers opt in via `tryPop()` / `topOrInvalid()`. Without this, the tokenizer phase would inherit a class of silent-failure bugs that the rest of the codebase already rejects.
- **`Snapshot` carries an owner-stack identity stamp.** Restoring a snapshot into a different `LexerModeStack` aborts. Speculative `Checkpoint` integration (parent plan #5) will multiply the chance of mixups; the stamp catches them at the wrong-call site rather than weeks later.
- **Mode names are case-sensitive but the loader warns on case-folded near-misses.** A config that declares both `"String-body"` and `"string-body"` gets `C_ConflictingField` (Warning) for the second; references resolve only to the exact-case spelling. The folding is observation-only — the loader doesn't normalize.
- **Keywords reject `modeOp`/`modeArg`** with `C_ConflictingField`. Soft-keyword demotion (contextual resolution) and lexer-mode switching are distinct mechanisms; mixing them on one entry conflates resolution responsibilities. Mode operations belong on `tokens` entries.

### PR2b deviations from this plan

- **`P_SchemaCursorDesync` info-level diagnostic added during review followup.** One-shot per build, fires on the first valid → invalid cursor transition in `pushToken::advance` or `closeFrame_::leaveRule`. Not in the plan's §5 design — surfaces what the conservative "invalid cursor → keep keyword" fallback was previously hiding. Severity is `Info`; the parse is still correct, just contextual resolution stays strict.
- **`SchemaCursor` now routes through `AltChoice` on `open()`.** New `GrammarSchema::routeToRuleLeaf(parentCur, rule)` walks an AltChoice tree looking for a `RuleLeaf(rule)` branch; `TreeBuilder::open` saves that RuleLeaf as the parent cursor so `leaveRule` on close stays valid even when the parent's slot is a `repeat`/`optional`/`alt` AltChoice. Without this, every nested rule under a `repeat` would trip cursor desync — c-subset's `root: repeat(topLevel)` is the motivating case.
- **`EmptySpace` tokens do not advance the schema cursor.** `pushToken` skips `schema_->advance` when the resolved meaning carries `NodeFlags::EmptySpace` (whitespace is off-grammar). Without the skip, every whitespace token between schema-tracked tokens would invalidate the cursor.
- **Loader rejects `keywords[]` entries with `kind: "Identifier"` AND `contextual: true`.** The would-be demotion is a no-op (`Identifier → Identifier`); the loader emits `C_MissingField` rather than silently treat the keyword as plain Identifier. `TreeBuilder::pushToken` also fails fatally if a contextual meaning resolves to Identifier at runtime — belt-and-suspenders for callers fabricating `LexemeMeaning` directly.

### PR1 contracts established for downstream PRs

These were not in the original plan §5.1 but landed during PR1 review followup. Future PRs should treat them as load-bearing.

- **`OperatorTable` 30-bit `SchemaTokenId` ceiling**, exposed publicly as `OperatorTable::kMaxSchemaTokenIdValue`. The packed `(id, arity)` key uses 30 bits for the id and 2 bits for the arity. Any code path that grows `SchemaTokenId` beyond 2^30 must coordinate with this table — `key()` aborts via `detail::opTableFatal` rather than silently aliasing. `OperatorArity` is constrained to ≤ 4 variants by a `static_assert`; growing it requires re-engineering the key shape, not just adding an enumerator.
- **Shape-kind mutual exclusion is enforced**, not advisory. A shape body declaring two of `{sequence, alt, optional, repeat, expr}` emits `C_UnknownShape` listing the conflicting kinds. PR4's speculative `alt` and PR5's interpolation shapes must compose with — not override — this exclusion; if a new shape kind needs to coexist with `sequence`, it has to be additive metadata at the rule level, not a sibling shape-kind key.
- **`expr` body allowlist**: only `atom` and `minPrecedence` are recognized; any other key emits `C_UnknownShape` with the offending key in the message. When the Pratt parser lands in the parser phase and grows `expr` with new fields (e.g., `excludeOperators` per the plan's deferred-to-v3 list), the allowlist must be extended in the same change.
- **`operators.groups[]` duplicate `(SchemaTokenId, OperatorArity)` entries are a load error.** Loader emits `C_InvalidPrecedenceTable` ("declared twice") before reaching the table's silent last-write-wins `insert_or_assign`. Same lexeme with different arities (e.g. `-` infix and `-` prefix) is fine — only the full `(id, arity)` key matters.
- **`operatorAssocName` / `operatorArityName`** are `constexpr` and use `std::unreachable()` after their exhaustive switch — non-exhaustive cases are compile errors, not silent `"Unknown"` returns. PR2b / PR3 diagnostic-rendering code can use them freely without a fallback path.
...
| Shipped grammar | `src/dss-config/sources/toy.lang.json` | **Must continue to load unchanged through every v2 PR** — smoke test pin. |
...
1. **Draft `src/dss-config/sources/c-subset.lang.json`** against the v1 schema as-is. Target the scope listed in §4 (typedef, operator precedence, simple string literals, block structure, function decls, if/while/return).
...
4. **Author `.plans/v2-gap-catalog - tbd.md`** following the §4.1 row format. Each gap row points to a §5 subsection.
...
7. **Plan-to-code ratio.** v1 already ships a heavy plan-to-code ratio. v2 will deepen this — `.plans/v2-gap-catalog - tbd.md` is new, plus per-PR design-diff appendices. **Recommendation:** the gap catalog stays terse (table only, no prose); design-diff appendices stay under 10 lines per row. Save the prose for the v2.5/v3 plans if they materialize.
...
## 3. Relationship to v1 (`01-tree-node-model-plan - ok.md`)
...
| `src/dss-config/sources/toy.lang.json` | Unchanged. Re-loaded as a smoke test in every v2 PR. | — |
...
| `src/dss-config/sources/c-subset.lang.json` | PR0 deliverable. | PR0 |
| `src/dss-config/sources/tsql-subset.lang.json` | PR7 deliverable. | PR7 |
| `.plans/v2-gap-catalog - tbd.md`               | Empirically-discovered gaps from PR0, updated through PR7. Format defined in §4.1. | PR0 |
...
Each gap becomes a row in `.plans/v2-gap-catalog - tbd.md`:
...
**Gaps from PR0 c-subset authoring (`v2-gap-catalog - tbd.md` §1, rows 1–18):**
...
`Checkpoint` should be cheap. Acceptance includes a benchmark target: **parsing a 1000-token source with one outer speculative alt completes in under 50 ms on the reference machine (GCC 13.2, Release build).** Each checkpoint allocates `O(open_frames + cursor_depth + closed_cookies + recent_window)` per snapshot — the open-frames vector, the cursor stack, the closed-cookies set, and the diagnostic reporter's recent-deque are all copied. Typical workloads keep all four small (≤ tens of entries); `maxSpeculationDepth` (default 64) bounds the multiplier. Arena, child-index, pending-children, and scope-stack rollbacks are integer truncations of the underlying vectors.
...
|---|---|---|---|---|---|---|
| **PR0** | ✅ shipped | `v2-empirical-c-subset` | Author `c-subset.lang.json` against v1 schema; build gap catalog | **5–8** | `src/dss-config/sources/c-subset.lang.json` + `.plans/v2-gap-catalog - tbd.md` | Config loads cleanly under the [1..2] window; gap catalog enumerates 18 gaps with branch-(a)/(b)/(c) tagging; `toy.lang.json` still loads; 6 new tests (3 grammar_schema + 3 c_subset) including a flip-target pin for PR1's precedence work. |
| **PR1** | ✅ shipped (+ review followup `048953f`) | `v2-precedence` | Operator precedence + associativity + arity | **3–5** | `grammar_schema.{hpp,cpp}`, `grammar_schema_json.cpp`, `parse_diagnostic.{hpp,cpp}` (new code `C_InvalidPrecedenceTable`), new `operator_table.hpp` (header-only with `kMaxSchemaTokenIdValue` ceiling + `opTableFatal`), `tests/core/test_operator_table.cpp` | `schema->operatorTable().lookup(plus, Infix)` returns precedence 50 / assoc Left; `lookup(minus, Prefix)` returns precedence 90 (distinct from infix 65); `lookup(identifier, Infix)` is empty; c-subset.lang.json bumped to v2 with a full C precedence table; multi-meaning lexemes require explicit group `kind`; `expr` shape kind recognized by loader (atom rule reference validated). Review followup added: key-packing fatal guard at 2^30, duplicate `(id, arity)` rejected, `expr` body keys allowlisted, shape-kind mutual exclusion enforced, direct `OperatorTable` API tests, `static_assert` block, `constexpr` name fns. 30 tests in `test_operator_table.cpp` (incl. 1 death) + 1 in `test_c_subset.cpp`. Pratt parse-tree assertions deferred until parser exists (the `ExpressionWithMixedOpsIsLeftFolded` flip-target test stays pinned). |
| **PR2a** | ✅ shipped | `v2-schema-walker` | Real `SchemaCursor::advance()` / `enterRule()` / `leaveRule()` / `expectedSet()` | **4–6** | `grammar_schema.{hpp,cpp}`, `grammar_schema_json.cpp`, `schema_cursor.hpp`, new `compiled_shape.hpp`, `tests/core/test_schema_cursor.cpp`, +1 conversion in `test_grammar_schema.cpp` | Compiled position tables built at load time. Per-rule FIRST/NULLABLE via fixed-point, per-position `nullableTail` via fixed-point. `advance` walks `TokenLeaf` and `AltChoice` slots (routes by `expectedSet`); `enterRule` / `leaveRule` form the descent/return pair. Cycles via repeat terminate (FIRST sets grow monotonically). `expectedAt` dropped — `ParseDiagnostic::expected` keeps working because the builder populates it directly from `schemaTokens().name(id)`. Closes v1 §0 deviation #7. |
| **PR2b** | ✅ shipped | `v2-contextual-keywords` | Soft keywords + reserved-word policy on top of the walker | **2–3** | `grammar_schema.{hpp,cpp}`, `grammar_schema_json.cpp`, `tree_builder.{hpp,cpp}`, `parse_diagnostic.{hpp,cpp}` (`P_ContextualKeywordResolution`), `tests/core/test_contextual_keywords.cpp` | `ReservedWordPolicy { Strict, Contextual }` enum + accessor. `LexemeMeaning.contextual` flag (loader-only writer; tokens reject the flag). Strict policy = unchanged v1 behaviour. Contextual policy implicitly marks every keyword soft. TreeBuilder owns a `SchemaCursor` + parallel stack walking on open/close/pushToken; contextual demotion in pushToken consults `expectedSet(cursor)` and falls back to keep-keyword on invalid cursor. 14 new tests including the canonical `int await = 0;` demotion and SQL-style `select <Identifier>` under contextual policy. |
