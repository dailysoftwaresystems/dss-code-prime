...
| | |
|---|---|
| Status        | ⏳ **PLANNED 2026-06-10 (rev. after a strict 3-reviewer plan-lock — agnosticism, slicing, accuracy).** Stepper row **V2-4.X**, between V2-4 (program-api CLI ✅) and V2-5 (RISC-V). NOT started. |
...
| Phase | What | Closes |
|---|---|---|
| **FC1** ⏳ | `%` modulo (the binary `%` token is literally absent today — only `%=`) + the **SIGNED** `idiv`+`cqo` div/mod codegen + the role-tagged output-index contract. (NB: the SIGNED-vs-UNSIGNED split + the unsigned-high-bit pin move to FC3, which is where `unsigned` arrives — see I-note below.) Hex-float literals; binary literals (`0b…`) + digit separators (`'`). | `D-CSUBSET-MOD-OP-CODEGEN`, `D-CSUBSET-MOD-OP-CODEGEN-OUTPUT-INDEX-CONTRACT`, `D-MIR-TO-LIR-DIV-SEQUENCE-AGNOSTIC` *(NOTE: `D-CSUBSET-DIVISION-OP-CODEGEN` is ALREADY CLOSED 2026-06-04 — signed division shipped; FC1 only adds modulo + the literals)* |
