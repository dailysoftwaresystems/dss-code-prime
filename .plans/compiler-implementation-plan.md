...
> - **[`substrate-hardening-plan.md`](./substrate-hardening-plan.md)** — three small de-risking PRs that land **between v2 and the tokenizer phase**: SH1 landing-log generator (kills the recurring hash-backfill bug class), SH2 Linux CI matrix entry (verifies the cross-platform claim), SH3 cross-tree `NodeId` guard (closes the documented `tree-model.md` §5.10 caveat). **🔵 in progress.** Prerequisite for parent phase #5 (tokenizer). SH2 was satisfied by the `DSS.DevOps@v2` reusable workflow (Linux/GCC + Linux/Clang+ASan + Windows/MSVC matrix default-enabled; macOS opted in here). SH3 shipped — `NodeId` now carries a `treeTag` and `NodeAttribute<T>` / `Tree::node_` validate it on every access. SH1 is the remaining substrate work.
>
...
| Substrate hardening (sub-plan SH1–SH3) — landing-log generator + Linux CI matrix + cross-tree `NodeId` guard | 🔵 **in progress** — SH2 ✅ (multi-OS matrix already shipped via `DSS.DevOps@v2`; macOS opted in). SH3 ✅ (`NodeId` carries `treeTag`; cross-tree access aborts loudly). SH1 pending. Prerequisite for tokenizer phase. |
| `source-factory/` thin facade | ⏳ waiting on remaining core-types work |
| `tokenizer/` + `analysis/{lexical,syntactic,semantic}/` | ⏳ blocked on Substrate Hardening |
...
| 4.5 | 🔵 **next** — sub-plan [`substrate-hardening-plan.md`](./substrate-hardening-plan.md) | `substrate-hardening` | SH1 landing-log generator + SH2 Linux CI matrix + SH3 cross-tree `NodeId` guard | core-types, schema-v2 | De-risks the substrate before the tokenizer/parser/semantic phases land on top of it. SH1 stops landing-log drift (recurring bug class through v2). SH2 verifies the cross-platform claim that `CLAUDE.md` asserts but only Windows has exercised. SH3 closes the documented cross-tree `NodeId` caveat (`docs/tree-model.md` §5.10). Bounded ~1–2 days of work; the per-PR review cadence is preserved. **Blocks** phase #5. |
| 5  | ⏳ blocked on #4.5 | `tokenizer` | Implement tokenizer | core-types, source-factory, substrate-hardening | Implement `Tokenizer` against `SourceBuffer` + `GrammarSchema`. Emits *all* tokens including whitespace/comments (`CoreTokenKind::Whitespace`/`LineComment`/`BlockComment`); the builder applies `NodeFlags::EmptySpace`. First real consumer of v2's `lexerModes` + `stringStyle` machinery — what it surfaces becomes the candidate set for any future `schema-expressiveness-v3-plan.md`. |
