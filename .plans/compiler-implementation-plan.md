...
| Schema expressiveness v2 (sub-plan PR0–PR5) — c-subset stress + operator precedence (`OperatorTable`) + real `SchemaCursor` walker (compiled position tables, FIRST/NULLABLE, AltChoice routing) + contextual keywords + `scopeRequire` + `TreeBuilder::Checkpoint` + `lexerModes` / `LexerModeStack` / `modeOp` | 🟡 **in flight** — PR0–PR5 shipped + review-fixed; PR6 (`stringStyle`), PR7 (tsql stress), PR8 (cross-plan close-out) pending |
| **Total ctest cases across both plans** | **458 cases across 24 suites, 100% pass** |
