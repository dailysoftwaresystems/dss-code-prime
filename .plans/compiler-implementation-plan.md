...
| 5  | 🔵 **next** | `tokenizer` | Implement tokenizer | core-types, source-factory, substrate-hardening | Implement `Tokenizer` against `SourceBuffer` + `GrammarSchema`. Emits *all* tokens including whitespace/comments (`CoreTokenKind::Whitespace`/`LineComment`/`BlockComment`); the builder applies `NodeFlags::EmptySpace`. First real consumer of v2's `lexerModes` + `stringStyle` machinery — what it surfaces becomes the candidate set for any future `schema-expressiveness-v3-plan.md`. Sub-plan in `.plans/tokenizer-plan.md` (to be authored). |
