...
| | |
|---|---|
| Status        | 🟢 **PA0 ✅ + PA1 ✅ done; PA2 next.** PA0 (`SchemaWalker` extraction) shipped as `c764c2a`. PA1 (schema-driven iterative RD driver with try-each-branch speculation) shipped + 5-agent review + fix-everything round. Speculation re-architected from "single probe + fall-through" to proper try-each-branch with snapshot/restore per candidate; speculation depth cap + abandon-token guard added. **PA1 follow-up: `SpeculationProbe` RAII guard landed** (open-question #5 closed) — the four-machine snapshot/restore is now structurally local to the probe, no longer convention; mirrors `TreeBuilder::Checkpoint`. 668 tests / 30 suites green (22 parser + 646 pre-existing); zero behavior change. <!-- LANDING-LOG-HASHES: PA1 --><!-- /LANDING-LOG-HASHES --> |
