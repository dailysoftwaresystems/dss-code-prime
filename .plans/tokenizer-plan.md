...
>
> **Next action: open the parser phase (#7).** `compiler-implementation-plan.md` §8 row 7 (`analysis-syntactic`) is `🔵 next`. The schema-driven cursor is in place, the tokenizer is feeding live tokens, the builder synthesizes meanings for built-in kinds and skips off-grammar body chars — what's missing is the actual parser driver that consumes a `TokenStream` and drives `TreeBuilder::open`/`pushToken`/`Checkpoint` from the compiled shape graph.
