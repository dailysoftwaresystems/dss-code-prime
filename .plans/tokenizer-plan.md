...
|---|---|---|
| TZ1 | ✅ done | Bare tokenizer skeleton (`src/tokenizer/`) — `SourceReader` + `Tokenizer` + `TokenStream`. Whitespace, single-char punctuation, identifiers/keywords, integer + float literals (hand-coded with base-prefix recognition for `0x`/`0b`/`0o` and restricted suffix charset), Eof. No lexer modes, no string styles, no comments. `Token.schemaKind` ownership moved from builder to tokenizer; `Builder.pushToken` gains a fast-path that trusts `tok.schemaKind` when scope-allowed, falls back to full lookup otherwise (preserves `P_AmbiguousToken` warning). New diagnostic code `P_IllegalChar`. Longest-match probe length now derived from `GrammarSchema::maxLexemeLength()`. UTF-8 BOM skipped at byte 0; lone continuation bytes flagged as `P_IllegalChar`. `SourceBuffer::fromFile/fromString` enforce the 4-GiB `ByteOffset` cap. Existing `test_tree_end_to_end.cpp` (9 tests) flipped from hand-tokenization to real tokenization against `toy.lang.json` — all pass identically. 57 new tokenizer unit tests across 3 suites (45 initial + 12 review-fix). 588 ctest cases / 29 suites, 100% pass. <!-- LANDING-LOG-HASHES: TZ1 -->`5d1d95d`<!-- /LANDING-LOG-HASHES --> + `8f08599` (review-fix). |
