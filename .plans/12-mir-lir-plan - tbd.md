...
| D-ML6-3.1 | Rewrite pass + spill load/store insertion + `LirVerifier` "no virtual registers after regalloc" rule | substrate ready post-3a (variant payload + reporter + `LirSpillSlot` strong id all in place). | **ML6 cycle 3b** — consumes `LirAllocation` to produce a rewritten `Lir` module with phys regs throughout + spill frame-load/frame-store pseudo-ops. ML7 callconv lowering materializes the pseudo-ops into stack-pointer + offset encoding per target. |
