...
| ML6 🟡 | Linear-scan register allocation                  | Per `07-prod-readiness G-512`. Live-range computation + virtual-reg coloring + spill-slot insertion. **Absorbs cycle-3e deferrals D-3e.1 (lowerSwitch first-cmp/first-jcc block-placement pin), D-3e.7 (`buildSyntheticFn` promotion), D-3e.8 (reverse Bitcast I64→F64 test), D-3e.9 (`condCodeForICmp` Optional return + poisonValue path)** — see §3.1. **Cycle 1 ✅ done 2026-05-29**: liveness analysis substrate (`src/lir/lir_liveness.{hpp,cpp}`) with `VRegBitset` sentinel-aware bounds-checking set, `LirLiveRange::make()` factory enforcing `start < end`, `LirLiveness::forFunc(LirFuncId)` accessor, RPO via `funcBlockAt`-enumerated mapping (no contiguous-arena assumption), iterative backward dataflow to fixpoint, half-step position numbering (early=2N / late=2N+1), block-end half-open boundary (`lastUse = blockEndPos - 1` for liveOut extension), orphan-block-tolerant. Target-blind (no `TargetSchema` param) + source-blind. All 4 cycle-3e ML6-anchored deferrals folded. 7-agent review fix-up folded: killed `VRegSet` anon helper in favor of substrate `VRegBitset`; dropped dead `TargetSchema` parameter; deleted dead `blockOrderIdx` local; fixed contiguous-arena `blockBase` arithmetic via `funcBlockAt` enumeration; D-3e.1 extended to a pre-jcc second pin point; D-3e.9 now also `poisonValue(id)` to prevent diagnostic cascade. **Real bug caught by tightened test**: liveOut-driven range-end was overshooting `totalPositions` by 1 (the `+1` for half-open `end` was applied on top of an already-half-open `blockEndPos` boundary). 12 LirLiveness tests + 96/96 ctest. **Cycle 2 ⏳ next**: linear-scan allocator consuming the substrate. |
...
| D-ML6-1.1 | Split-aware sub-interval `LirLiveRange` list (currently flat single interval per vreg; linear-scan with split-point selection wants disjoint `[start_i, end_i)` segments per vreg) | architect HIGH in ML6 cycle 1 review | needs ML6 cycle 2 allocator as co-design consumer to validate the sub-interval shape. Flat intervals work for correctness; sub-intervals refine spill-point quality. | **ML6 cycle 2** — substrate-shape decision made alongside the allocator that consumes it. |
