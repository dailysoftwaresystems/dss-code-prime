...
| D-ML7-1.1 | Shared `lir_pass_util.hpp` for `emitTerminator`/`report`/`remapOperand` helpers (currently duplicated across cycle-3b rewrite + cycle 3b verifier + ML7 callconv) | simplifier HIGH — cross-cycle refactor touches cycle 3b. | **cycle 4+ LIR substrate cleanup pass** alongside `kLirRegClassCount` + `PerLirRegClass<T>` (type-design Action 2 from cycle 3a) + `LirRewriteResult.ok` derivation (matching cycle-3a discipline). |
| D-ML7-1.2 | `NamedRegisterRef` collapse (`StackPointerRef` + `LinkRegisterRef` are exact-copy structs) | type-design MED. | **cycle 4+ LIR substrate cleanup** — fold with D-ML7-1.1. |
