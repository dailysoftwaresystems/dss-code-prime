...
| D-ML6-2.1 | `LirRegAssignment` → `std::variant<LirReg, std::uint32_t>` payload (eliminates `UINT32_MAX` sentinel coupling; default-constructed slot becomes well-typed "unassigned" arm) | architect CRITICAL + type-design top finding in ML6 cycle 2 review | substantial type-change touching every consumer of `LirRegAssignment`. Current struct + factories enforce phys-XOR-spill correctly — variant is a type-design quality improvement, not a correctness fix. | **ML6 cycle 3** (rewrite pass) — cycle 3 will iterate the assignments per LIR inst; variant migration done as the first cycle-3 substrate change so the rewrite loop is written against the final shape. |
| D-ML6-2.2 | `DiagnosticReporter&` threading through `allocateRegisters` (currently schema-config errors fatal via `regallocFatal`; spill decisions silent) | silent-failure CRITICAL/HIGH + architect MED in ML6 cycle 2 review | substantial signature change. Current invariant violations (no CC declared, factory misuse, None-class vreg) are genuine programmer errors that `regallocFatal` is correct severity for. Reporter routing is needed for surfacing SPILL REASONS to consumers (debugging quality). | **ML6 cycle 3** (rewrite pass) — natural fold-in point since the rewrite pass must surface "vreg X spilled because of reason Y" diagnostics anyway. |
