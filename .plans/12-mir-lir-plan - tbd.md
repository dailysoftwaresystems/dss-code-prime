...
| ML2 🟡 | HIR→MIR lowering                                 | **Cycle 1 ✅ done 2026-05-28**: new `src/mir/lowering/` OBJECT lib with `lowerToMir(hir, literals, interner, reporter) → HirToMirResult`. Vertical slice through straight-line c-subset: Function decl + Block + ReturnStmt + Literal + Ref(param/var-with-init) + BinaryOp (Add/Sub/Mul/Div/Rem/BitAnd/BitOr/BitXor/Shl/Shr/Eq/Ne/Lt/Le/Gt/Ge — type-driven signed/unsigned/float opcode pick). Implicit-void-return synthesis for `void f() {}`. New `MirBuilder::openBlockHasTerminator()` introspection. Failed lowerings seal blocks with `addUnreachable` so `finish()` completes + diagnostics reach the caller (no aborts). Top-level Global/Extern*/ImportGroup emit `H_UnsupportedLoweringForKind` fail-loud (per project discipline); TypeDecl is a no-op (no runtime effect). 88/88 ctest including 6 ML2 tests. Cycle 2 next: control flow (If/While/For), Call, MemberAccess+Index lowering, source-map injection for spans on diagnostics. |
