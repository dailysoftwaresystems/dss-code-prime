...
⭐ **UPDATED 2026-09-04 — the caveat this paragraph carried is gone, because the escape hatch is.** It used to warn that `@manual` (an annotation suspending automatic reclamation, at one point proposed as `@kernel`) was **not** the freestanding mechanism, since this plan's own use of *kernel* made the spelling actively misleading. [`plan-24`](./24-dss-axis-language-reserved-plan%20-%20tbd.md) §2.3a **deletes `@manual` from DSS Axis entirely** — no escape hatch under any spelling — so a driver simply gets T0–T3 automatically, with no annotation to avoid and no confusion to warn about.

⚠ **What replaces the caveat, and it is a real constraint on this plan.** The new model materializes a **dynamic ownership record (TD)** wherever a lifetime cannot be proven statically ([`plan-24`](./24-dss-axis-language-reserved-plan%20-%20tbd.md) §3.1c, [`plan-09.5`](./09.5-dss-hir-plan.md) §4.2a), and **TD needs atomics plus a declared storage provider.** Atomics are ISA-level and freestanding-safe; storage is not — a boot image has none until it builds one. ⇒ a freestanding profile **omits TD from its lattice**, and there an unprovable residual is a **build refusal naming the missing provider** rather than silent runtime machinery linked into a kernel image. That is the correct posture for this plan and it is declared in config, never per object. See [`plan-24`](./24-dss-axis-language-reserved-plan%20-%20tbd.md) §3.1a and §9.1 below.

★ **And the un-inferable case the annotation existed for — a DMA buffer whose lifetime the hardware owns — is now DESCRIBED rather than hand-managed:** its storage is owned by an external agent, which is a semantic fact the compiler acts on at the FFI boundary ([`plan-09.5`](./09.5-dss-hir-plan.md) §4.3's honesty clause), not a reason to switch the model off for it.
