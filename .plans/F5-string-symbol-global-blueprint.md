...
3. **Assembler dispatch** — 3a ✅ **DONE (commit pending, 404/404)**: lowerMirGlobalsToDataItems gained an `absPtrRelocKind` param (asm.hpp+asm.cpp); compile_pipeline.cpp finds it by FORMULA (widthBytes==8 && !pcRelative) from target.relocations() + passes it; the MirSymbolAddrValue asm arm emits an 8-byte zero slot + a Relocation{offset 0, target, abs64-kind, addend} into AssembledData.relocations (section already chosen const→.rodata/mutable→.data). Builds; ctest green (no symbolAddr corpus exercises the linker path yet). **3b PENDING (PE .data gate — the bug-prone spot):** pe.cpp line 705 `dataBytes` is `const&` → make a MUTABLE copy; add a `dataOffsetByIndex` map (mirror `rdataOffsetByIndex` at 709-713 from `dataDataLayout`); the data-item reloc loop (955-1061, currently fails loud at 984-992 if the item isn't `.rdata`) → look up the item in rdata OR data, patch into the right buffer (rdataBytes/dataBytes) at the right section RVA (`rdata->rva`/`data->rva`), keeping the abs64 baseRelocSiteRvas collection (line 1058). VERIFY the patched mutable `dataBytes` is what's emitted to the image (grep `dataBytes` for the emission site). Then test `decl_string_global` on x86 PE.
