...
| D-AS4-5 | **x86 indexed/scaled memory addressing** (`[base + index*scale + disp]`) — the second half of D-AS4-1's original scope (the first half — `[base+disp32]` — closed at the D-LK10-2 cycle 2026-06-01). Needs: new `OperandKindFilter::Reg` slot at a NEW position in load/store guards (after `MemBase` carrying scale), new `EncodingSlotKind::SibIndex` (writes SIB.index bits 3-5 from a Reg operand's lo3 + REX.X from hi bit — `rexX` field on `EncodingState` is already declared at AS2 but unused, gets wired here), generalized scale support in `wireMemBaseScale` (accepts scale ∈ {1,2,4,8}), walker change to emit SIB byte unconditionally when an index is present (independent of the rsp/r12 force-presence rule that fires today on no-index addressing). The MIR→LIR isel's `lea` lowering at `src/lir/lowering/mir_to_lir.cpp:255` already builds the 4-operand `[base, index_reg, MemBase(scale), MemOffset(disp)]` shape — the assembler just doesn't decode it yet. | **AS-cycle when first c-subset corpus uses array indexing** (e.g. `int x = a[i];` lowered to `Gep(base, i*sizeof(int))` → `lea reg, [base + i*4 + 0]`). | First `[base + index*scale]` shape reaching the assembler. |
| D-AS4-6-byte-pin | **No-SIB negative test coverage gap** — the D-LK10-2 cycle's byte-pin tests in `tests/asm/test_asm_x86_variable.cpp` pin the no-SIB family with `[rbp+0x10]` (rm.lo3=5) and `[rbx+0x10]` (rm.lo3=3). The remaining no-SIB lo3 values (6=rsi, 7=rdi, and lo3=0-2=rax/rcx/rdx) aren't pinned. Low-severity: the SIB-forced rule fires ONLY on lo3==4, so a regression that breaks the no-SIB path would also break rbp and rbx — the existing tests are sufficient defense. Anchored for completeness when D-AS4-5 lands (the cycle that touches the SIB emission logic). | **D-AS4-5 cycle landing** — same code path that introduces indexed addressing also gains lo3∈{0..3,5..7} no-SIB negative tests. | When the SIB emission logic next changes. |
