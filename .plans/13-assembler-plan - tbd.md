...
| D-AS3-2 | **Walker-util consolidation** (scope EXPANDED 2026-05-29 per architect AS4 review): `hwEncodingOf` (5 vs 4-bit-cap parameter), `operandsMatchGuard`, `filterToLirKind`, `PendingRelocSlot` POD, AND the per-instruction relocation-emit block (`relocs.push_back(Relocation{out.size(), …})`) are all duplicated x86_variable.cpp ↔ fixed32.cpp — five items, not one. The `byte_emit.hpp` substrate (AS3 followup) already hoisted `appendU32LE`. | **AS-cycle when a 3rd walker lands** (e.g. `vliw_bundle` or RISC-V compressed 16-bit). 3-walker threshold is the project's standard lift trigger. The cycle author hoists `walker_util.{hpp,cpp}` covering all 5 items alongside the new walker. | Third format-walker landing. |
