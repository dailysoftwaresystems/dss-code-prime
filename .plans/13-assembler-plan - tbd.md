...
| ~~D-AS3-2~~ ✅ **partially closed 2026-05-29 (AS5 follow-up)** — `walker_util.hpp` hoisted | `asm/format/walker_util.hpp` shipped with `hwEncodingOf` (bit-width-parameterised — `maxBitWidth=4` for x86, `maxBitWidth=5` for fixed32), `operandsMatchGuard`, `filterToLirKind`, `readU32LE`. Architect AS5 review's "no blocker exists, extract now" — four files (`x86_variable.cpp`, `fixed32.cpp`, `x86_variable_disasm.cpp`, `fixed32_disasm.cpp`) had been duplicating the same helpers; one source of truth now. **Still owed**: `PendingRelocSlot` POD (currently in both encoder walkers as a local struct — has a single use point inside encode() so the duplication is shallow) AND the per-instruction relocation-emit block (`relocs.push_back(Relocation{out.size(), ...})`) — that block is 5 lines and currently appears 2x. These two close in the same AS-future cycle that touches the walker emit tails (likely AS6 or D-AS5-3's restructure). 108/108 ctest unchanged through extraction. | — (partial close) |
...
| D-AS5-3 | **`DisassembledInst` shape — split `slots` into `optional<DisassembledSlot> result; vector<DisassembledSlot> wires`.** Type-design AS5 review rated this "right fix" — currently the flat `slots: vector` requires the caller to know the positional contract "first slot is resultSlot when present, rest are wires." Mirroring the encoder's `TargetEncodingVariant` shape (which already splits `resultSlot: optional` + `wires: vector`) makes the invariant structural. Zero-runtime-cost; touches both walkers' tail emit blocks + `roundTripVerify`'s slot iteration. Also folds the related "symbol-slot value sentinel" finding: change `DisassembledSlot::value` from `int64_t` to `optional<int64_t>` so Disp32/Imm26 slots use `std::nullopt` instead of the `0` sentinel (which today collides with a legitimately-zero Imm32). | **AS-cycle when D-AS3-2 walker-util consolidation lands**, OR a dedicated AS5-followup cycle if walker-util waits longer than D-AS5-3 can stand. Type-design rated this routinely fixable but invasive (touches every walker emit and every test consumer); waiting for the natural walker-touch window avoids two structural changes. | First walker-side refactor that touches the disasm tail emit blocks. |
