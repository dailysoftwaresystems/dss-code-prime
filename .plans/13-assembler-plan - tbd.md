...
| ~~D-AS4-3~~ ✅ **CLOSED 2026-06-08** (the ARM64 `lea` = ADRP+ADD multi-word cycle — its named trigger "ARM64 ADRP imm21 + ADD imm12 pair" fired). Generalized the fixed32 encoder from one-word-≤1-reloc to a GENERIC multi-instruction-macro / multi-relocation substrate, all generic vocabulary (zero `if(arch)`): `TargetEncodingTemplate.fixedWords` (N-word base) + `wordCount()`/`wordAt()`; `TargetEncodingWire.wordIndex`; `PendingRelocSlot.wordIndex`; a per-word `words` vector + per-(word,slot) `wroteSlot`; `orInto(slot,value,wordIndex)`; the SymbolRef arm widened from `!=Imm26` to `!isSymbolBearingSlot` + a `pendingRelocs` VECTOR; the emit loop stamps each reloc at its word's START (`out.size()` before that word — derived from the cursor, NOT `base+i*4`). New generic `EncodingSlotKind::SymbolPatchMarker` (width-0 symbol-patch marker — the linker owns the field; one marker serves BOTH ADRP page-reloc + ADD lo12-reloc, distinguished by the wire's wordIndex+relocationKind) + `TargetEncodingVariant.extraResultSlots` (the result reg threaded into word0.Rd + word1.Rd + word1.Rn). validate() made word-aware (per-(wordIndex,slot) uniqueness across resultSlot∪extraResultSlots∪wires; wordIndex<wordCount; fixedWords-only-on-fixed32; both-set reject; extraResultSlots-requires-resultSlot). Single-word path byte-identical (207/207, x86 + all existing fixed32 unchanged). Proof: `Arm64Encoder.{LeaEmitsAdrpAddWordPair, LeaStampsTwoRelocsAtWordOffsets, LeaProducesOneSrcMapEntrySpanning8Bytes}` (RED-on-disable DEMONSTRATED: word-blind `wroteSlot` → all 3 fail) + 4 `EncodingValidate.MultiWord*` synthetic pins. Linker side already proven (`test_aarch64_reloc_formulas.cpp`). RUNNABLE end-to-end corpus DEFERRED (see D-LK1-ELF-EXEC-DATA-SECTIONS — the ELF ET_EXEC writer emits no .data/.rodata, so a data global can't reach the binary). | ✅🧾 (closed 2026-06-08 — the ARM64 lea multi-word cycle; the verdict prose sits in the preceding cell, which is where the mark had wrongly been) | First non-trailing symbol-bearing slot — FIRED: ARM64 lea ADRP+ADD. |
...
| D-AS4-1 | **x86 memory addressing modes**: SIB/Disp32 slot kinds (`SibBase`/`SibIndex`/`SibScale`/`Disp8`/`Disp32`) + Load/Store/Lea encoding rows. `EncodingState::rexX` already declared but unused (AS2 follow-up substrate). **PARTIAL CLOSE 2026-06-01** at the LK10 cycle 2 ret-arm fold cycle. **Closed half**: `[base + disp32]` form for `load`/`store` opcodes — new `OperandKindFilter::MemBase/MemOffset` + new `EncodingSlotKind::ModRmRmMem/MemBaseScale/Disp32Mem` (substrate; symbol-bearing distinction preserved), new `ModMode` enum + SIB-forced rule (when `rm.lo3 == 4` for rsp/r12 family) + signed 32-bit displacement emission, validate-rule G's `isDestSlot` extended to include `ModRmRmMem` (silent-failure F-G1 post-fold catch). 7 byte-pin tests in `tests/asm/test_asm_x86_variable.cpp` cover rsp-SIB / rbp-no-SIB / r12-SIB+REX.B / negative-disp two's-complement / prologue / epilogue forms. **Still open**: `lea` encoding rows (architecturally similar to load/store, separate row when first consumer surfaces), indexed/scaled addressing (`SibIndex`/`SibScale` slots + `EncodingState::rexX` wiring — first `[base + index*scale + disp]` consumer), `Disp8` (8-bit displacement form — peephole when disp fits in i8 saves 3 bytes per inst). | **Remaining half**: AS-cycle when first indexed-address consumer lands (e.g. c-subset `a[i]` lowered to `[base + index*scale + 0]` via Gep), OR a dedicated peephole pass for the disp8 form. The substrate is ready; only schema variants + walker dispatch extension remain. | First c-subset corpus that exercises indexed addressing OR `lea` byte emission. |
...
| D-AS5-MULTIWORD-DISASM | **Multi-word round-trip disassembly.** `fixed32_disasm::disassemble` reads a SINGLE 32-bit word (matches `tmpl.fixedWord`, `bytesConsumed = 4`). With D-AS4-3's multi-word encoder (2026-06-08), a multi-word opcode (`tmpl.fixedWords.size() > 1`, e.g. ARM64 `lea` = ADRP+ADD) is NOT round-trippable through the oracle — it fails LOUD (`A_RoundTripMismatch`: the variant's `fixedWord` base is 0 so no single-word match), never silently mis-decodes. The shipped multi-word opcode (lea) is proven instead by exact byte-pins (`Arm64Encoder.Lea*`) + the linker reloc-formula tests — strictly stronger than round-trip for a reloc-bearing opcode whose immediate is linker-owned. Cited at `fixed32_disasm.cpp` (the `bytesConsumed = 4` site). | **Cycle when a SECOND multi-word opcode lands OR multi-word round-trip coverage is needed beyond byte-pins.** Add a per-word match/extract loop (read N words, match each against `wordAt(i)`, sum `bytesConsumed = N*4`), AND generalize the symbol-bearing-slot special-case (currently `== Imm26`) to `isSymbolBearingSlot(slotKind)` so `SymbolPatchMarker` yields `std::nullopt` (consult the Relocation, not the bytes). The multi-place result reg (lea's Rd in 3 slots) needs a consistency check. | Second multi-word fixed32 opcode, or a multi-word round-trip requirement. |
...
| ~~D-AS4-5~~ | ✅ **CLOSED 2026-06-01.** x86 indexed/scaled memory addressing (`[base + index*scale + disp]`). New `EncodingSlotKind::SibIndex = 11` + `kRelocFormulaTable` extension; slotShapeFor + isSymbolBearingSlot updated; static_assert count 12. Walker `x86_variable.cpp`: SibIndex wire writes `EncodingState.sibIndex3` + drives `rexX` (AS2-declared `rexX` field finally consumed); rejects `rsp` (lo3=4, hi=0) as index (reserved as no-index marker); generalised `wireMemBaseScale` accepts scale ∈ {1,2,4,8} storing 2-bit exponent into `sibScaleExp`; new coherence guard rejects SibIndex without ModRmRmMem (silent-failure defense). Walker emission: SIB byte emits in TWO cases — (a) D-AS4-1 force-presence (rm.lo3==4) OR (b) SibIndex wired; ModR/M.rm correctly emits the SIB-follows marker (0b100) instead of base.lo3 in BOTH cases (fixed the pre-existing "worked by accident because rsp/r12.lo3==4" code path). Schema: 3 new opcode encoding variants in `x86_64.target.json` — `lea` 3-op (no-index) + `lea` 4-op (indexed) + `load` 4-op (indexed) + `store` 5-op (indexed). 11 new tests (LeaWithIndex scale=1/4/8, R12-derived REX.X, rsp-index reject, illegal-scale reject, 3-op regression, D-AS4-6-byte-pin no-SIB negatives for rax/rsi/rdi). MIR→LIR isel's existing 4-operand `[base, index, MemBase, MemOffset]` Gep lowering at `mir_to_lir.cpp:589-600` now flows end-to-end. **Paired closure**: D-AS4-6-byte-pin folded with this cycle as anchored (the cycle that touches SIB emission). 127/127 ctest. |
| D-AS4-7 | **`EncodingState::wroteModRm{Reg,Rm}` flags → `std::array<bool, kEncodingSlotKindCount>` hoist** — at D-AS4-5 close (2026-06-01), there are 2 `wroteX` plain bools left after the `wroteSibIndex` collapse to `sibIndex3.has_value()` (code-simplifier S1). `target_schema.hpp:48` comment already cites the slot-ordinal-table approach used elsewhere. Hoisting now would force every `EncodingSlotKind` (12 today) to carry a slot — premature given we only need 2. **Trigger**: 4th `wroteX` flag landing (e.g. a new ModR/M-family slot or a new SIB-family slot). Type-design O1 + code-simplifier S1 anchored. | **AS-cycle that adds a 4th ModR/M-family or SIB-family wireSlot arm with a duplicate-write check**. | First non-Reg/Rm wireSlot arm needing a write-guard flag. |
| D-AS4-8 | **`EncodingState::ModMode` enum extension** — disp8 (mod=01) + no-disp (mod=00) short-form encodings. The closed enum's underlying value IS the ModR/M.mod field bits (invariant); extending with `MemDisp8 = 0b01` and `MemNoDisp = 0b00` scales cleanly. Walker dispatch: peephole during emission ("if disp fits in i8, emit 3 fewer bytes"). Today the walker always emits disp32 — leaves 3 bytes per spilled stack-frame access. Type-design D-AS4-5 post-fold anchored. | **Peephole optimizer cycle that targets stack-frame access size** (likely paired with plan 22 OPT* once OPT1 substrate lands) OR a dedicated AS-cycle when first `[base+small_disp8]` workload measurably dominates code size. | First measurable code-size regression on a stack-spill-heavy corpus. |
| ~~D-AS4-6-byte-pin~~ | ✅ **CLOSED 2026-06-01 alongside D-AS4-5.** Added no-SIB negative tests for rax (lo3=0), rsi (lo3=6), rdi (lo3=7) in `tests/asm/test_asm_x86_variable.cpp`. Combined with pre-existing rbp (lo3=5) and rbx (lo3=3) tests, the no-SIB family is comprehensively pinned. The remaining lo3 values (rcx=1, rdx=2) are pinned indirectly via the ModR/M.reg-side tests. |
