...
| D-AS3-2 | **Walker-util consolidation**: `hwEncodingOf` / `operandsMatchGuard` / `filterToLirKind` duplicated x86_variable.cpp ↔ fixed32.cpp. The two helpers are byte-for-byte the same shape; the `byte_emit.hpp` substrate (AS3 followup) hoisted `appendU32LE` already. | **AS-cycle when a 3rd walker lands** (e.g. `vliw_bundle` or RISC-V compressed 16-bit). 3-walker threshold is the project's standard lift trigger (simplifier explicitly cited this). The cycle author hoists `walker_util.{hpp,cpp}` alongside the new walker. | Third format-walker landing. |
...
| D-AS3-6 | **Fixed32 immediate/shift slot kinds**: `Imm12` (12-bit immediate, bits 10..21 for ADD/SUB-imm), `ImmShift` (shift amount), `Sf` flag (64-bit-operand selector). AS3 cycle 3 walker is register-only (validate rejects non-Reg variants on fixed32). | **AS3b cycle (ARM64 binary ops extension)** — paired with `add`/`sub`/`mul`/`and`/`or`/`xor` reg-imm encoding rows on `arm64.target.json`. One new `EncodingSlotKind` entry per slot + one new walker arm per consumer in the same PR. | ARM64 binary-op encoding cycle (AS3b). |
...
| D-AS5-2 | **Format-dispatched disassembler harness** (plan 13 §3 row AS5). Per-format `*_disasm.cpp` files round-trip every encoded variant against a disasm oracle. | **AS5** — explicit plan row already exists. | Coverage need; can land independently of AS4. |
