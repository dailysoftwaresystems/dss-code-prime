...
| CLI surface   | ✅ **`--transpile <files>...` wired into `Program::run` dispatcher at LK10 cycle 3 (2026-06-01).** Today `Program::transpile(sourceFiles, languageName, targets)` fails loud with `D_PlanNotLanded` citing this plan. When ST1..ST6 lands, the implementation slot is here — the CLI surface stays stable across plan 10's arrival (no driver-tier churn needed). |
