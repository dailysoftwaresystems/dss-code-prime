...
| D-LK1-2 | **ELF ET_EXEC executable image path.** LK1 cycle 1 ships ET_REL relocatable .o only. ELF executables need `e_type = ET_EXEC`, program headers (PT_LOAD segments + PT_PHDR), an `e_entry` start address, and section-to-segment mapping. Paired with D-LK2-1 (PE32+) + D-LK3-2 (Mach-O image) — the three image-side paths share the structural delta of "add an optional/program-header block" + entry-point machinery, and naturally land together. | **LK1 cycle 2** — paired with LK2 cycle 2 + LK3 cycle 2. The three image-side paths form a single coherent vertical slice. | First Linux executable end-to-end test. |
