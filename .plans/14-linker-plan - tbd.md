...
| D-LK4-9 | **`StringTable` substrate hoist** (simplifier deferral). ELF and PE both ship a `StringTable` class with `add(name) → offset` that builds a flat byte buffer used in the symbol/section name tables. The two implementations diverge on initialization (ELF seeds with NUL byte at offset 0; PE reserves 4 bytes for an inclusive u32 size prefix and finalizes the size at `release()`) — premature templating would obscure the per-format invariants. Defer until a third format (Mach-O LK3 OR WASM LK8 OR SPIR-V LK9) consumes the same shape, at which point the common API (add + view + size) lifts to `src/link/format/string_table.hpp` with the format-specific init/finalize hooks declared as policy. | **LK3** (Mach-O) — the third consumer. The lift goes in the same PR as the Mach-O walker so all three walkers migrate together. | Third format walker landing. |
