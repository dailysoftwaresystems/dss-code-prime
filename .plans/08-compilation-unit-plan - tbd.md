...
| CU2 | Multi-file `UnitBuilder` + driver wiring        | `UnitBuilder::addFile(path)` / `addInMemory(text, label)` → tokenize → parse → `addTree`. **Lexer + parser diagnostics unified in the `Tree`** via `TreeBuilder::ingestDiagnostics` + an optional `Parser` lexer-diag ctor param (Q1=C; see §2.6 C2-L1 — cross-plan into 01 + 05). Continue-on-failure (§2.6 C2-L2). First `D_*` codes: `D_FileNotFound`/`D_EmptyInput`/`D_DuplicateFile` (§2.6 C2-L3). Per-file order preserved (matters for include semantics). Entry-point is **not** here — re-routed to the driver/project-config layer (§2.6 D-PLAN08-C2-D2-ENTRY-POINT-SELECTION-THE-ROUTED). |
...
| CU5 | **Multi-language CU support** ✅ (landed with HR11) | Per the universal-compiler decisions in [`00-master`](./00-compiler-implementation-plan%20-%20tbd.md) §1 (rev 2): a single CU can contain files of **different source languages** when all converge on the same artifact via [HIR](./09-hir-plan%20-%20ok.md). Use case: a project with `.c` (c-subset) files calling FFI exports of `.sql` (tsql-subset) sprocs, compiled to one CLI binary. **Landed 2026-05-28** on `feature/hir-1` as the substrate half of HR11 — `UnitBuilder` holds a **schema registry** (`registerSchema` + multi-schema ctor); `addFile(path)` routes by matching the path extension against each config's declared `fileExtensions` (`.c/.h`→c-subset, `.sql/.tsql`→tsql, `.toy`→toy), `addInMemory(src,label,schema)` takes an explicit schema; each `Tree` carries its own schema (the hook already existed); a homogeneous CU is a registry of one and every CU1–CU4 caller is unchanged (the single-schema ctor delegates). Per-tree import resolution: each tree's imports resolve via **that tree's** schema's resolver, edges land in the CU-global `crossRefs`, injection is language-blind. `CompilationUnit::schema()` stays the documented first-tree/homogeneous convenience; per-tree language comes from `Tree::schema()`. **Explicitly DEFERRED (not in CU5/HR11):** (1) **auto-discovery of cross-*language* reference edges** — a c-subset call resolving to a tsql sproc requires FFI import/export name+ABI matching, which is [`11-ffi-plan`](./11-ffi-plan%20-%20tbd.md)'s job; CU5 delivers the *uniform, language-blind resolution substrate* (an existing/injected cross-tree edge resolves regardless of language) but does not synthesize cross-language edges. (2) **artifactProfile aggregation** when languages disagree → [`06-artifact-profile-plan`](./06-artifact-profile-plan%20-%20tbd.md) (the project-driver's job, not the CU's). (3) cross-language **type/ABI marshalling** → plan 11. CU6 (cross-CU references) still v1.x. |
| CU6 | **Cross-CU references** (v1.x)                    | Lifts the v1 single-CU-per-binary assumption: a `CompilationUnit` can hold typed references to symbols defined in *another* `CompilationUnit`. Use cases: (a) **shared libraries / DLLs** — each lib is a CU; the executable's CU links against them at the symbol level; (b) **incremental compilation** — each translation unit becomes a CU; the linker combines them. Builds additively on `CompilationUnitId` provenance (CU1 L2) and the SH3-analog cross-CU `NodeId` guard (CU3 D-PLAN08-D3-SH3-ANALOG-CROSS-NODEID-GUARD-NODEID) — both already exist by the time CU6 lands, so CU6 introduces no retroactive substrate change. **Couples with** [`14-linker-plan`](./14-linker-plan%20-%20tbd.md) **LK11** (cross-CU linking, also v1.x — see linker §2.12 for the LK11 scope, sequencing, and out-of-scope notes). **v1.x deliverable — trigger:** the first artifact profile that needs multiple CUs in one image (`lib` or `staticlib` consumed by a separate `cli`, or incremental rebuild scope) enters real scope. **UPDATE 2026-06:** the single-CU-per-image assumption HAS since been lifted at the MIR/linker tier — plan 14 LK11a/b (cross-CU symbol merge + cross-CU call) + `Program::compileUnits` + the MIR-tier `mergeCuMirs` whole-program merge (`D-OPT7-CROSSCU-MIR-MERGE` ✅ 2026-06-05; `examples/c-subset/cross_cu_call` runs exit 42), and the substrate hooks (compound `(cuId, SymbolId)` keying + guard) are consumed there. CU6's own FRONTEND surface — typed cross-CU references held by a `CompilationUnit` — remains open v1.x. |
...
> **The deferral rows that were listed here have moved into the anchor registries.** The open ones are in `.plans/_deferred-anchor-registry-production.md` and the closed ones in `.plans/_deferred-anchor-registry-done.md`, under `D-PLAN08-*`. A row whose id this plan spelled in a form no registry could hold carries its former spelling in its `Cross-refs` cell. Read the registry, never this document, for what is still open.


### 2.6 CU2 — locked decisions + explicit deferrals

> **Fulfills CU1 deferrals:** **D-PLAN08-D1-UNITBUILDER-ADDFILE-PATH-ADDINMEMORY-TEXT-LABEL** (`addFile`/`addInMemory`) ✅, **D-PLAN08-D2-DRIVER-LEVEL-DIAGNOSTIC-CODES-HEX-0XD001** (`D_*` codes — the file-level subset) ✅. **Re-routes D-PLAN08-D5-ENTRY-POINT-SELECTION-WHICH-SOURCE-THE** (entry-point) out of the CU layer entirely. Same discipline as §2.5: every cut names where it lands and why.
...
| C2-L3 | **Driver diagnostic codes** (all three) | `D_FileNotFound` (`0xD001`, Error), `D_EmptyInput` (`0xD002`, Info), `D_DuplicateFile` (`0xD003`, Warning). `addFile` dedups by `std::filesystem::weakly_canonical` path (warn + skip the repeat); `addInMemory` does **not** dedup (labels may legitimately repeat). Empty source → `D_EmptyInput` (Info) but the empty tree is still added (empty translation unit is valid, consistent with "empty CU is valid"). | Opens the `0xD` block per D-PLAN08-D2-DRIVER-LEVEL-DIAGNOSTIC-CODES-HEX-0XD001. Requires the `0xD` nibble in `diagnosticCodePrefix` + `diagnosticCodeName` cases + a `test_parse_diagnostic.cpp` pin. |
...
| 3 | How does the CU represent "this file is the entry point" (for `cli` artifactProfile)? | **Resolved (CU2, Q4):** the CU does **not** represent it. The entry-point is declared in `.dss-project.json`, parsed by the driver/program layer (phase #12), and stamped onto `CompilationContext` beside the `artifactProfile` ([06](./06-artifact-profile-plan%20-%20tbd.md) AP2/AP3). The earlier "first file added" heuristic is dropped — project config is the source of truth. See §2.6 D-PLAN08-C2-D2-ENTRY-POINT-SELECTION-THE-ROUTED. |
