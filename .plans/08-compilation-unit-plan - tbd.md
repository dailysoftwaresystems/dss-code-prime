...
| | |
|---|---|
| Status        | 🟢 **CU1 + CU2 shipped.** CU1: `CompilationUnit`/`UnitBuilder`/`CrossTreeRef`/`CompilationUnitId`. CU2: `addFile`/`addInMemory` (tokenize→parse→ingest, continue-on-failure), `D_*` driver codes, lexer+parser diagnostics unified in the Tree (cross-plan into 01/05 via `TreeBuilder::ingestDiagnostics` + optional `Parser` lexer-diag param). Full suite 48/48 green; substrate-tier 5-agent review + fix pass on each. CU3–CU4 ⏳ planned; CU5/CU6 v1.x. v1 production-critical; gates phase #8 (`analysis-semantic`). Production-readiness G-110 ✅ / G-111 (CU4). |
