...
| D7 | Cross-CU operations of any kind (references between two CUs) | **CU6** (this plan, v1.x — paired with [`14-linker-plan`](./14-linker-plan%20-%20tbd.md) **LK11** per its §2.12) | v1 ships single-CU binaries. CU1's `CompilationUnitId` provenance + CU3's cross-CU `NodeId` guard exist so CU6/LK11 land additively, with no retroactive substrate change. Trigger: first artifact profile needing multiple CUs in one image. **UPDATE 2026-06:** the MIR/linker-tier multi-CU image path has since landed (plan 14 LK11a/b + `compileUnits` + `mergeCuMirs`; `cross_cu_call` exit 42) — the single-CU contract no longer holds at that tier; CU6's frontend typed-reference surface remains open. |
