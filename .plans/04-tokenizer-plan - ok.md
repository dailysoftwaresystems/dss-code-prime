...
| | |
|---|---|
| Status        | ✅ **done.** TZ1, TZ2, TZ3 all shipped. 626 cases / 29 suites, 100% pass. Parent plan phase #5 (`tokenizer`) is closed; phase #6 (`analysis-lexical`) is subsumed; phase #7 (`analysis-syntactic`) opens next. |
| Predecessors  | ✅ v1 T0–T12 (`01-tree-node-model-plan - ok.md`); ✅ v2 PR0–PR8 (`02-schema-expressiveness-v2-plan - ok.md`); ✅ SH1–SH4 (`03-substrate-hardening-plan - ok.md`). |
...
|---|---|---|
| TZ1 | ✅ done | Bare tokenizer skeleton (`src/tokenizer/`) — `SourceReader` + `Tokenizer` + `TokenStream` returning a named `TokenizeResult { stream, diagnostics }`. Whitespace, single-char punctuation, identifiers/keywords, integer + float literals (hand-coded with `0x`/`0b`/`0o` base-prefix recognition; restricted suffix charset; `P_MalformedNumber` on empty-digit-prefix bodies like `0x_`), Eof. No lexer modes, no string styles, no comments. `Token.schemaKind` ownership moved from builder to tokenizer; `Builder.pushToken`'s fast-path trusts `tok.schemaKind` when scope-allowed, falls back to full lookup otherwise (preserves both `P_AmbiguousToken` warning AND slow-path-equivalent `matchCount`). New diagnostic codes `P_IllegalChar`, `P_MalformedNumber`. Longest-match probe length now derived from `GrammarSchema::maxLexemeLength()` with a debug-only consistency check in the ctor. UTF-8 BOM skipped at byte 0; mid-source BOM accepted as legitimate U+FEFF via byte-pass-through identifier model (v3 work for full Unicode XID semantics); lone continuation bytes (0x80-0xBF / reserved leads) flagged as `P_IllegalChar`. `SourceBuffer::fromFile/fromString` enforce the 4-GiB `ByteOffset` cap; `fromFile` also checks `ifstream::bad()` so mid-stream IO errors don't silently truncate. `Token` gains a trivially-copyable `static_assert`; `TokenStream` gets explicit move ops (defaulted-from-vector "valid but unspecified" was leaking past the empty-stream guard). Existing `test_tree_end_to_end.cpp` (9 tests) flipped from hand-tokenization to real tokenization against `toy.lang.json` — all pass identically. 68 new tokenizer unit tests across 3 suites (initial + two review-fix rounds). 599 ctest cases / 29 suites, 100% pass. `5d1d95d` (initial) + `8f08599` (review-fix round 1) + `33b3ec4` (review-fix round 2). Squash-merged into `fabe7b4` via PR #5. |
| TZ2 | ✅ done | Lexer modes + strings + comments. Tokenizer owns a single `frames` stack of `Frame { LexerModeId, StringStyle const*, std::string dynamicSuffix }` (review-fix replaced three parallel vectors — defensive empty-check guards retired as fatal-asserts). Body-mode branch resolves "doubled-delim → endsAt → char-escape → per-codepoint defaultToken" with UTF-8 codepoint length detection. `escapeKind`: `Char` consumes lead + next codepoint; `DoubledDelimiter` consumes doubled endsAt as a literal; `None` plain pass-through. Main-mode `modeOp`/`modeArg` applied AFTER token emit; `tagPattern` regex captured at opener for dynamic-suffix close matching (compiled regex cached per `StringStyleId`; plumbing in place; no shipped schema exercises it yet). EOF with open modes emits `P_UnterminatedString` or `P_UnterminatedComment` per schema-declared `unterminatedAs` enum (no more substring-match on mode name). New `LexerMode::defaultToken` holds a `DefaultTokenSpec { SchemaTokenId kind; NodeFlags flags }`; `Token` gains a `flags` field that the builder OR-merges with `meaning.flagsApplied` at pushToken time — c-subset's comment modes use `defaultToken.flags: ["EmptySpace"]` so the AST cursor skips comment bodies wholesale (closes `v2-gap-catalog - tbd.md` row 3 end-to-end). New diagnostic codes: `P_UnterminatedString`, `P_UnterminatedComment`, `P_InvalidEscape`. Loader-side cross-field validation rejects `endsAtLongestMatch + tagPattern` (silently disagrees at runtime); runtime fatal-asserts guard against `PushMode` into a defaultToken-bearing mode without a stringStyle and against `escapeKind=Char` with `escapeChar=0`. Initial commit added 10 test cases (line + block comments, untermination diagnostics, T-SQL doubled-delimiter + bracket-id, non-nested block comments, mode-stack reset, schema-side EmptySpace pin); review-fix round 2 added 5 inline-schema branch tests (char-escape, endsAtLongestMatch, replaceMode, explicit popMode, doubled-delim single-occurrence close), tightened EXPECT_GE → EXPECT_EQ on 3 tests with full-token breakdowns, pinned position invariants on NestedBlockComments and ModeStackResets, and added a builder-side `TokenFlags_PropagateToBuilderLeafFlags` test that pins CR1's effectiveFlags wiring (the test uses an inline schema where the global meaning carries no flags so the OR-merge must depend on `tok.flags` alone). 623 cases / 29 suites, 100% pass. `90c7f75` (initial) + `8612486` (review-fix round 2, 24 items addressed). Squash-merged into `fabe7b4` via PR #5. |
| TZ3 | ✅ done | All-language end-to-end. Every tree-building E2E test in `tests/core/test_c_subset.cpp` and `tests/core/test_tsql_subset.cpp` flipped from hand-tokenization (`at()`/`atNth()` substring-lookup helpers) to live `Tokenizer`-driven `TokenStream`. New `tests/core/e2e_harness.hpp` extracts the canonical `E2EHarness { src, schema, stream, lexerDiags }` + `tokenizeShipped(configName, text)` pattern; `test_tree_end_to_end.cpp` (TZ1's already-flipped target) refactored to use the same shared harness. 3 new body-mode E2E pins added: doubled-delimiter `'it''s'` escape, empty `''` literal, UTF-8 `N'héllo'` body. <br><br>**Builder + tokenizer gaps surfaced and closed by the flip:** (1) **Built-in / body-mode kind trust** — the tokenizer pre-resolves built-in kinds (IntLiteral, StringChar, BracketIdChar) for lexemes the per-`tokens` table doesn't carry. The builder's `resolveMeaning` now synthesizes a `LexemeMeaning` from `tok.schemaKind` when candidates are empty (excludes `Error` so recovery still flows through `P_UnknownToken`). (2) **Body-mode cursor skip** — `TreeBuilder` snapshots every `lexerModes.<m>.defaultToken.kind` at ctor time into `bodyDefaultTokenKinds_`; tokens with those kinds skip the schema-cursor advance (they're off-grammar by construction, so advancing them always desyncs). Distinct from `EmptySpace` skip — body chars remain visible in the AST. (3) **Multi-char operators starting with id-start byte** — T-SQL `N'`, MySQL `B'…'`, Python `r"…"` etc. The tokenizer's identifier branch now non-destructively probes the id-run length, runs a global `longestMatch`, and prefers the longer hit; `if` / `if_foo` / `Nxyz` still tokenize correctly. <br><br>**Review-fix round 1** (13 items addressed): extracted `makeSyntheticMeaning(schema, kind, bodyKinds)` helper and added a debug `assert` that synthesized kinds are either in `bodyDefaultTokenKinds_` or in the predeclared built-in literal set (IntLiteral/FloatLiteral/StringLiteral/CharLiteral/BoolLiteral/NullLiteral). `errorKind` cached on the builder at ctor (eliminates two interner lookups per `pushToken`). `bodyDefaultTokenKinds_` retyped from `unordered_set<uint32_t>` to `unordered_set<SchemaTokenId>` (the loose-int store leaked the `.v` newtype). **New loader pass** `validateBodyDefaultKindsOffGrammar` emits `C_BodyDefaultKindInShape` (0xC029) when a `lexerModes.*.defaultToken.kind` is also referenced from any `shapes/*` — pre-TZ3 such a config would silently never match (cursor-skip is unconditional). **E2EHarness fail-loud**: `tokenizeShipped` now `std::abort`s on schema load failure (no more half-built harnesses); `~E2EHarness` asserts `lexerDiags.empty()` by default with an opt-out `dismissLexerDiags()` for tests that intentionally trip tokenizer diagnostics (used by `BrokenPath_UnknownTokenRecovered` + `BrokenPath_PushErrorRecovered`). **New unit pins** (7 cases): `SynthesizesMeaningForBuiltinLiteralKind`, `SynthesisExcludesErrorKindAndEmitsUnknownToken`, `BodyDefaultKindsEmptyWhenSchemaHasNoBodyModes` (in `test_tree_builder.cpp`); `IdStartProbe_LongerGlobalLexemeBeatsIdRun`, `IdStartProbe_LongerIdRunBeatsShorterGlobalLexeme`, `IdStartProbe_EqualLengthFavorsIdRun`, `IdStartProbe_KeywordPrefixDoesNotSplitIdentifier` (in `test_tokenizer.cpp`). Comment hygiene: stripped all TZ-anchored / pre-TZ migration narration from the test files; stripped 40+ per-token `// SELECT` / `// FROM` WHAT-comments on `advance()` calls; fixed the contradictory "10 + 8 = 18" arithmetic comment in `test_tsql_subset.cpp` by inlining the breakdown into the `EXPECT_EQ` operand. Tokenizer: one-line note that `coreKindForByte` intentionally returns `Operator` for id-start matches (e.g. `N'`). <br><br>625 cases / 29 suites, 100% pass. `fa0df8d` (initial) + `_pending_` (review-fix round 1, 13 items addressed). Squash-merged into `fabe7b4` via PR #5. |
| TZ4 | 🔵 **deferred — pinned follow-up** | **Body-lexed literal VALUE capture** (char/string literals as lowerable values). Surfaced as a real blocker in HIR plan §HR9 (2026-05-27) and recorded in `v2-gap-catalog - tbd.md` row 19. Today a body mode emits its `defaultToken.kind` **per byte**, and those kinds are off-grammar by design (TZ3's `C_BodyDefaultKindInShape` + the builder's `bodyDefaultTokenKinds_` cursor-skip), so the literal body cannot be captured as a CST subtree — it spills into the enclosing frame as trailing off-grammar tokens. Consequence: the HIR lowering engine can resolve a char/string *type* but **not its value**; it fails loud (`H_UnsupportedLoweringForKind`), never miscompiles. **Scope when picked up:** give a body mode a way to coalesce its run into ONE token spanning the literal (preferred — a single `CharLiteral`/`StringLiteral` token with the decoded/raw body), OR emit a distinct close token (`CharEnd`/`StringEnd`) on mode-pop so `operand` can match `[Start, body…, End]` as a subtree. Either unblocks HR9-style value lowering for char AND string with no per-language C++. Must keep the existing comment/EmptySpace body modes working (they intentionally stay off-grammar). |
...
**Goal.** Tokenizer becomes a full mode-stack-aware engine. Resolves `defaultToken` per-char inside non-main modes. Honors `stringStyle.endsAt` / `escapeKind` / `endsAtLongestMatch` / dynamic `tagPattern`. c-subset gains comment modes (closes `v2-gap-catalog - tbd.md` row 3 authoring task).
...
**Goal.** Every shipped `.lang.json` config tokenizes + parses end-to-end via the real tokenizer. Tests that previously hand-tokenized via `TokenSeq` flip to `Tokenizer`. Surfaces any v3-candidate gaps as `v2-gap-catalog - tbd.md` entries.
...
- `v2-gap-catalog - tbd.md` updates — record any new schema gaps the live tokenizer exposes (e.g., a mode-transition footgun, a `defaultToken`-interaction with EmptySpace flagging, a longest-match corner case in `endsAt`).
...
- `00-compiler-implementation-plan - tbd.md` §8 row 5 flips to ✅ done; row 6 flips to ✅ subsumed.
- `04-tokenizer-plan - ok.md` §0 flips to ✅ done.
