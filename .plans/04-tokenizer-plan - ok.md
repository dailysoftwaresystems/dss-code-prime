...
| TZ4 | ✅ **done 2026-05-28** | **Body-lexed literal VALUE capture** (char/string literals as lowerable values). Done via the preferred coalesce route: a per-mode `DefaultTokenSpec.coalesce` flag. When set, the body-mode loop reuses all its escape / doubled-delimiter / endsAt / unterminated logic but emits ONE in-grammar token spanning the body (the single-token model `IntLiteral` uses) + consumes the close delimiter tokenless. A coalesced kind is NOT added to `bodyDefaultTokenKinds` (so it stays in-grammar and `operand` can reference it via `charLiteralExpr`/`stringLiteralExpr`); the loader excludes it from the `C_BodyDefaultKindInShape` set. Per-codepoint emission (comment/EmptySpace bodies) is untouched — those modes leave `coalesce` false and stay off-grammar. Lowering decodes the body via shared `core/types/char_decode.hpp` (C escapes resolved at lowering; fail-loud on unknown escape): char→Char codepoint, string→`Array<Char,N+1>`. Closed gap-catalog row 19 + HIR §HR9's char/string blocker. Commit f08f72d on `feature/hir-1`; tests in test_char_decode.cpp + test_hir_lowering_c_subset.cpp + test_tokenizer.cpp (unterminated coalesced string). |
