...
- **PR2b:** ✅ shipped. `LexemeMeaning.contextual` + `ReservedWordPolicy`. `TreeBuilder` walks `SchemaCursor` on open/close/pushToken; contextual keywords consult expectedSet(cursor) and demote when not expected. Row 2 (typedef contextual identifier) closer to resolved — the mechanism exists; the typedef-symbol-table side is parser work.
- **PR3..PR8:** ⏳ pending.
