...
- **PR6:** ✅ shipped (`<commit pending review>`). New `string_style.{hpp,cpp}` with `EscapeKind` enum (`None`/`Char`/`DoubledDelimiter`) and `StringStyle` POD (escapeKind, escapeChar, endsAt, endsAtLongestMatch, hasMatchedDelimiterTag, multiline, tagPattern). `LexemeMeaning::stringStyleIdx` is an `optional<uint32_t>` into `GrammarSchemaData::stringStyles`; LexemeMeaning stays trivially copyable. Loader validates the full surface: required `escapeKind` + non-empty `endsAt`; `escapeChar` required iff `escapeKind: "char"`; `delimiterTag: "matched"` enables the tag-capture machinery with default `tagPattern: "[A-Za-z0-9_]{0,16}"`; `tagPattern` regex compiled at load via `std::regex` (malformed → `C_InvalidStringStyle`). Keywords reject `stringStyle` (`C_ConflictingField`) — word-shaped tokens can't open delimited strings. Rows 4 + 5 (string + char escape sequences) schema-side resolved. New code: `C_InvalidStringStyle`.
- **PR7..PR8:** ⏳ pending.
