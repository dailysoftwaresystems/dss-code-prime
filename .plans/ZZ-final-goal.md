...
- **Memory — proven-static reclamation, and NO garbage collector.** ★ **REVISED 2026-08-12.** *This bullet formerly read: "Garbage collection — async GC. Differentiator… Async GC participates in the same scheduler as user `async` tasks — collection is itself an `async` task… if it ships, it is the moat for the DSS Code Prime language." It is recorded rather than deleted because the replacement is only legible against it.* **DSS Axis has no collector of any kind** — no GC, no cycle collector, no deferred reclamation. Memory is reclaimed at program points the compiler chose and can name, via a **T0–T4 mechanism lattice** (stack → unique-owner+move → borrow → region → elided compile-time refcount) and a **C0–C3 cycle lattice** that decides cycles **statically, on the SCCs of the owning-edge type graph, before any value exists**. Where it cannot prove safety it **refuses at compile time** rather than leaking at run time. **The moat moved and got wider:** async GC was a better implementation of a category others already occupy; this is strictly stronger than **Rust** (whose `Rc` cycles leak silently and are declared *safe*) and **Swift** (which leaks without annotations), *checks* the acyclicity **Koka/Lean assume**, and — because it costs nothing at run time — makes the language expressible in a **kernel driver or a boot image**, which no GC-bearing language can claim. Owned by [plan 09.5](./09.5-dss-hir-plan.md) §4; language surface in [plan 24](./24-dss-axis-language-reserved-plan%20-%20tbd.md) §2.3.
  - ⚠ **GC left the LANGUAGE, not the ENGINE.** §3 commits to compiling C#, Java, Python and JS, all of which require a real collector. [Plan 21](./21-runtime-reserved-plan%20-%20tbd.md) §2.1 remains live — its trigger simply moved from Axis to the first managed language onboarded.
- **The DSS Native Runtime (DSS-HIR).** An optional tier above HIR that **absorbs platform discrepancies** — threads, async, filesystem, sockets, timers, synchronization, workers, gui — which **any** source language may opt into, not just Axis. `workers` is the one that proves the thesis: it presents one abstraction over real OS threads *and* over platforms that forbid them (WASM, mobile background limits). Owned by [plan 09.5](./09.5-dss-hir-plan.md).
...
- The runtime (the DSS Native Runtime — scheduler, GPU, networking; **no GC**, see §4).
