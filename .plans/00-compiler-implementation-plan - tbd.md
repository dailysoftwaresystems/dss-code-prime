...
| **IR** — HIR + MIR + LIR (rev 2; replaces single-IR plan) | 🟡 in progress — HIR HR1–HR9 ✅ (2026-05-26..27, `feature/hir-1`; HR8 = config-driven CST→HIR lowering engine proven on c-subset; HR9 = toy enriched into a typed language + arrays un-deferred end-to-end; char/string literal VALUES blocked → plan 04 TZ4). Production-readiness §3 (G-301a/b/c + G-302..G-310). Owned by [`09-hir-plan`](./09-hir-plan%20-%20tbd.md) (HIR — the pivot; HR1–HR9 done, HR10–HR11 pending) + [`12-mir-lir-plan`](./12-mir-lir-plan%20-%20tbd.md) (MIR SSA-over-CFG with structured-CF markers + LIR per-target ISA; ⏳ planned). |
...
| 8 | [09](./09-hir-plan%20-%20tbd.md) HR1–HR11 | HIR — language-neutral structured typed pivot; CST→HIR lowering per shipped language | step 7 | 🟡 HR1–HR9 ✅ (2026-05-26..27, `feature/hir-1`; HR8 = config-driven lowering engine on c-subset; HR9 = toy enriched into a typed language + arrays un-deferred end-to-end, char/string literal VALUES blocked on a tokenizer change pinned as plan 04 TZ4 / gap row 19); HR10–HR11 pending |
...
|---|---|---|---|---|---|
| D1 | **Pointer *type-level* modelling** + `&` addr-of (c-subset) | ✅ **array type-level modelling closed in HR9 (2026-05-27):** `int a[10]` now resolves + lowers to `Array<I32,10>` via a config-driven `DeclarationRule.arraySuffix` declarator-suffix descriptor + semantic-time constant-length eval (fail-loud on non-constant/out-of-range). **Remaining (pointer only):** c-subset uses C declarator syntax (`int *p`), not a named type-constructor rule, so a `typeShapes`/grammar round is needed for typed pointers; the expression-side `*p` (deref) already works. Not codegen-blocking for the current corpus. | feature/config | future c-subset grammar+`typeShapes` round; [07](./07-production-readiness-plan%20-%20tbd.md) G-105 | a corpus program needs typed pointers, or HIR lowering of pointer types |
...
| D7 | **Ternary `? :` (mixfix)** | The v1+v2 operator-arity enum (`Prefix`/`Infix`/`Postfix`) can't model a mixfix operator; the existing `GroupedPostfix` machinery models an opener+closer (`(`…`)`) but not a three-position `? … : …`. Needs a new `Ternary`/`Mixfix` arity (a key-packing change in `OperatorTable`) + Pratt-walker support + a schema field — a focused schema-expressiveness round (M), stays config-driven. NOT blocked by any downstream phase. | blocked (schema) | a dedicated "schema v3 — mixfix operators" round; [v2-gap-catalog](./v2-gap-catalog%20-%20tbd.md) row 17 | a shipped language needs ternary (its own PR, do before that language ships) |
