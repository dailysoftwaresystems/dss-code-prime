...
| 13.7 cycle 5 | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT4 c3 SSA copy-prop | SSA copy-prop on post-Mem2Reg IR. Phi with all-equal incomings collapses to common value; chained copies (`a = b; c = a; d = c`) shorten via `MirRebuildPolicy::substituteOperand`. First fixed-point pass → triggers `D-OPT1-PASS-RUN-MAX-ITER` + `D-OPT-FIXED-POINT-LOOP` + `D-OPT-COPYPROP-BLOCK-CURSOR`. Activates `D-OPT1-COPY-PROP-JOIN-PIN` runtime arm on `copy_prop_across_join`. | step 13.7 cycle 4 ✅ | ⏳ planned |
| 13.7 cycle 6+ | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT5+ | CSE/GVN (OPT5) → SimplifyCFG → LICM (OPT6) → tier-neutral `scalar_eval` consolidation → cost/machine/peephole/pipeline as JSON → OPT10 autotuner. | step 13.7 cycle 5 ✅ | ⏳ planned |
