...
| 13.6 | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT1 | **OPT1 — const-fold + DCE + copy-prop + dominator-tree + liveness** (the v1-mandatory passes). Multi-tier optimizer per plan 22 (HIR/MIR/LIR/per-target), source/target/linker-agnostic, config-driven. Reuses const-eval's scalar core (tier-neutral `scalar_eval`, OPT2). `X_*` diag at 0x2xxx. **Differential-verification methodology**: run each of the 14 step-13.5 corpus examples BEFORE and AFTER each optimizer pass; assert exit codes + captured stdout match byte-for-byte. Any rewrite that shifts a runnable's observable behavior IS a real bug — the corpus pays its dividends here. **READY TO OPEN 2026-06-03** (gate met: file emission ✅ 2026-06-02; corpus ✅ 2026-06-03). | step 13.5 ✅ | ⏳ **READY-TO-OPEN — next step in sequence** |
