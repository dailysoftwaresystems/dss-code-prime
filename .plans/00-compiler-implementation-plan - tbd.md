...
| **Compilation unit** (sub-plan CU1–CU6) — multi-file `CompilationUnit` + per-language `ImportResolver` | 🟢 **CU1 + CU2 done.** CU1 type/builder; CU2 multi-file `addFile`/`addInMemory` + `D_*` codes + lexer+parser diagnostics unified in the Tree (cross-plan amendments to 01/05). **G-110 ✅ resolved.** CU3–CU4 ⏳ pending — phase #7.5. See [`08-compilation-unit-plan - tbd.md`](./08-compilation-unit-plan - tbd.md). Bridges parser (per-file `Tree`) and semantic (cross-file symbol table). |
...
| **3** | [08](./08-compilation-unit-plan%20-%20tbd.md) CU3 | Cross-tree `SymbolId` shape + `CrossTreeRef` + SH3-analog cross-CU `NodeId` guard | step 2 ✅ | ⏭️ **next — unblocked** |
