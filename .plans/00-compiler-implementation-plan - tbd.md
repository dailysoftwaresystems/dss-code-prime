...
| **Total ctest cases** | **625 cases across 29 suites, 100% pass** (post TZ3 + TZ3 review-fix round 1) |
...
| **Parser** (sub-plan PA0–PA5b, 7 PRs) — substrate `SchemaWalker` extraction + RD driver + Pratt walker + recovery + corpus + LSP server + LSP semantic-stub handlers | 🔵 **next** — phase #7. See [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md). PA5a/PA5b ship LSP with diagnostics only; semantic-powered LSP methods land post-phase #8 in a spun-off `09-lsp-plan`. |
