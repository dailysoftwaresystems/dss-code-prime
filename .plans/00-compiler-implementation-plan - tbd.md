...
| D5 | **c-subset structs / unions / enums** | 🟡 **PULLED FORWARD 2026-05-28** (before ML2) to make ML2's HIR→MIR lowering honor "no real-blocker deferrals" on `MemberAccess`/`ConstructAggregate`/`TypeDecl`. **Kitchen-sink scope** confirmed: core structs + typedef'd + anonymous structs + designated initializers + unions + enums + first-class MIR aggregates (ExtractValue/InsertValue). **6-PR sub-arc**: D5.1 core structs → D5.2 typedef'd/anon → D5.3 designated init → D5.4 unions → D5.5 enums → D5.6/ML2-prelude MIR aggregate opcodes → then ML2. **D5.1 in progress**: foundation commit `251eca0` (additive config vocab + `SymbolRecord::fieldIndex`/`structScope`) + engine substrate (Pass 1 ordinal-stamping + `structScope` recording, Pass 1.5 `structType` composition, Pass 2 member-access resolution via new `compositeScopeByType` map, new `memberAccesses` schema facet + `MemberAccessRule` + `S_NotAPointer`/`S_NotAComposite` codes + cross-field loader validation) — committing this round; schema-surface in `c-subset.lang.json` (struct keyword, `.` / `->` postfix design hits operator-table architectural decision), HIR lowering, verifier rule, full test corpus still owed. | feature (multi-PR) | the 6-PR D5 sub-arc (D5.1 in progress) | maximally honest c-subset for ML2 |
