...
| 22 | tokenizer engine + [11](./11-ffi-plan%20-%20tbd.md) FF11 | **Context-sensitive per-mode lexing** (`D-TOKENIZER-CONTEXT-SENSITIVE-LEXING`) — wire `lookupLexemeInMode` into the scanner + parse per-mode token overrides; agnostic engine capability, FULL cross-language regression gate + synthetic-grammar test. **THEN FF11** (C-faithful shipped headers: `#include <h>` angle form on a config `shippedLibDirs` system path → merged via existing include-following → existing FFI synthesis; inline-extern KEPT). User §B 2026-06-05 chose Option A (tokenizer investment) over a localized grammar workaround → 2-cycle path. Spec in plan-11 FF11 entry. | cross-CU ✅ (cycles 15–19) | 🔨 **next-up** (tokenizer cycle first) |
