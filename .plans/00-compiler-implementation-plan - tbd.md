...
| **Total ctest cases** | **112 ctest suites, 100% pass** post AS6 + LK4 substrate + post-audit substrate hoist (2026-05-29). Growth trace from 56 suites at SE1-pre baseline through HIR (HR1–HR11) → MIR (ML1–ML4) → LIR (ML5 cycle 1–3e) → ML6 (liveness + linear-scan + rewrite) → ML7 (callconv) → ML8 (`.dsslir` emitter + parser + round-trip) → AS1–AS6 (assembler) → LK4 (linker substrate). |
...
| 11 | ⏳ **in flight** — sub-plan [`14-linker-plan`](./14-linker-plan%20-%20tbd.md) | `linker` | In-tree linker + JSON-configured object formats | assembler, ffi, **artifactProfile** | **Hermetic** (replaces system-linker integration). **Config-driven object formats** (rev 3): `src/dss-config/object-formats/*.format.json` for ELF / PE / Mach-O / WASM / SPIR-V + ONE format-blind engine + TLS lowering + dynamic imports. **LK4 substrate ✅ landed 2026-05-29** (`src/link/object_format_schema.{hpp,cpp,_json.cpp}` + `linker.{hpp,cpp}` + `K_*` diag at 0x8xxx). Ten PRs total (LK1–LK10); LK1–LK3 + LK5–LK10 ⏳. WASM-skeleton (LK8) + SPIR-V-skeleton (LK9) for v1.x backends. |
