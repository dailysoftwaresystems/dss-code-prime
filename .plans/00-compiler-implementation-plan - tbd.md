...
| 13.7 cycle 10q | [`13`](./13-assembler-plan%20-%20tbd.md) Arc A continuation — x86 idiv/CDQ/CQO encoding + MIR→LIR Div arm + regalloc consumer | Plan: declare idiv `REX.W F7 /7` + CDQ `99` + CQO `REX.W 99` in x86_64.target.json with `implicitRegisters: {inputs:[rax,rdx], outputs:[rax,rdx], clobbered:[rdx]}` on idiv. MIR→LIR Div arm emits CDQ/CQO (signed) or xor-RDX (unsigned) pre-divide. Regalloc consumes opcode `implicitRegisters` to pin RAX/RDX. Tests: byte-pin encoding + MIR→LIR signed/unsigned + regalloc constraint propagation. | step 13.7 cycle 10p ✅ | ⏳ next cycle (substrate ready) |
