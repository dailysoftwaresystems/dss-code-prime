...
| **V3-3** → **promoted into the ARM64 arc** | Mach-O exec-verify (Apple Silicon) | 🟢 **ACTIVE 2026-06-08 (user-directed) — the macOS-runner trigger has FIRED**: `macos-latest` CI = Apple Silicon (arm64), so a `macho64-arm64-darwin-exec` corpus runs natively there. Folded into the ARM64 arc (see 13X.2) as the macOS-Mach-O platform alongside Linux-ELF; completes 3-OS execution verification. | 13X.2 | macos-latest=arm64 (FIRED) | 🟢 **ACTIVE** |
