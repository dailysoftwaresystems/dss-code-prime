...
> **v1 production target.** End-to-end binaries for every language in `src/source-config/languages/` (`toy`, `c-subset`, `tsql-subset`) on **{Windows, Linux, macOS} × {x86_64, ARM64}**, gated by a new [`artifactProfile`](./06-artifact-profile-plan - tbd.md) compatibility mechanism. **See [`07-production-readiness-plan - tbd.md`](./07-production-readiness-plan - tbd.md) for the master gap catalog** (127 numbered gaps across frontend / semantic / IR / optimizer / codegen / driver / CI / corpus / perf / docs / process).
>
...
> - **[`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md)** — opens parent phase #7 (`analysis-syntactic`): substrate refactor + schema-driven RD driver + Pratt walker + recovery + corpus + LSP scaffolding. **Seven PRs** (PA0 `SchemaWalker` extraction ✅, PA1 RD driver ✅, PA2 Pratt walker ✅, PA3 recovery + diagnostic UX ✅, PA4 corpus stress, PA5a LSP server + diagnostics, PA5b LSP semantic-stub method handlers + golden-replay harness). LSP spins off to `09-lsp-plan` post-PA5b once scope outgrows parser-plan. Closes `v2-gap-catalog - tbd.md` row 1 and production-readiness G-115 (diagnostic UX) end-to-end. **🟢 PA0 + PA1 + PA2 + PA3 ✅ done; PA4 next.**
...
> - **[`08-compilation-unit-plan - tbd.md`](./08-compilation-unit-plan - tbd.md)** — opens **phase #7.5** (`compilation-unit-model`): bridges parser (per-file `Tree`) and semantic (cross-file symbol table) via a `CompilationUnit` type + driver-level aggregator + per-language import resolver. Four PRs (CU1 CU type, CU2 multi-file driver, CU3 cross-tree symbol shape, CU4 import resolver). Closes production-readiness G-110 (multi-file translation units) + G-111 (imports/modules). **⏳ planned**, gated by parser PA1 shipping; precedes phase #8 (`analysis-semantic`).
>
...
| **Total ctest cases** | **36 ctest suites, 100% pass** post PA4 + 5-agent review + fix-everything round. PA4 adds `tests/analysis/syntactic/test_corpus.cpp` (3 corpus tests covering c-subset / tsql-subset / toy) + the gap-closure smoke tests pinning function calls, array indexing, prefix deref, postfix inc, empty-args, in-bracket precedence climb, and missing-closer recovery. |
...
| Tokenizer (sub-plan TZ1–TZ3 + review-fix round 1) — `SourceReader`, `Tokenizer`, `TokenStream`, `LexerModeStack`, `stringStyle` body handling, comment modes, `C_BodyDefaultKindInShape` loader guard, synthesis-allowlist assertion, fail-loud `E2EHarness` | ✅ **done** — phase #5 closed; phase #6 (`analysis-lexical`) ✅ subsumed |
| **Parser** (sub-plan PA0–PA5b + PA5a-prep + PA-Walker-LeftRec) — substrate `SchemaWalker` ✅; iterative RD driver ✅; Pratt walker ✅; recovery + diagnostic UX ✅; corpus stress ✅; c-subset readiness shapes ✅; postfix-chain walker redesign ✅; LSP server + LSP semantic-stub handlers ⏳ | 🟢 **PA0–PA4 + PA5a-prep + PA-Walker-LeftRec ✅ done; PA5a next** — phase #7. PA0–PA3 as before; PA4 onboarded real corpus programs (c-subset / tsql-subset / toy) and closed v2-gap-catalog rows 6/7/8/12 (postfix call/index/inc/dec, prefix deref). PA4 follow-ups (PA4-F1..F8) landed substrate refactors (`OperatorTable::Entry → optional<GroupedPostfix>`, `bodyDefaultTokenKinds` dedup), additional test coverage, and corpus golden harness. PA5a-prep closed v2-gap-catalog rows 12-declarator (`int a[10];`), 16 (compound assignment), 29 (`extern` declaration). PA-Walker-LeftRec added the `TreeBuilder::wrapLastChildInFrame` substrate primitive + rewrote the Pratt walker's postfix branch — `f(a)[i]`, `a[i][j]`, `f()()` chains now produce correctly-nested `postfixExpr` wraps. 36 ctest suites green. PA5a now ships the LSP server skeleton + diagnostics; PA5b adds semantic-stub LSP method handlers + golden replay; semantic-powered LSP methods land post-phase #8 in a spun-off `09-lsp-plan`. |
| **Compilation unit** (sub-plan CU1–CU4) — multi-file `CompilationUnit` + per-language `ImportResolver` | ⏳ pending — phase #7.5. See [`08-compilation-unit-plan - tbd.md`](./08-compilation-unit-plan - tbd.md). Bridges parser (per-file `Tree`) and semantic (cross-file symbol table). Closes G-110 + G-111. |
| **Semantic** — symbol table, scope resolution, type checking | ⏳ pending (phase #8). Consumes `CompilationUnit` from phase 7.5. See production-readiness §2 (G-201..G-212). Symbol-table shape is now CU-scoped — design decision required before PR1. |
| **IR** — design + verifier + lowering + text format | ⏳ pending (phase #9). Production-readiness §3 (G-301..G-308). **G-301 IR design = single most consequential decision still open.** Default: SSA over a CFG. |
...
| **Codegen** — ELF + PE + Mach-O × x86_64 + ARM64 + per-platform ABI + linker integration | ⏳ pending (phase #11). **Largest single chunk of v1 work.** Production-readiness §5 (G-501..G-543). Current state: Windows PE demo only, not IR-driven. |
...
| **Real-world corpus** — `tests/corpus/{c-subset,tsql-subset,toy}/` programs that exercise the full grammar | ⏳ pending. Production-readiness §7.3 (G-730..G-734). Currently every test is library-level; nothing actually runs a compiled binary. |
...
| 7  | 🔵 **PA0–PA3 ✅ done; PA4 next** — sub-plan [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md) | `analysis-syntactic` | Implement syntactic analysis | analysis-lexical | ✅ `SchemaWalker` substrate extraction (PA0, `c764c2a`); ✅ schema-driven RD driver with try-each-branch speculation (PA1); ✅ Pratt walker for operator precedence (PA2, closes v2-gap-catalog row 1); ✅ panic-mode recovery + `followSetOf` + clang-style diagnostic rendering (PA3, closes G-115 end-to-end). ⏳ corpus stress (PA4) + LSP server with diagnostics (PA5a) + LSP semantic-stub handlers + golden replay (PA5b). **v1 must deliver:** every test in `tests/core/test_*_subset.cpp` flips to `Parser::parse()`; `tests/corpus/{toy,c-subset,tsql-subset}/` programs parse cleanly; clang-quality diagnostic rendering; LSP server publishes diagnostics in real editors. See parser-plan §3 acceptance criteria. |
| 7.5 | ⏳ **new** — sub-plan [`08-compilation-unit-plan - tbd.md`](./08-compilation-unit-plan - tbd.md) | `compilation-unit-model` | Multi-file translation units + import resolution | analysis-syntactic | Bridges parser (per-file `Tree`) and semantic (cross-file symbol table). New `CompilationUnit` type + `UnitBuilder` aggregator + per-language `ImportResolver`. Four PRs (CU1 type, CU2 multi-file driver, CU3 cross-tree `SymbolId` shape, CU4 import resolver). Closes production-readiness G-110 + G-111. **v1 must deliver:** every shipped language has at least one multi-file integration test; semantic phase consumes `CompilationUnit` instead of bare `Tree`; codegen lowers per-CU. Gates phase #8. |
| 8  | ⏳ pending | `analysis-semantic` | Implement semantic analysis | compilation-unit-model | Symbol table + scope resolution + type checking + const-correctness + typedef resolution. **Consumes `CompilationUnit`** (from phase 7.5) — symbol table spans every Tree in the CU + cross-file refs are pre-resolved. Populates `NodeAttribute<TypeInfo>` / `NodeAttribute<SymbolId>` over each CST. New `S_*` diagnostic namespace. **v1 must deliver:** valid multi-file c-subset programs type-check cleanly; type-mismatch / undeclared-identifier / redeclaration / const-violation each emit an actionable `S_*` diagnostic; tsql-subset column references resolve against `CREATE TABLE` declarations within the same CU. Production-readiness §2 (G-201..G-212). **G-201 (symbol table design — now CU-scoped) + G-203 (type representation)** are the two design decisions still open — make them before PR1. |
| 9  | ⏳ pending | `gen-intermediate` | Implement IR generation | analysis-semantic | IR design + verifier + lowering pass + text format. **v1 must deliver:** SSA-over-CFG IR (default; revisit if a constraint demands otherwise), round-trippable text format, verifier that rejects malformed SSA, per-language lowering pass that walks CST + attribute tables and emits IR. Profile-driven knobs honor `artifactProfile` (cli/lib emit different entry-block patterns; script/sproc skip IR). New `I_*` diagnostic namespace. Production-readiness §3 (G-301..G-308). **G-301 IR design = single most consequential decision still open.** |
| 10 | ⏳ pending | `gen-optimizer` | Implement IR optimizer | gen-intermediate | **v1 mandatory passes:** constant folding (G-401), block-level DCE (G-402), copy propagation (G-403), dominator-tree analysis (G-408), liveness analysis (G-409). **Deferred to v1.1:** CSE, LICM, inlining, strength reduction. Production-readiness §4 (G-401..G-410). |
| 11 | 🟦 partial (Windows PE demo only) | `gen-link` | Implement code emission & linking | gen-optimizer, **artifactProfile** | **v1 must deliver:** ELF + PE + Mach-O emission, x86_64 + ARM64 instruction selection + encoding, linear-scan register allocation, per-platform ABI conformance (SysV AMD64 / Microsoft x64 / AAPCS64 / Microsoft ARM64), system-linker integration with structured `D_*` error capture, DWARF 5 (ELF + Mach-O) + PDB (PE) debug info, per-`artifactProfile` codegen variants (cli/gui/lib/staticlib for native; script/sproc for text). **Largest single chunk of v1 work.** Current state: standalone Windows PE generator (`TargetWindowsX86_64::generateSimpleGui`) NOT IR-driven. Production-readiness §5 (G-501..G-543). |
| 12 | 🟦 skeleton only | `program-api` | Implement program API & driver | core-types, gen-link, **artifactProfile** | **v1 must deliver:** `dss-code-prime build my-project.dss-project.json` CLI entry point; `.dss-project.json` loader (per [`06-artifact-profile-plan - tbd.md`](./06-artifact-profile-plan - tbd.md) §2.2); driver enforces `project.artifactProfile ∈ language.artifactProfiles` (new `D_*` diagnostic namespace); `CompilationContext` carries resolved profile through every phase; diagnostic-rendering layer produces clang-quality output; deterministic byte-identical output for unchanged input. Production-readiness §6 (G-601..G-609). |
| 12.5 | 🔵 **new** — sub-plan [`06-artifact-profile-plan - tbd.md`](./06-artifact-profile-plan - tbd.md) | `artifact-profile` | `artifactProfile` mechanism end-to-end | core-types | New cross-cutting concept: language config declares supported profiles (`cli`/`gui`/`lib`/`staticlib`/`script`/`sproc`); project config picks one; driver enforces; codegen reads. Four PRs (AP1 schema + loader, AP2 project config + driver enforcement, AP3 `CompilationContext` plumbing, AP4 onboard shipped languages). **v1 must deliver:** every shipped `.lang.json` declares `artifactProfiles`; mismatched project emits `D_ArtifactProfileNotSupported` at config-load; codegen backends key off the resolved profile (not the language name). Gates phase #11 acceptance. |
| 13 | ⏳ pending | `targets-expand` | Expand target support | gen-link | Post-v1: iOS / Android / WASM / kernel modules / embedded. The v1 cross-platform matrix ({Win, Linux, macOS} × {x86_64, ARM64}) is delivered in phase #11 directly. |
...
- ~~**Grammar format**: BNF chosen for readability; PEG is an alternative if ambiguity becomes an issue.~~ **Resolved** by [01-tree-node-model-plan - ok.md §5.12](./01-tree-node-model-plan - ok.md): the schema is a tree of expected shapes (`sequence` / `alt` / `optional` / `repeat`) with no backtracking on `alt`. PEG-style speculative lookahead is a deferred escape hatch (sub-plan §9 item 11).
- ~~**Error recovery**: Parser will implement panic-mode recovery (sync to statement boundaries). More sophisticated recovery later.~~ **Resolved** by [01-tree-node-model-plan - ok.md §5.15](./01-tree-node-model-plan - ok.md): structured recovery per failure mode, forward-progress watchdog, `HasError` propagation, structured `ParseDiagnostic`s with `expected`/`actual`/`scopeStack`/`related`. [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md) PA3 implements the strategy on top.
- ~~**Cross-platform scope.**~~ **Resolved 2026-05-21:** v1 production matrix is **{Windows, Linux, macOS} × {x86_64, ARM64}** — all 6 combos mandatory.
- ~~**v1 language scope.**~~ **Resolved 2026-05-21:** v1 ships end-to-end binaries for every language in `src/source-config/languages/` (`toy`, `c-subset`, `tsql-subset`). New language onboarding (full C99, C#, etc.) is post-v1 work under a future `languages-onboarding-plan.md`.
- ~~**Output target distinction.**~~ **Resolved 2026-05-21:** new [`artifactProfile`](./06-artifact-profile-plan - tbd.md) mechanism. Language config declares supported profiles (`cli`/`gui`/`lib`/`staticlib`/`script`/`sproc`); project config picks one; driver enforces compatibility; codegen reads to pick entry-point + subsystem + output extension.

### Still open — decide before the phase ships

- **G-201 / G-203: Semantic phase data structures.** Symbol table (stack-of-hashmaps + `SymbolId` interner is the default) and type representation (`TypeKind` enum + `TypeId` interner is the default). Decide before phase #8 PR1. See [`07-production-readiness-plan - tbd.md`](./07-production-readiness-plan - tbd.md) §2.
- **G-301: IR design.** SSA over a CFG is the default; revisit if a constraint demands otherwise. Decide before phase #9 PR1. See [`07-production-readiness-plan - tbd.md`](./07-production-readiness-plan - tbd.md) §3.
- **G-110 / G-111: Multi-file translation units + imports/modules.** Current model is one file → one tree. Real production C-subset programs span multiple files (`#include`); T-SQL has `USE database;` cross-script references. Affects parser + semantic + IR. Decide scope before phase #8.
- **G-114: Recovery quality benchmark.** Clang-level? tsc-level? Pin the bar with stakeholders before parser-plan PA3 ships.
- **G-762: Milestone calendar.** v1 scope is now defined; v1 dates are not. Default (negotiable): parser PA1+PA2 Q3, semantic + IR Q4, first c-subset → ELF binary by year-end.
- **G-760: PR review tiering.** 5-agent review for substrate; 2-agent (code + tests) for feature; skip for docs. Codify in CONTRIBUTING when it lands (G-743).

### Long-running design notes

- **Standard library**: Languages may need runtime support (memory allocation, I/O). Out of v1 — separate "runtime" module post-v1.
- **LLVM backend**: An alternative to hand-written target emitters. Could be added as an optional backend in future. Default for v1 is hand-written.
- **Incremental compilation**: Not in v1 scope. Full recompilation per invocation. The tree's arena + immutability design *permits* it later.
- **Register allocation**: Linear-scan for v1 (simpler, faster compile times, "good enough"). Graph-coloring is v1.1+ if perf demands it.
- **IR level**: ~~Starting with simple three-address code; may evolve to SSA.~~ Superseded — default is SSA over a CFG (see G-301 above); three-address code reconsidered only if SSA proves too complex for the v1 mandatory optimizer subset.
- **Source-to-target language translation (future).** The compiler today targets two output kinds — compiled binaries (ELF/PE/Mach-O via codegen) and "script" / "sproc" textual deployments (tsql-subset's `script` / `sproc` artifactProfiles emit a `.sql` file directly). A third output kind is **translation to another source language** (e.g. compile c-subset → JavaScript, or compile a DSL → another DSL). This requires:
  1. **A target schema** — the destination language's `.lang.json`. Loaded the same way as a source schema (shipped name OR explicit `--schema-dir` per the PA5a precedent), making language definitions symmetric inputs/outputs of the compiler.
  2. **A schema map** — a new artifact (target `.map.json`?) declaring the translation rules from source rule names → target rule shapes. The map drives a CST→CST transformation pass (analog of codegen's CST→IR→native pass).
  3. **A new artifact profile kind** (`"transpile"`?) that selects the schema-map-driven backend instead of the IR-driven one. Aligns with [`06-artifact-profile-plan - tbd.md`](./06-artifact-profile-plan%20-%20tbd.md)'s existing profile-dispatched backend split.
  Out of v1 scope; track here so the architectural anchors (LSP's schema resolver, the artifact-profile mechanism, the IR's choice of "what does compile mean for a SQL script" — see v2-gap-catalog row 27) all leave room for this in their designs. A dedicated `10-source-translation-plan.md` will own the detail when the trigger lands (likely first user: c-subset → JS for a Web target).
