...
| 13.3b | LIR/asm: local-int codegen | ⏳ **NEW BLOCKER for hello_writefile.** c-subset's grammar accepts `int x;` (local declaration in function body), but the LIR/asm pipeline today has NO alloca-to-lea pre-encoding pass — `MnemonicSlot::Alloca` is emitted by `mir_to_lir` but `x86_64.target.json` declares no `encoding` field for it (the schema's `$comment` says "AS1 turns this into a frame-relative `lea` once the frame layout is known" — but the AS1 conversion pass has never landed). Plus `neg` opcode (unary minus on `-11` for `STD_OUTPUT_HANDLE`) is similarly un-encoded today. **Anchored D-CSUBSET-LOCAL-INT-CODEGEN**: extend frame-layout with a `localStackSlots` array; pre-encoding pass rewrites `Alloca` → `Lea reg, [rbp - localOffset]` using the computed slot offsets; add `neg` encoding row to `x86_64.target.json` (`F7 /3` form, 3 bytes for r/m64). Existing tests don't exercise local int + bodies (corpus tests stop at parsing; examples use only globals + params) so the gap was latent until hello_writefile surfaced it. | step 13.3a ✅ | ⏳ **NEXT** — direct blocker for hello_writefile |
| 13.3 | examples + plan 14 | ★ **hello_writefile — CRT-free hermetic print.** New `examples/c-subset/hello_writefile/main.c` using `kernel32.GetStdHandle` + `kernel32.WriteFile` (5 args; arg-5 = `lpOverlapped` on the stack) + `kernel32.ExitProcess`. Execution-verified by `examples_runner` (Windows host) — same byte-for-byte exitCode + capturedStdout discipline as hello_puts. Closes D-LK10-KERNEL32-WRITE-PATH. **First execution-verified DSS binary that prints with ZERO msvcrt dependency** — more honest than hello_puts (which leans on msvcrt's DllMain self-init). | steps 13.1, 13.2 ✅, 13.3a ✅, 13.3b | ⏳ blocked on 13.3b |
| 13.4 | [12](./12-mir-lir-plan%20-%20ok.md) variadic ABI + c-subset `...` grammar | Variadic-call ABI substrate: SysV `%al = SSE-arg-count` rule (callee reads `al` for vararg-prolog dispatch), Win64 vararg-double-spill (every float vararg duplicated in the matching GPR slot). Per-cc lowering rules in callconv pass. C-subset grammar extension: `...` parameter + `va_list` / `va_arg` / `va_start` / `va_end` types (per `src/dss-config/ffi-headers/libc/stdio.h` comment "No varargs awaits a c-subset grammar extension"). Unlocks `printf` with format specifiers AND any libc varargs surface (`scanf`, `fprintf`, etc.). | step 13.1 (NOT step 13.3 — independent of WriteFile) | ⏳ unblocks `printf` |
| 13.5 | examples + corpus expansion | **Non-trivial host-runnable c-subset programs** — loops (while/for), nested conditionals, multi-function call graphs, local arrays + alloca + Array→Ptr decay (closes D-LK4-RODATA-PRODUCER-LOCAL-ARRAY-DECAY), struct field access returning values, char→int promotion (closes D-LANG-NUMERIC-PROMOTION-CONFIG). Each example execution-verified by `examples_runner`. **Builds the corpus the optimizer will differentially verify against** (you can't measure const-fold without a program where const-fold matters). Each example surfaces the next bug via the methodology — this is where the v1 c-subset feature-completeness gaps get found + closed. | step 13.4 | ⏳ corpus expansion |
| 13.6 | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT1 | **OPT1 — const-fold + DCE + copy-prop + dominator-tree + liveness** (the v1-mandatory passes). Multi-tier optimizer per plan 22 (HIR/MIR/LIR/per-target), source/target/linker-agnostic, config-driven. Reuses const-eval's scalar core (tier-neutral `scalar_eval`, OPT2). `X_*` diag at 0x2xxx. **Deliberately last on the corpus path**: optimizing tiny demos is pointless; meaningful optimization requires the step-13.5 corpus to differentially verify against. | step 13.5 | ⏳ planned (plan 22, rev 1) — was step 13.5; renumbered to land AFTER corpus expansion per 2026-06-02 strategic sequencing |
| 13.7 | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT2–OPT10 | Tier-neutral `scalar_eval` + dominator-tree pass infrastructure + heuristics-as-data (cost/machine/peephole/pipeline as JSON → OPT10 autotuner). Linker-agnostic via format-neutral `SymbolBinding`/`SymbolVisibility` threaded onto MIR. | step 13.6 | ⏳ planned (plan 22, rev 1) |
...
>
> - **Stage 2 — c-subset corpus expansion (strict sequence, 2026-06-02+):** 13 → **13.1** (ML7-2.2 stack args) → **13.2** (c-subset `void*`) → **13.3** (hello_writefile — CRT-free hermetic print) → **13.4** (variadic ABI + c-subset `...` grammar) → **13.5** (non-trivial host-runnable corpus expansion) → **13.6** (OPT1 const-fold + DCE + copy-prop) → **13.7** (OPT2–OPT10 multi-tier optimizer). Steps 13.1–13.5 build the corpus the optimizer differentially verifies against; sequencing optimizer after corpus is deliberate ("runs everywhere" before "runs fast" — but "runs USEFULLY" before either).
