...
| 13.3 | examples + plan 14 | ★ **NEXT.** **hello_writefile — CRT-free hermetic print.** New `examples/c-subset/hello_writefile/main.c` using `kernel32.GetStdHandle` + `kernel32.WriteFile` (5 args; arg-5 = `lpOverlapped` on the stack) + `kernel32.ExitProcess`. Execution-verified by `examples_runner` (Windows host) — same byte-for-byte exitCode + capturedStdout discipline as hello_puts. Closes D-LK10-KERNEL32-WRITE-PATH. **First execution-verified DSS binary that prints with ZERO msvcrt dependency** — more honest than hello_puts (which leans on msvcrt's DllMain self-init). | steps 13.1, 13.2 ✅ | ⏳ **NEXT** — the ML7-2.2 + 13.2 payoff |
