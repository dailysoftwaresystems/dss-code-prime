...
| **Semantic** — symbol table, scope resolution, type checking | ⏭️ **next; SE1 unblocked** (phase #8; sub-plan [`08.6-semantic-plan`](./08.6-semantic-plan%20-%20tbd.md)). Consumes `CompilationUnit` (7.5) + `TypeLattice` (8.5); lands on the fully language-agnostic core proven by 08.55. **Config-driven** (`semantics` block, schema v4; one language-agnostic engine — no per-language C++, per thesis decision #4). G-201..G-212. |
...
| **7** | [08.6](./08.6-semantic-plan%20-%20tbd.md) SE1–SE7 | Symbol table + scope resolution + type checking — **config-driven** (`semantics` block, schema v4; one language-agnostic engine). SE1 toy engine+config → SE2 c-subset config → SE3 tsql config → SE4–SE7 | step 7a ✅ | ⏭️ **next — unblocked** |
