...
> **v1 production target locked.** End-to-end binaries for `toy` / `c-subset` / `tsql-subset` on **{Windows, Linux, macOS} × {x86_64, ARM64}**. Frontend (tokenizer + parser + LSP) ✅; CU + semantic + HIR + MIR + LIR + assembler + linker + FF6 runtime print ✅; OPT1–OPT6 v1 + v1.x mandatory subset ✅ (OPT5+ c3 closed 2026-06-03; v1 ConstFold + DCE done; v1.x Mem2Reg + CopyProp + CSE + SimplifyCFG + LICM all live). Driver (`compileProject`) + `artifactProfile` mechanism + debug info + cross-host ARM64 / Linux / macOS Stage 2 still ⏳ — see [`07-production-readiness-plan - tbd.md`](./07-production-readiness-plan - tbd.md) for the 127-item gap catalog.
...
| **Optimizer** — multi-tier, config-driven (source/target/linker-agnostic): HIR-transpile + MIR + LIR + per-target | 🟩 **v1 + v1.x mandatory subset DONE 2026-06-03** — sub-plan [`22-optimizer-plan`](./22-optimizer-plan%20-%20tbd.md). **OPT1 ✅** (substrate: PassId / pipelines / X_UnknownPassId, commits `5d29532` + `7d5932e`) + **OPT2 c1 ✅** (ConstFold + JSON pipeline loader + differential-verify runner, `8bae225` + `1a2bed5`) + **OPT3 c1 ✅** (DCE + CompileConfig threading + linkage assertion, `52c1380`) + **OPT4 c1 ✅** (substrate refactors: `mir_dom.hpp` + `mir_rebuild_helper.{hpp,cpp}`, `256b970`) + **OPT4 c2 ✅** (Mem2Reg via Cytron-Ferrante, `2e89b83`) + **OPT4 c3 ✅** (SSA CopyProp Phi-collapse, `f496aea`) + **OPT5 c1 ✅** (CSE / GVN dom-tree value numbering + `path_compress.hpp` helper, `ec7220b`) + **OPT5+ c2 ✅** (SimplifyCFG branch-fold + jump-thread + pipeline-level fixed-point loop, `96bb941`) + **OPT6 c1 ✅** (LICM + onBlockBeforeTerminator hook, `164a1ca` + `93362f1`) + **OPT5+ c3 ✅** (general block-merge + StructCfMarker re-derivation single-non-Linear case, `738413d`) + **effectiveness substrate ✅** (`OptResult.passMutationCount: std::array<size_t, kPassIdCount>` + ConstFold-rerun-post-Mem2Reg effectiveness pin, `86044f4`). **Release pipeline shipped**: `[Identity, ConstFold, Mem2Reg, CopyProp, Cse, Licm, SimplifyCfg, Dce]` with `maxIterations: 4`. Heuristics-as-data from PR1 → OPT10 autotuner. **Remaining v1.x → v2**: alias analysis + value-range substrate (closes `D-OPT-LOAD-ALIAS-ANALYSIS` + `D-OPT6-LICM-TRAP-SAFE-HOIST` + enables full `D-OPT4-1-NON-LINEAR-MARKER-MERGE`); OPT7 inlining; OPT8 LIR peephole + regalloc coalescing; OPT9 vectorization; OPT10 autotuner. Production-readiness §4 (G-401..G-410). |
...
| 13.7 cycle 10+ | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT7+ | Tier-neutral `scalar_eval` consolidation → cost/machine/peephole/pipeline as JSON → OPT10 autotuner. Alias-analysis + value-range analysis substrate (closes `D-OPT-LOAD-ALIAS-ANALYSIS` + `D-OPT6-LICM-TRAP-SAFE-HOIST` + enables D-OPT4-1-NON-LINEAR-MARKER-MERGE). | step 13.7 cycle 9 ✅ | ⏳ planned |
