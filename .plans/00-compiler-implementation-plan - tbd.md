...
| 13.7 cycle 6 | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT5 CSE/GVN | Common-subexpression elimination + global value numbering on the post-Mem2Reg/CopyProp SSA. Triggers `D-OPT-COPYPROP-BLOCK-CURSOR` (per-block dominator cursor for use-substitution), `D-OPT1-CSE-NONCOMMUTATIVE-PIN`. Activates `cse_candidate` + `cse_noncommutative` corpus arms. | step 13.7 cycle 5 ✅ | ⏳ planned |
| 13.7 cycle 7+ | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT5+ | SimplifyCFG (with StructCfMarker repair) → LICM (OPT6) → tier-neutral `scalar_eval` consolidation → cost/machine/peephole/pipeline as JSON → OPT10 autotuner. | step 13.7 cycle 6 ✅ | ⏳ planned |
