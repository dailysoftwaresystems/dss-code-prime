...
| 10.5 | [12.5](./12.5-const-eval-plan%20-%20tbd.md) CE1–CE5 | **Shared constants-evaluation engine** in `src/core/types/type_lattice/` — opens immediately after the MIR substrate-PR. Lifts ML2's inline `tryConstFold` into one language-blind engine consumed by MIR-globals init, HIR verifier (array-length, future enum-bounds), D5.5 enum-value computation, and the optimizer's const-fold pass. Closes the float-arithmetic fold gap delimited by ML2 globals + the `Cast`-fold gap delimited by HR Cast emission. | step 10 | ⏳ **Reserved** — opens when MIR substrate-PR closes |
