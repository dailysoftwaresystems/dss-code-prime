...
> - **[`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md)** — opens parent phase #7 (`analysis-syntactic`): schema-driven recursive-descent driver + Pratt walker for operators + error recovery. Four PRs (PA1 RD driver, PA2 Pratt walker, PA3 recovery + diagnostic UX, PA4 real-world corpus stress). Closes `v2-gap-catalog - tbd.md` row 1 end-to-end. **🔵 next.**
...
| **Parser** (sub-plan PA1–PA4) — schema-driven RD driver + Pratt walker + recovery + real-world corpus stress | 🔵 **next** — phase #7. See [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md). |
| **Semantic** — symbol table, scope resolution, type checking | ⏳ pending (phase #8). See production-readiness §2 (G-201..G-212). Undesigned — design decision required before PR1. |
...
| 7  | 🔵 **next** — sub-plan [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md) | `analysis-syntactic` | Implement syntactic analysis | analysis-lexical | Schema-driven recursive-descent driver (PA1) + Pratt walker for operators (PA2) + error recovery & diagnostic UX (PA3) + real-world corpus stress (PA4). Closes `v2-gap-catalog - tbd.md` row 1 end-to-end. **v1 must deliver:** every test in `tests/core/test_*_subset.cpp` flips to `Parser::parse()`; `tests/corpus/{toy,c-subset,tsql-subset}/` programs parse cleanly; clang-quality diagnostic rendering. See parser-plan §3 acceptance criteria. |
| 8  | ⏳ pending | `analysis-semantic` | Implement semantic analysis | analysis-syntactic | Symbol table + scope resolution + type checking + const-correctness + typedef resolution. Populates `NodeAttribute<TypeInfo>` / `NodeAttribute<SymbolId>` over the CST. New `S_*` diagnostic namespace. **v1 must deliver:** valid c-subset programs type-check cleanly; type-mismatch / undeclared-identifier / redeclaration / const-violation each emit an actionable `S_*` diagnostic; tsql-subset column references resolve against `CREATE TABLE` declarations within the same script. Production-readiness §2 (G-201..G-212). **G-201 (symbol table design) + G-203 (type representation)** are the two design decisions still open — make them before PR1. |
