...
| **Parser** (sub-plan PA0–PA5b, 7 PRs) — substrate `SchemaWalker` ✅; iterative RD driver ✅; Pratt walker + recovery + corpus + LSP server + LSP semantic-stub handlers ⏳ | 🟢 **PA0 + PA1 ✅ done; PA2 next** — phase #7. PA0 (`SchemaWalker` extract, `c764c2a`, PR #7) + PA1 (schema-driven driver with try-each-branch speculation, depth cap, abandon-token guard, lock-step builder; 22 tests). **PA1 follow-up `SpeculationProbe` RAII guard landed** — closes parser-plan open question #5; four-machine snapshot/restore is now structurally local, no behavior change, all 668 tests still green. See [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md). PA5a/PA5b ship LSP with diagnostics only; semantic-powered LSP methods land post-phase #8 in a spun-off `09-lsp-plan`. |
...
| 7  | 🔵 **PA0 ✅ done; PA1 next** — sub-plan [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md) | `analysis-syntactic` | Implement syntactic analysis | analysis-lexical | ✅ `SchemaWalker` substrate extraction (PA0, `c764c2a`, PR #7); ⏳ schema-driven RD driver (PA1) + Pratt walker (PA2) + error recovery & diagnostic UX (PA3) + corpus stress (PA4) + LSP server with diagnostics (PA5a) + LSP semantic-stub handlers + golden replay (PA5b). Closes `v2-gap-catalog - tbd.md` row 1 + production-readiness G-115 end-to-end. **v1 must deliver:** every test in `tests/core/test_*_subset.cpp` flips to `Parser::parse()`; `tests/corpus/{toy,c-subset,tsql-subset}/` programs parse cleanly; clang-quality diagnostic rendering; LSP server publishes diagnostics in real editors. See parser-plan §3 acceptance criteria. |
