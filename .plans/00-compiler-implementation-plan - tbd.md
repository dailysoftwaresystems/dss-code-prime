...
| **`artifactProfile` mechanism** — language config declares supported profiles; project config picks one; codegen reads it | 🟡 **AP1 ✅ + AP2 ✅ + AP3 ✅ (2026-06-09); AP4 remains.** Language config declares its profiles (AP1); project config picks one; the driver enforces `profile ∈ language-set` (AP2, `D_ArtifactProfileNotSupported=0xD010`) + `profile ∈ format-served-set` (AP3, `D_ArtifactProfileFormatMismatch=0xD011`, the format declares `artifactProfiles[]`). Codegen-READING the profile (entry-symbol/subsystem) is deferred trigger-gated on a non-format-redundant consumer (`D-AP2-COMPILATION-CONTEXT`). See [`06-artifact-profile-plan - tbd.md`](./06-artifact-profile-plan - tbd.md). Gates `gen-link` acceptance. |
