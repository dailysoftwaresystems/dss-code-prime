...
| 13.1 | [12](./12-mir-lir-plan%20-%20ok.md) **D-ML7-2.2** stack-passed args | **NEXT — warmest-context move.** Extend `computeFrameLayout` with `outgoingArgAreaSize` + pre-scan for max stack-arg count across call sites + emit `frame_store value, [rsp + shadow + (k-poolSize)*slot]` on caller-side + matching `frame_load` on callee-side. Closes the `L_StackPassedArgUnsupported = 0xB007` fail-loud surface. Builds directly on the D-LK10-ENTRY-ML7-FRAME-BIAS-UNIFY substrate just landed (same `alignedSizeWithBias` helper, same `hasCalls` scan, same `cc.callPushBytes` field) — lowest context-switch / highest continuity. ~200–300 LOC net per prior audit estimate. **NOT a variadic gate** (variadic = step 13.4); this unlocks 5+-fixed-arg calls (`WriteFile`, `fread`, etc.). | step 13 | ⏳ **NEXT** |
| 13.2 | [01](./01-tree-node-model-plan%20-%20ok.md) c-subset typing | `void*` opaque-pointer support in c-subset grammar + typing. Minimum substrate for `HANDLE` / `LPCVOID` / `LPOVERLAPPED` in kernel32 imports (per D-LK10-KERNEL32-WRITE-PATH anchor: "c-subset has no `void*` declared type"). ~50 LOC in c-subset.lang.json + HIR Cast rule for `void*` ↔ other pointer types. Small prereq for step 13.3. | step 13.1 | ⏳ small prereq |
| 13.3 | examples + plan 14 | ★ **hello_writefile — CRT-free hermetic print.** New `examples/c-subset/hello_writefile/main.c` using `kernel32.GetStdHandle` + `kernel32.WriteFile` (5 args; arg-5 = `lpOverlapped` on the stack) + `kernel32.ExitProcess`. Execution-verified by `examples_runner` (Windows host) — same byte-for-byte exitCode + capturedStdout discipline as hello_puts. Closes D-LK10-KERNEL32-WRITE-PATH. **First execution-verified DSS binary that prints with ZERO msvcrt dependency** — more honest than hello_puts (which leans on msvcrt's DllMain self-init). | steps 13.1, 13.2 | ⏳ the ML7-2.2 payoff |
