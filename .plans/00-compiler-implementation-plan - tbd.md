...
| 13.2 | [01](./01-tree-node-model-plan%20-%20ok.md) c-subset typing | **NEXT.** `void*` opaque-pointer support in c-subset grammar + typing. Minimum substrate for `HANDLE` / `LPCVOID` / `LPOVERLAPPED` in kernel32 imports (per D-LK10-KERNEL32-WRITE-PATH anchor: "c-subset has no `void*` declared type"). ~50 LOC in c-subset.lang.json + HIR Cast rule for `void*` ↔ other pointer types. Small prereq for step 13.3. | step 13.1 ✅ | ⏳ **NEXT** |
