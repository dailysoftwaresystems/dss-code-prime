...
| **Semantic** — symbol table, scope resolution, type checking | 🟢 **SE1+SE2+SE3 shipped** (phase #8; sub-plan [`08.6-semantic-plan`](./08.6-semantic-plan%20-%20tbd.md)). One language-agnostic `analyze()` engine driven by a `semantics` schema-v4 block produces a `SemanticModel` (symbol table + scope tree + node→SymbolId / node→TypeId side-tables + `S_*` diagnostics). Toy/c-subset/tsql onboarded purely by config; cross-file visibility is `crossRefs[]`-import-driven. SE4–SE7 (const-correctness G-205 / typedef G-206 / overloads G-207 / LSP wiring) remain. G-201..G-204/G-208/G-209/G-211/G-212 addressed. |
...
| **7** | [08.6](./08.6-semantic-plan%20-%20tbd.md) SE1–SE7 | Symbol table + scope resolution + type checking — **config-driven** (`semantics` block, schema v4; one language-agnostic engine). 🟢 **SE1+SE2+SE3 shipped** (engine + toy/c-subset/tsql `semantics` configs; per-tree-root scopes + `crossRefs[]` import injection; 63/63). SE4–SE7 (const / typedef / overloads / LSP) remain. | step 7a ✅ | 🟢 **SE1–SE3 done** |
