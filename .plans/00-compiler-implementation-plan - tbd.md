...
| 13.7 cycle 7 | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT5+ SimplifyCFG | Unreachable-block elim + block-merge + jump-thread + branch-fold, with `StructCfMarker` repair (D-OPT4-1 — load-bearing for WASM/SPIR-V lowering). After CSE/GVN; thereafter a recurring pipeline pass (multiple pipeline.json entries, run to fixed point with ConstFold+DCE — triggers `D-OPT-FIXED-POINT-LOOP` + `D-OPT1-PASS-RUN-MAX-ITER`). | step 13.7 cycle 6 ✅ | ⏳ planned |
| 13.7 cycle 8+ | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT6+ | LICM (loop-invariant code motion) → tier-neutral `scalar_eval` consolidation → cost/machine/peephole/pipeline as JSON → OPT10 autotuner. | step 13.7 cycle 7 ✅ | ⏳ planned |
