...
| **V3-3** → **promoted into the ARM64 arc** | Mach-O exec-verify (Apple Silicon) | 🟢 **ACTIVE 2026-06-08 (user-directed) — the macOS-runner trigger has FIRED**: `macos-latest` CI = Apple Silicon (arm64), so a `macho64-arm64-darwin-exec` corpus runs natively there. Folded into the ARM64 arc (see 13X.2) as the macOS-Mach-O platform alongside Linux-ELF; completes 3-OS execution verification. **§B 2026-06-08 — BLOCKED on code-signing → ad-hoc codesign goes FIRST.** Apple Silicon's kernel SIGKILLs unsigned arm64 binaries + DSS only RESERVES `LC_CODE_SIGNATURE` placeholder bytes (real fill = Plan 16 / V3-2, unbuilt), so a macho64-arm64-darwin-exec could be byte-pinned but NOT RUN (the §A.5 trap). User chose: build minimal **ad-hoc Mach-O codesign** (`D-LK7-ADHOC-CODESIGN-MACHO` — SHA-256 in `src/core/crypto` + a config-driven `codeSignature` block filled by the `macho.cpp` walker; plan LOCKED in the registry) as the active prerequisite cycle, THEN this macОS-ARM64 exec + a runnable corpus. | 13X.2 + `D-LK7-ADHOC-CODESIGN-MACHO` | **ad-hoc codesign (next cycle)** then macos-latest=arm64 (FIRED) | 🟡 **BLOCKED on ad-hoc codesign (the active prerequisite)** |
