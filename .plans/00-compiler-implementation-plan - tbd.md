...
| **V2-4** | program-api CLI (~2–3) | `dss-code-prime build my-project.dss-project.json`: project loader + driver + **clang-quality diagnostic renderer**. Makes the compiler a usable tool. **MUST include (user decision 2026-06-09; strict tests are an acceptance bar, not nice-to-have) — `D-DIAG-CLI-POSITION-RENDER-AND-ASSERT`:** **(A)** wire the CLI stderr path to the EXISTING `DiagnosticReporter::format()`/`formatAll()` clang-style renderer (`--> file:line:col` + source line + `^` caret + related-location notes; built + unit-tested at `diagnostic_reporter.cpp:298-413` but currently UNWIRED — the driver's `drainDiagnosticsToStderr` (program.cpp:83, 14 sites) prints a code-only one-liner with NO position); **(B)** EXPLICIT position assertions (`.span` / `lineCol`, not code-only) on the `P_*`/`S_*` parser+semantic error tests (today they assert the code + sometimes `.actual`, almost never the position → a wrong-position regression passes silently); **(C)** corpus expected-compile-error + miscompile identification — extend `examples_runner` + `expected.json` with an `expectError`/`expectDiagnostic` (+ position) field so a malformed source asserts a SPECIFIC diagnostic at a SPECIFIC location, alongside the existing optimized==baseline negative-miscompile pins. "Very good error detection" (compile correctly + identify miscompiles + precise positioned diagnostics) is the V2-4 acceptance bar. | §8 program-api (step 16) | V2-3 (artifactProfile) | ⏳ |
