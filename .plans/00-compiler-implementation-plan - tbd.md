...
| **Total ctest cases** | **668 cases across 33 suites, 100% pass** (post PA1 + 5-agent review + fix-everything round). PA1 adds 22 parser tests (12 toy / 6 speculation / 1 c-subset smoke / 3 death). |
...
| **Parser** (sub-plan PA0–PA5b, 7 PRs) — substrate `SchemaWalker` ✅; iterative RD driver ✅; Pratt walker ✅; recovery + diagnostic UX ✅; corpus + LSP server + LSP semantic-stub handlers ⏳ | 🟢 **PA0–PA3 ✅ done; PA4 next** — phase #7. PA0 substrate, PA1 RD driver, PA2 Pratt walker (closes v2-gap-catalog row 1 end-to-end), and PA3 panic-mode recovery + `followSetOf` + clang-style diagnostic rendering (closes G-114 + G-115) all shipped on `feature/parser-phase`. 715 tests / 35 suites green. PA4 onboards real corpus programs; PA5a/PA5b ship LSP with diagnostics only; semantic-powered LSP methods land post-phase #8 in a spun-off `09-lsp-plan`. See [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md). |
