...
| **Parser** (sub-plan PA0–PA5b, 7 PRs) — substrate `SchemaWalker` ✅; iterative RD driver ✅; Pratt walker ✅; recovery + diagnostic UX ✅; corpus stress ✅; LSP server + LSP semantic-stub handlers ⏳ | 🟢 **PA0–PA4 ✅ done; PA5a next** — phase #7. PA0–PA3 as before; PA4 onboarded real corpus programs for c-subset / tsql-subset / toy, closed v2-gap-catalog rows 6/7/8/12 via operator-table extensions (`endsAt` + `bodyRule` for grouped postfix; prefix `*`; argList shape) + a new off-grammar body-token drain in the parser, tokenizer flag-propagation fixes, and tsql-subset schema additions (`--`/`/* */` comments, optional INSERT column list, `tableRef` with alias, `qualifiedName` in `operand`). 36 ctest suites green. PA5a/PA5b ship LSP with diagnostics only; semantic-powered LSP methods land post-phase #8 in a spun-off `09-lsp-plan`. PA4 follow-ups (type refactors, additional test coverage, postfix-chain investigation) tracked in [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md) §2.3.1. |
