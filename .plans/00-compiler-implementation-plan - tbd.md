...
> **Version: v0.0.2 (in development) · v0.0.1 SHIPPED.** **v0.0.1** = the config-driven c-subset→native toolchain, end-to-end: tokenizer/parser/LSP + CU + semantic + HIR→MIR→LIR + x86_64/ARM64 hand-encoders + ELF/PE/Mach-O in-tree linker + cross-CU LTO merge + FFI shipped-lib ingestion (FF11) + the full optimizer **OPT1–OPT7** (ConstFold/DCE/Mem2Reg/CopyProp/CSE/SimplifyCFG/LICM/block-merge/**Inlining** — shipped in release, recursion-safe, cross-CU, intrinsic-aware). Runnable binaries; **Windows-PE + Linux-ELF (x86_64) execution-verified**. **v0.0.2 roadmap** (the recommended-interleave build-out, §0.1): ARM64 runnable+verified ∥ language-agnostic shipped-lib ingestion → artifactProfile + `dss-code-prime build` CLI → RISC-V → WASM → OPT7+ → carried-forward v0.0.1 backlog. §0.1 is now SPLIT — the **v0.0.2 ROADMAP** (next-up, the list `/dss-cycle` walks) sits above the **v0.0.1 as-built ledger** (done).
...
> Walk top-to-bottom. Each arc is one or more `/dss-cycle` cycles (a cycle = one priority, fully, to push). New-target / new-backend arcs are **substrate cycles** → `/dss-cycle` Step 3.5 pauses for a §B design sign-off before building. Suggested version cut-points: **v0.0.2** = V2-1…V2-4 (ARM64 + shipped-lib + artifactProfile + CLI) · **v0.0.3** = V2-5…V2-7 (RISC-V + WASM + OPT7+) · **v0.0.4** = V2-8+ (the rest) (drawn as arcs complete; the loop walks rows regardless of the cut-point labels).
...
| **V2-8** | Mach-O exec-verify | macOS CI runner → execution-verify the Mach-O target already emitted (the macho exit block 13X.2). | 13X.2 | macOS runner | ⏳ trigger-gated (runner) |
| **V2-9** | Debug info (DWARF/PDB) | DB1–DB12: DWARF 5 (ELF+Mach-O) + PDB (PE) + CFI / SEH / compact-unwind. | [15](./15-debug-info-plan%20-%20tbd.md) (step 14) | steps 10/13 ✅ | ⏳ |
| **V2-10** | Codesign / publish | CS1–CS9: Apple codesign + notarization + Authenticode + RFC 3161 + APK v3 (vendored BearSSL). | [16](./16-codesign-publish-plan%20-%20tbd.md) (step 15) | steps 13/14 | ⏳ |
| **V2-11** | SPIR-V / shader-GPU | SG1–SG10: SPIR-V codegen + shader HIR + GPU bindings + dual CPU+GPU lowering (`spirv-1.6.format.json` exists). | [17](./17-shader-gpu-plan%20-%20tbd.md) (step 18) | steps 8/10 ✅ | ⏳ |
| **V2-12** | Source-to-source translation | ST1–ST6: HIR-pivot transpilation (`.map.json` + HIR→HIR walker + target-CST + pretty-printer). | [10](./10-source-translation-plan%20-%20tbd.md) (step 19) | step 8 ✅ | ⏳ |
| **V2-13** | CU5 multi-language CUs | One binary, mixed source languages, HIR convergence. | [08](./08-compilation-unit-plan%20-%20tbd.md) CU5 (step 20) | steps 4/8 ✅ | ⏳ v1.1 |
| **V2-14** | CU6 cross-CU / incremental | Incremental rebuild + shared libs/DLLs (LK11 cross-CU largely landed cycles 15–19; CU6 incremental remains). | [08](./08-compilation-unit-plan%20-%20tbd.md) CU6 + [14](./14-linker-plan%20-%20tbd.md) LK11 (step 21) | — | ⏳ v1.x |
| **V2-15** | P2 docker · P3 CI/fuzzing | Dockerfile + cross-compile toolchains; nightly fuzzing + extended sanitizers (rolls continuously). | §8 | — | 🟦 partial |
| **V2-16** | Reserved (gated) | R1 HW HIR (VHDL/Verilog/SystemVerilog), R2 custom language, R3 runtime (GC/exceptions/coroutines/threading). | [19](./19-hir-hw-reserved-plan%20-%20tbd.md) / [20](./20-custom-language-reserved-plan%20-%20tbd.md) / [21](./21-runtime-reserved-plan%20-%20tbd.md) | trigger | 🔒 |
