...
| 7.5 | ⏳ **new** — sub-plan [`08-compilation-unit-plan - tbd.md`](./08-compilation-unit-plan - tbd.md) | `compilation-unit-model` | Multi-file translation units + import resolution | analysis-syntactic | Bridges parser (per-file `Tree`) and semantic (cross-file symbol table). Six PRs (CU1–CU6; CU5 multi-language added rev 2; CU6 cross-CU references paired with [`14-linker-plan`](./14-linker-plan%20-%20tbd.md) LK11 per its §2.12). Closes G-110 + G-111. **v1:** CU1–CU4. **v1.1:** CU5. **v1.x:** CU6 ↔ LK11; trigger = first artifact profile needing multiple CUs in one image. |
