...
| **Parser** (sub-plan PA0–PA5b + PA5a-prep + PA-Walker-LeftRec) — substrate `SchemaWalker` ✅; iterative RD driver ✅; Pratt walker ✅; recovery + diagnostic UX ✅; corpus stress ✅; c-subset readiness shapes ✅; LSP server + LSP semantic-stub handlers ⏳; postfix-chain walker redesign ⏳ | 🟢 **PA0–PA4 + PA5a-prep ✅ done; PA5a + PA-Walker-LeftRec in flight** — phase #7. PA0–PA3 as before; PA4 onboarded real corpus programs (c-subset / tsql-subset / toy) and closed v2-gap-catalog rows 6/7/8/12 (postfix call/index/inc/dec, prefix deref). PA4 follow-ups (PA4-F1..F8) landed substrate refactors (`OperatorTable::Entry → optional<GroupedPostfix>`, `bodyDefaultTokenKinds` dedup), additional test coverage, and corpus golden harness. PA5a-prep closed v2-gap-catalog rows 12-declarator (`int a[10];`), 16 (compound assignment), 29 (`extern` declaration). 36 ctest suites green. PA5a now ships the LSP server skeleton + diagnostics; PA-Walker-LeftRec lands the postfix-chain redesign in parallel (`f(a)[i]` left-recursion). PA5b adds semantic-stub LSP method handlers + golden replay; semantic-powered LSP methods land post-phase #8 in a spun-off `09-lsp-plan`. |
