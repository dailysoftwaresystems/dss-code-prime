...
| D5 | **c-subset structs / unions / enums** | 🟡 **PULLED FORWARD 2026-05-28** (before ML2) to make ML2's HIR→MIR lowering honor "no real-blocker deferrals" on `MemberAccess`/`ConstructAggregate`/`TypeDecl`. **Kitchen-sink scope** confirmed: core structs + typedef'd + anonymous structs + designated initializers + unions + enums + first-class MIR aggregates (ExtractValue/InsertValue). **6-PR sub-arc**: D5.1 core structs → D5.2 typedef'd/anon → D5.3 designated init → D5.4 unions → D5.5 enums → D5.6/ML2-prelude MIR aggregate opcodes → then ML2. **D5.1 substrate landed in 3 additive checkpoints** (`feature/mir-lir`): `251eca0` (config vocab + `SymbolRecord::fieldIndex`/`structScope`), `9079916` (Pass 1/1.5/2 engine consumption + `S_NotAPointer`/`S_NotAComposite` codes + cross-field loader validation), and a new `followerRule` operator-table primitive (generic postfix "op + single rule" form, e.g. `obj.field`) + `MemberAccessRule.operatorToken` discriminator + post-shapes followerRule validation. **Grammar-architecture blocker found**: c-subset's parser does first-token alt disambiguation WITHOUT backtracking, so `structDecl` (starts with `struct`) and `topLevelDecl` using `structTypeRef` (also starts with `struct`) collide. Proper fix requires factoring the shared `struct Identifier` prefix into a common path with tail-alt disambiguation + extending `kindByChild` to conditionally set `fieldChildren` — next D5.1 cycle. | feature (multi-PR) | the 6-PR D5 sub-arc (D5.1 substrate ✅; grammar refactor + lowering + tests next) | maximally honest c-subset for ML2 |
