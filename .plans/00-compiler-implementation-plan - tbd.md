...
| **Diagnostic UX** — positioned source-context (`file:line:col` + caret) + actionable messages (DSS's own renderer, NO clang/LLVM) | 🟢 **partial — CLI positioned rendering wired 2026-06-09 (V2-4 Part A).** The driver `drainDiagnosticsToStderr` now routes buffer-bearing (parser/semantic) diagnostics through DSS's own `DiagnosticReporter::format()` (`--> file:line:col` + source line + `^` caret) via a `BufferRegistry` built from the CUs; buffer-less driver `D_*` stay code-only. **Remaining (V2-4 B+C):** explicit position assertions on the `P_*`/`S_*` error tests + a corpus `expectError`(+position) field. Production-readiness G-115 + G-608. |
