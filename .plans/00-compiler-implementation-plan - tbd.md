...
|---|---|---|---|---|---|
| **V2-1** ∥ | ARM64 runnable + verified (~2–4) | Build **`D-LK10-ENTRY-ARM64`** (ARM64 `svc #0` syscall + GOT/PLT macro-op `ADRP`+`LDR`+`BLR` rows in `arm64.target.json` + per-format `processExit` for `elf64-aarch64-linux-exec`) so ARM64 binaries actually exit/run; THEN add `qemu-aarch64` to the ubuntu CI runner + flip `examples_runner`'s cross-host SKIP to RUN aarch64 under QEMU → execution-verify the whole ARM64 backend. Builds the QEMU verify harness V2-5 reuses. | [14](./14-linker-plan%20-%20tbd.md) 13X.1 · §8 testing | — (ARM64 emits; needs exit + QEMU) | ⏳ **NEXT** (∥ V2-2) |
| **V2-2** ∥ | Language-agnostic shipped-lib ingestion (~3–4) | The neutral-JSON `ImportSurface` descriptor store (**`D-FFI-SHIPPED-LIB-DESCRIPTOR-AGNOSTIC`**): shipped libs (libc/kernel32/libSystem) described ONCE in a language-neutral JSON format under `src/dss-config/shippedLibs/<platform>/` + per-language `shippedLibDirs` search-path, feeding the existing `ingest()` matcher — superseding the per-language C-header (`.h`) path FF11 shipped in cycle 21. **USER-PRIORITY.** | [11](./11-ffi-plan%20-%20tbd.md) FF11+ | — (FF11 core ✅ c21) | ⏳ **NEXT** (∥ V2-1) |
