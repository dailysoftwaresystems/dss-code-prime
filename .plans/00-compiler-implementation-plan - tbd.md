...
| 13.7 cycle 4 | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT4 c2 Mem2Reg | Cytron-Ferrante SSA construction: promotable-alloca scan + iterated dominance frontier + Phi insertion + rename DFS. Likely closes `D-OPT-MIR-REBUILDER-ONBLOCKBEGIN-HOOK` (Mem2Reg's Phi-insertion shape). | step 13.7 cycle 3 ✅ | ⏳ planned |
| 13.7 cycle 5+ | [`22`](./22-optimizer-plan%20-%20tbd.md) OPT4 c3+ | SSA copy-prop (after Mem2Reg exposes Phi-collapse opportunities) → CSE/GVN (OPT5) → SimplifyCFG → LICM (OPT6) → tier-neutral `scalar_eval` consolidation → cost/machine/peephole/pipeline as JSON → OPT10 autotuner. | step 13.7 cycle 4 ✅ | ⏳ planned |
