...
> **v1 production target locked.** End-to-end binaries for `toy` / `c-subset` / `tsql-subset` on **{Windows, Linux, macOS} × {x86_64, ARM64}**. Frontend (tokenizer + parser + LSP) ✅; CU + semantic + HIR + MIR + LIR + assembler + linker + FF6 runtime print ✅; OPT1–OPT6 v1 + v1.x mandatory subset ✅ (OPT5+ c3 closed 2026-06-03; v1 ConstFold + DCE done; v1.x Mem2Reg + CopyProp + CSE + SimplifyCFG + LICM all live). **OPT7-prep 10a-10o ✅** 2026-06-04 — alias analysis substrate + char-exception config + alias-arc strict capstone (corpus + MIR-tier effectiveness) + LICM chained-invariants closure + `long`→I64 in c-subset + sext encoding + schema-mutation test substrate + diagnostic code per-kind split. Driver (`compileProject`) + `artifactProfile` mechanism + debug info + cross-host ARM64 / Linux / macOS Stage 2 still ⏳ — see [`07-production-readiness-plan - tbd.md`](./07-production-readiness-plan - tbd.md) for the 127-item gap catalog.
