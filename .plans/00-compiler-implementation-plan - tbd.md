...
| **Total ctest cases** | **646 cases across 30 suites, 100% pass** (post PA0 + 5-agent review + fix-everything + 5th-reviewer round) |
...
| **Parser** (sub-plan PA0–PA5b, 7 PRs) — substrate `SchemaWalker` extraction ✅; RD driver + Pratt walker + recovery + corpus + LSP server + LSP semantic-stub handlers ⏳ | 🔵 **PA0 ✅ done; PA1 next** — phase #7. PA0 (`SchemaWalker` extract, `c764c2a`, PR #7) shipped + reviewed; 21 new walker tests, 646 cases total. See [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md). PA5a/PA5b ship LSP with diagnostics only; semantic-powered LSP methods land post-phase #8 in a spun-off `09-lsp-plan`. |
