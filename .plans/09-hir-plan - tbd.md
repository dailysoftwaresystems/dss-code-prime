...
| HR9 | CST→HIR lowering — toy + c-subset completions | Enrich `toy` into a real small typed language (config-only) so it lowers end-to-end; complete c-subset's HR8-deferred constructs (externs/FFI per plan 11, compound-assign + `++`/`--` once HIR has once-only-lvalue eval, arrays once the lattice resolves Array types, strings). Structs/unions/enums when they land. |
