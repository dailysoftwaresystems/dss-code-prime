...
| | |
|---|---|
| Status        | 🟡 **in progress.** HR1 ✅ (substrate-tier: arena + node shapes + walker + ids + extension registry) + HR2 ✅ (substrate-tier: typed expressions — operator open-core+registry, typed builder helpers, `HirVerifier` expression-typing rule) — landed 2026-05-26 on `feature/hir-1`. HR3–HR11 still to do. v1 production-critical; the single largest substrate addition in v1. |
