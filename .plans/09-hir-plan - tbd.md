...
| | |
|---|---|
| Status        | ✅ **COMPLETE (HR1–HR11 all landed 2026-05-26..28 on `feature/hir-1`).** HR1 ✅ (substrate-tier: arena + node shapes + walker + ids + extension registry) + HR2 ✅ (substrate-tier: typed expressions — operator open-core+registry, typed builder helpers, `HirVerifier` expression-typing rule) + HR3 ✅ (substrate-tier: structured CF — statement builders, typed read-accessor layer, break/continue + per-kind-arity verifier rules) + HR4 ✅ (declarations + extern surface — Module/Function/Global/TypeDecl/Extern*/ImportGroup builders + accessors, FfiMetadata side-table, checkDeclarationShape verifier rule) + HR5 ✅ (substrate-tier: attribute system + side-tables — `HirSourceLoc`/Shader/Transpile/Diagnostic value structs + `hir_attrs.hpp` catalog, verifier threads an optional `HirSourceMap` for real diagnostic spans, **HR2 span-stash IOU closed**) — landed 2026-05-26 on `feature/hir-1`. + HR6 ✅ (substrate-tier: full verifier — block dead-code + non-void return-completeness + Call-arg-vs-FnSig + intrinsic-registered + shader-restriction subverifier; new `HirIntrinsicRegistry` in the frozen module; optional `TypeInterner` injection; `H_UnknownIntrinsic`/`H_ShaderViolation` codes) — landed 2026-05-27 on `feature/hir-1`. + HR7 ✅ (substrate-tier: round-trippable `.dsshir` text format — `emitHir`/`parseHir` free fns + non-owning `HirTextContext` injection (mirrors verifier) + heap-stable `HirParseResult`; hand-rolled lexer + recursive-descent parser; inline structural types (nominal-by-name), positional `%N` symbol handles bound to names in the preamble, all five side-tables inline, `DiagnosticInfo.origin` as a pre-order node index; verify-on-load; in-memory + golden-corpus tests; `H_TextMalformed`/`H_TextVersionMismatch`/`H_TextUnknownName`) — landed 2026-05-27 on `feature/hir-1`. + HR8 ✅ (config-driven CST→HIR lowering engine + the `hirLowering` schema facet, proven end-to-end on **c-subset** — reordered ahead of toy because shipped toy is too minimal to produce a verifiable HIR; one language-agnostic engine, no `schema.name()` branch, reuses `semantics()`; per-expression type inference + a `HirLiteralPool` of decoded literal values; verify-on-load; deferred constructs fail loud via `H_UnsupportedLoweringForKind`) — landed 2026-05-27 on `feature/hir-1`. + HR9 ✅ done 2026-05-27..28 (toy enriched in place into a typed language + generic lowering test; arrays un-deferred end-to-end; compound-assign/`++`/externs/genericity) **+ gap-closure pass 2026-05-28** completing the frontend→HIR layer: char/string literal VALUES (coalesce-body lexer), HIR `SeqExpr` (value-yielding `++` / assignment-as-expression / complex-lvalue compound-assign), pointers (`int *p`), ternary `?:` (real mixfix arity), `#include` skip, and `.dsshir` inline literal values (faithful value round-trip). 6-agent review clean, 81/81. Only full C declarators deferred (plan-00 D11). + HR10 ✅ done 2026-05-27..28 (CST→HIR lowering for **tsql-subset** through the SAME language-agnostic engine — role-explicit SQL extension nodes via a generic `childGathering` config vocabulary + `ChildLower` enum; flat-expr lowering; coalesced/doubled-delimiter string VALUES → `Array<Char,N>`; `NULL`→`TSQL::Null`; relational names→`refExtensionKind` leaf, not core `Ref`; `ReferenceRule.hardParents` positional table-vs-column resolution; SQL calls reuse `callRules`. 5-perspective review + fix pass clean, 82/82). **+ HR11 ✅ done 2026-05-28 (multi-language CU lowering — one CU of mixed `.c`+`.sql` files → one verified HIR module through the same engine; per-schema config bundles selected by `tree.schema()`, per-file schema routing by extension (CU5), per-language builtin scoping, language-blind cross-ref substrate; cross-language edge/type marshalling deferred to FFI plan 11). **Plan 09 (HIR) COMPLETE — HR1–HR11 all ✅.** v1 production-critical; the single largest substrate addition in v1. **D5.3 designated initializers — READY TO RESUME 2026-05-28** (memory `project_d5_3_paused_on_parser_substrate` + `project_pratt_walker_speculation_gap_2026_05_28`): D5.3 architecture + full C99 §6.7.8 scope decided (2 cycles: HIR substrate then MIR slot-backed; recursive `std::vector<HirLiteralValue>` aggregate variant arm; chained-brace + dot-chained designators; field + index designators; mixed positional/designated with fill-cursor; string-as-char-array; all 4 context-typing sites + compound-literal `(T){...}`; new `HirLoweringConfig` fields `braceInitListRule` / `initElementRule` / `designatedFieldRule` / `designatedIndexRule` / `compoundLiteralRule` — strictly config-driven, zero `schema.name()` branches). **Substrate sub-cycle A ✅ done** (plan 05 post-close, commit `6a8f146`): speculative-alt now handles token-leaf branches. **Substrate sub-cycle B ✅ done** (plan 05 post-close, commits `14d789b` + `19c0b8b`): SchemaWalker recognizes auto-interned Pratt wrapper rules via `GrammarSchema::isAutoInternedWrapperRule` and suppresses the false-positive desync latch trip via O(1) `wrapDepth_ > 0` check; 3-agent review fixes folded inline. **D5.3 implementation is unblocked** — can resume on the existing 2-cycle architecture. Next implementation start: re-apply the c-subset speculative `operand` + `parenExpr` extraction + `compoundLiteralExpr` rule, then proceed with the HIR substrate cycle.

**Post-closure substrate fix 2026-05-28** (commit `3e6b523`): `lowerTopLevel` (the `topLevelDecl`→Decl path for c-subset / similar dual-purpose decl rules) was reading the initializer expression raw via `lowerExpr(c).id`, skipping the `coerce(initE, type)` call that `lowerVarLike` correctly applies for local VarDecls. Result was a declaration like `int g = 1.7 + 2.5;` landing with an F64-typed BinaryOp init under an I32-typed global — the implicit narrowing the runtime would perform was missing from the HIR. Gap uncovered while writing the plan-12.5 CE5 end-to-end MIR test; fix is one missed call site of the same language-blind `coerce` helper (no per-schema logic, no policy change). 89/89 ctest. **Post-closure verifier-evolution mapped item (from plan 12.5 §0.2 D2)**: a future HIR verifier sub-cycle adds a `refuseOnLossyFloatConversion` knob to `EvalOptions` so the verifier can surface "you're losing bits in this float→int / int→float conversion" diagnostics without forcing every codegen consumer to refuse. Engine substrate is ready; verifier wiring is the consumer-side work. |
...
| HR9 | CST→HIR lowering — toy + c-subset completions | 🟢 **substantially done** on `feature/hir-1`. ✅ **landed 2026-05-27**: compound-assignment (`x += 1` → `x = x + 1`) + statement-position `++`/`--` (correct because c-subset's only lvalue is a side-effect-free variable `Ref`, safely duplicated; complex/value-yielding cases fail loud); **externs** → `ExternFunction`/`ExternGlobal` (grammar split `externTail`→named `externFuncTail` so `kindByChild` matches + `externDecl` semantics decl + `lowerExternDecl`; FFI *metadata* stays plan 11); **genericity test** (`test_hir_lowering_generic.cpp` — a synthetic non-shipped language lowers through the engine, proving no `schema.name()` dependence — HR8 follow-up); **toy enriched in place** into a real small typed language (config-only: funcs+params, typed `var`, if/else/while, return, calls, Pratt exprs) + a generic lowering test (`test_hir_lowering_toy.cpp`, 10 cases + `.dsshir` golden) — `var`-at-module-scope→Global vs local-`VarDecl` split is by lowering context (one rule, no second rule); ~9 dependent parser/semantic/tree/corpus/LSP test files updated + goldens regen'd; **arrays** un-deferred end-to-end (`int a[10]`→`Array<I32,10>`): `TypeKind::Array`/`TypeInterner::array` already existed, so the new work is a config-driven `DeclarationRule.arraySuffix` declarator-suffix descriptor + semantic-time constant-length eval (`constIntLength`/`applyArraySuffix`), `S_NonConstantArrayLength` (0xE00B) on non-constant/absent length (fail loud, no pointer decay), a shared `decodeInteger` (`core/types/number_decode.hpp`, also the F2 DRY win), and HIR init-scan skipping of the suffix subtree; 3-agent review folded in (two consistency fixes). **+ gap-closure pass 2026-05-28** (separate from HR9 but completes the same frontend→HIR layer; see [v2-gap-catalog](./v2-gap-catalog%20-%20tbd.md) rows 17/19 + plan-00 D1/D7/D11): **char/string literal VALUES** ✅ (tokenizer coalesce-body flag — TZ4; the former blocker is resolved, char→Char, string→`Array<Char,N+1>`); **value-yielding `++` / assignment-as-sub-expression / complex-lvalue compound-assign** ✅ (new HIR `SeqExpr` core node + an `Lvalue` temp-pointer abstraction — sound in loop conditions); **pointers** `int *p`/`int **p` ✅ (`semantics.pointerToken` depth-count + `&` prefix); **ternary `?:`** ✅ (real `OperatorArity::Ternary` mixfix); `#include` lowering-skip ✅; **`.dsshir` inline literal values** ✅ (G17 — `HirLiteralPool` threaded through `HirTextContext`/`HirParseResult` like the side-tables; `lit int 42`/`lit str "hi"` faithful value round-trip; lexer gained a Float token; index `#N` form kept for pool-less modules). 6-agent + focused-3-agent reviews clean (81/81); Windows-CI golden CRLF made line-ending agnostic. ⏳ still deferred (pinned plan-00 D11): **full C declarators** (function pointers, arrays-of-pointers, multi-declarator). Structs/unions/enums when they land. |
| HR10| CST→HIR lowering — tsql-subset ✅        | **Landed 2026-05-27..28** on `feature/hir-1`. SQL statements lower to **role-explicit HIR extension nodes** through the SAME language-agnostic engine — zero SQL vocabulary in `src/hir/lowering`, no `schema.name()` branch. Each statement is a `HirKind::Extension` of a registered kind (`TSQL::Select`/`Insert`/`Update`/`Delete`/`CreateTable`) whose role-children are gathered generically per a new **`childGathering` config vocabulary** (`ChildSlotSpec{ matchRule\|classifier, optional, list, lower, role }`, where `lower` ∈ a `core`-local `ChildLower` enum `{Expr,FlatExpr,Ext,Ref,VarDecl}` validated at LOAD); groupings (`TSQL::Projection`/`Star`/`ValueList`/`ColumnList`/`AssignList`/`Assignment`/`Columns`) are nested extension nodes; conditions/values **reuse core `BinaryOp`/`Literal`** (typed: comparison→Bool); columns reuse core `VarDecl`. New engine facets, all config-driven: **flat-expression lowering** (`flatExprRule`/`flatBinaryOpRule` — SQL's `operand (op operand)*` left-folded into core `BinaryOp`, distinct from the Pratt path; shared `combineBinary`); **string VALUES** coalesce + decode to `Array<Char,N>` (per-mode `coalesce` flag emitting the in-grammar `StringLiteral` body token; **doubled-delimiter** `''` decoder via `stringDoubledDelimiter` + loader-derived `stringDelimiter`; second opener `unicodeStringStartToken` for `N'…'`); **`NULL`** → a leaf `TSQL::Null` extension operand (`nullToken`/`nullExtensionKind`); **relational names** (table/column/function refs) → a leaf `refExtensionKind` (`TSQL::Name`) extension rather than a core `Ref`, because SQL names are NOT typed value reads (the verifier requires `Ref`s to be typed) — the Call node carries the result type. SQL **function calls** (`COALESCE(...)`) reuse the semantics `callRules`. Semantic enabler: `ReferenceRule.hardParents` — a reference unresolved under a must-resolve parent (`tableRef`/`insertStmt`/`updateStmt`/`deleteStmt` = a TABLE) emits `S_UndeclaredIdentifier`, but the SAME `qualifiedName` rule in an expression (a relational COLUMN, bound against the FROM relation this frontend doesn't model) stays soft (sym 0, name recoverable from provenance). Fail-loud throughout: undeclared ext-kinds validated at load (`requireDeclaredExtKind`), nameless `ref` slots / empty required `list` slots / malformed flat exprs all report. 11 lowering tests + a `.dsshir` golden of a 6-statement DML program (verify + round-trip); coalesce reworked the tsql `.tree` corpus golden + 4 tokenizer/`test_tsql_subset` string tests (per-codepoint → coalesced). 5-perspective review + fix pass clean; 82/82 ctest. Pins the per-language extensibility story: a new SQL-family construct = new config only. |
| HR11| Multi-language CU lowering ✅            | Per CU5 from `08-compilation-unit-plan`. **One CU → one HIR program with per-file lowering** through the SAME language-agnostic engine. **Landed 2026-05-28** on `feature/hir-1`: the semantic engine (`analyze`) and the HIR lowering engine (`lowerToHir`) each build a **per-schema config bundle** (the schema-specific `RuleId`/`SchemaTokenId` index maps — `declByRule`/`refByRule`/`ruleMap_`/`binOp_`/`litType_`/…), one per distinct schema in the CU, selected per tree by `tree.schema()` — because a `RuleId` is only meaningful within its own schema. No `schema.name()` branch anywhere (dispatch is schema identity). Type extensions from every schema in the CU are registered into the one CU-global `TypeInterner`; HIR extension kinds from every language coexist in the one CU-global `HirKindRegistry` (already CU-global — no change). One `HirModule` holds decls from all trees; its `sourceLanguage` label is the composite of the distinct schema names. A homogeneous CU is the degenerate one-bundle case (every existing single-language test unchanged). **Builtin functions are scoped PER-LANGUAGE** (each tree's root parents to its own schema's builtin scope) so they don't leak across languages — a c-subset file does NOT resolve tsql's `COALESCE` (that would be a cross-language reference, deferred to FFI). An unroutable `addFile` extension in a multi-language CU fails loud (`D_UnknownFileExtension`), never silently parses under the wrong grammar. Proven end-to-end: a mixed c-subset + tsql-subset CU → one verified HIR module (a c `Function` + a `TSQL::Select` extension node), via the multi-language `UnitBuilder` (CU5); same-language cross-tree references still resolve across the interleaved other-language file. **Explicitly DEFERRED (boundaries, not gaps):** (1) cross-*language* reference **edge auto-discovery** (c calls a sql sproc) = FFI import/export matching → [`11-ffi-plan`](./11-ffi-plan%20-%20tbd.md); HR11 delivers the uniform language-blind resolution substrate (an injected/same-language cross-tree edge resolves regardless of language) + per-tree same-language import resolution, but does not synthesize cross-language edges. (2) cross-language **type/ABI marshalling** → plan 11. (3) **artifactProfile aggregation** → [`06-artifact-profile-plan`](./06-artifact-profile-plan%20-%20tbd.md). Pins the per-language extensibility story end-to-end: many languages, one HIR, config only. |
