...
| | |
|---|---|
| Status        | ⏳ **planned (rev 1).** v1 ships the mandatory scalar subset; the full multi-tier roadmap + the autotuner research arc are scoped here and anchored as deferred cycles. **Maximal scope by directive** — HIR/MIR/LIR/per-target passes AND the learned-heuristics frontier all live in this one plan. |
| Predecessors  | ✅ [`12-mir-lir-plan`](./12-mir-lir-plan%20-%20ok.md) (MIR SSA-over-CFG + dominators-in-verifier + the `lir_pass_util` build-once-freeze pass pattern — ML1–ML8 closed). ✅ [`12.5-const-eval-plan`](./12.5-const-eval-plan%20-%20ok.md) (shared scalar const-eval engine — the MIR fold pass reuses its arithmetic core). **Technically unblocked today by plan 12 alone.** |
| Project gate  | **Sequenced right after the pipeline first emits a runnable executable FILE** (per directive): the optimizer enters the [`00`](./00-compiler-implementation-plan%20-%20tbd.md) stepper after **LK10** (hermetic end-to-end — a config-driven build that writes an on-disk executable that loads + runs) **+ the driver's file-emission wiring**. **Accurate state 2026-05-30:** the linker today produces only *in-memory* `LinkedImage::bytes` (test-validated) + intra-module-reloc in-memory images (LK6 c1) — **no on-disk executable file is emitted yet**. Rationale: finish "it runs everywhere" (a real file that loads + runs) before making it run *fast* everywhere. |
