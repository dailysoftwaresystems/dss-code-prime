...
| OPT4  | Copy-prop + dominator-tree extraction      | ⏳ **NEXT (planned)** | —           | — |
...
† **SimplifyCFG is a recurring cleanup, not a linear OPT-N step** — hence the `OPT5+` label rather than its own integer (renumbering LICM→Autotuner would churn name-stable pins for no gain). Implemented **once** in the cycle after OPT5 (CSE/GVN), carrying the load-bearing **StructCfMarker repair** (D-OPT4-1, §2.7 — re-derive marked regions post-transform so WASM/SPIR-V codegen stays intact; this is the DSS-specific constraint LLVM's SimplifyCFG doesn't have). **Thereafter a recurring pipeline citizen**: listed multiple times in `*.pipeline.json` and run to fixed point interleaved with ConstFold + DCE — ConstFold turns `if (5 < 3)` → `if (false)`, SimplifyCFG deletes the dead arm, DCE removes the now-unreachable code, exposing more folding (the classic mutually-enabling cleanup cluster). See the §3 SimplifyCFG row + `D-OPT-FIXED-POINT-LOOP`.
...
|---|-------|----------|---------|-------------|
| OPT4 c1 | **Copy-prop** | Operand-rewrite via dominator-tree-gated substitution. First fixed-point pass (D-OPT1-PASS-RUN-MAX-ITER triggers). | D-OPT-DOMTREE-EXTRACTION ready | `D-OPT-MIR-REBUILDER-EXTRACT`, `D-OPT-DOMTREE-EXTRACTION`, `D-OPT1-PASS-RUN-MAX-ITER`, `D-OPT-FIXED-POINT-LOOP` |
| OPT5 c1 | **CSE / GVN** | Value-numbering + redundant-computation elimination. Consumes copy-prop's dominator tree. | OPT4 ✅ | `D-OPT1-CSE-NONCOMMUTATIVE-PIN` activates |
| OPT5+ c1 | **SimplifyCFG (cfg-simplify)** | Unreachable-block elim + block-merge + jump-thread + branch-fold (MIR-tier), **with StructCfMarker repair** (D-OPT4-1 — the load-bearing piece: re-derive marked regions post-transform so WASM/SPIR-V stay intact). Implemented right after CSE/GVN; thereafter a **recurring** pipeline pass (multiple `pipeline.json` entries, run to fixed point with ConstFold+DCE). NOT a linear OPT-N step — see the §0 `OPT5+` footnote. | OPT5 ✅ + `D-OPT-FIXED-POINT-LOOP` | `D-OPT4-1` (marker repair) + `D-OPT4-2` (no-volatile branch-fold bound) + `X_StructCfMarkerBroken` |
| OPT6 c1 | **LICM** | Loop-invariant code motion. Requires natural-loop detection on top of dominator tree. | OPT4/5 ✅ + loop-nest substrate | `D-OPT1-LICM-CONDITIONAL-PIN` activates; `D-OPT-RESULT-CHANGED-FIELD` triggers (LICM moves, doesn't delete) |
| OPT7+ | **Inlining / scheduling / peephole / vectorization / autotuner** | Per plan 22 §3 PR roadmap. | OPT2–OPT6 baseline | enumerated in §3 + §3.1 |
...
| D-OPT-PASS-METRICS | **Per-pass metric reporting via an optional `OptMetricSink*` parameter.** When per-pass counters become a consumer concern (profiling / diagnostic / autotuner), add `void onPassComplete(PassId, std::size_t mutationCount)` to a virtual `OptMetricSink` base + thread `OptMetricSink* sink = nullptr` through `optimize()`. Production callers pass null (zero allocation overhead); profiling callers pass a concrete sink. Do NOT bake `vector<PerPassMetric>` into `OptResult` — would tax every compile with an allocation no consumer reads today. | **First consumer of per-pass metrics** (autotuner / diagnostic introspection). | First request for per-pass metric visibility. |
