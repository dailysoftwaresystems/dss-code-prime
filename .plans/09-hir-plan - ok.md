...
| Predecessors  | ✅ [`08-compilation-unit-plan`](./08-compilation-unit-plan%20-%20tbd.md) (CU1–CU4 done; CU5 multi-language reserved — not needed until HR11). ✅ [`08.5-substrate-prep-plan`](./08.5-substrate-prep-plan%20-%20ok.md) SP1 + SP2 (the generalized arena substrate + core type lattice HR1 instantiates). ✅ Phase #8 semantic SE1–SE7 (HIR consumes `UnitAttribute<TypeId>` + `UnitAttribute<SymbolId>` from the SemanticModel). |
| Successors    | [`10-source-translation-plan`](./10-source-translation-plan%20-%20tbd.md) walks HIR. [`12-mir-lir-plan`](./12-mir-lir-plan%20-%20ok.md) lowers HIR→MIR. [`17-shader-gpu-plan`](./17-shader-gpu-plan%20-%20tbd.md) lowers shader-shaped HIR→SPIR-V. [`18-wasm-plan`](./18-wasm-plan%20-%20tbd.md) lowers HIR (with structured-CF markers from MIR) to WASM bytecode. [`11-ffi-plan`](./11-ffi-plan%20-%20tbd.md) emits extern decls as HIR nodes. |
...
Extension HIR kinds are **declared per-language** in the schema (the `hirLowering` block, schema v4 — additive sibling of `semantics`/`imports`/`numberStyle`/`typeExtensions[]`), mirroring how shader/SQL extension *types* are declared via `typeExtensions[]` per [08.5 SP2](./08.5-substrate-prep-plan%20-%20ok.md). Both follow the open core + per-language registered-extensions pattern.
