...
|-------|-----|-------------------|--------------|
| G-101 | Parser driver shipped (PA0–PA4 ✅, G-001 detail). Substrate, iterative RD driver, Pratt walker, recovery + diagnostic UX, and real-world corpus stress all landed; PA5a/PA5b LSP remain. | `src/analysis/syntactic/parser.{hpp,cpp}` + 36 ctest suites | [`05-parser-plan - tbd.md`](./05-parser-plan - tbd.md) PA0–PA4 ✅, PA5a–PA5b ⏳ |
...
| G-104 | ✅ Array indexing `a[0]`. Closed by parser-plan PA4 — grouped-postfix `[` with `endsAt: "]"` + `bodyRule: expression`, routed through `prattWalker->walkExpression` so operator climb runs inside the brackets. | `ParserCSubsetSmoke.{ArrayIndexParsesAsPostfix, ArrayIndexBodyClimbsPrecedence}` | parser-plan PA4 ✅ (v2-gap-catalog row 12) |
