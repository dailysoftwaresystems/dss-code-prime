...
> ✅ **CLOSED — phase #8 shipped (SE1–SE7, plan [`08.6-semantic-plan - ok.md`](./08.6-semantic-plan%20-%20ok.md)).** One language-agnostic `analyze()` engine driven by the `semantics` schema-v4 block produces a `SemanticModel` (symbol table + scope tree + node→SymbolId/TypeId + `S_*` diagnostics). G-201 stack-of-hashmaps scope tree ✅; G-202 scope resolution ✅; G-203 type representation = the SP2 `TypeKind`/`TypeId` lattice ✅; G-204 type checking (assignment + call arg + **return-type**, gap-closure A) ✅; G-205 const-correctness ✅; G-206 typedef ✅; G-207 function overloads + builtin functions ✅; G-208 cross-CU resolution via `crossRefs[]` injection ✅; G-209 forward refs (two-pass) ✅; G-210 unused-var **still deferred to optimizer**; G-211 `S_*` codes ✅ (UndeclaredIdentifier/RedeclaredSymbol/TypeMismatch/NotCallable/ArgCountMismatch/UnknownType/ConstViolation/ReturnTypeMismatch/ControlOutsideLoop); G-212 death-test + fail-loud discipline ✅. **Pre-HIR gap-closure (2026-05-26)** added beyond the original list: return-type checking, duplicate-parameter detection, break/continue-outside-loop (`S_ControlOutsideLoop`), tsql bracket-id `[Name]` name matching, in-memory/include dedup, split `# include`. Everything config-driven (no per-language C++).
