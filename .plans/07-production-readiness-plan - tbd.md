...
| G-002 | ✅ **Semantic phase designed; SE1 next.** [`08.6-semantic-plan - tbd.md`](./08.6-semantic-plan%20-%20tbd.md) specifies one language-agnostic `SchemaDrivenSemantics` engine driven by a schema v4 `semantics` block. SE1 lands the engine + toy config; SE2/SE3 are pure-config c-subset/tsql onboardings. Closes G-201..G-212 on completion. | Required for any non-trivial codegen. Type errors are the most common real-world diagnostic. |
