...
| G-005 | ✅ **Project-config format SHIPPED (AP2 + AP4, 2026-06-09).** ~~No project-config format. Driver layer is unspecified; users can't actually invoke the compiler against a real project.~~ `dss-code-prime --project <file.dss-project.json>` is a real entry point (`src/program/cli_args.cpp`, `src/core/types/project_config.{hpp,cpp}`) declaring language / artifactProfile / targets / sources — it is the manifest that builds full-source SQLite (`D-AP2-SOURCES-GLOB` source globs, `D-AP2-OUTPUT-ROUTING` output routing). | Without this, every other artifact has no entry point — ✅ satisfied. |
| G-006 | ✅ **Compilation-unit model SHIPPED (CU1–CU4; phase #7.5 complete).** ~~No compilation-unit model. Parser produces one Tree per file; nothing bundles them. Semantic / IR / codegen would each invent their own answer.~~ `CompilationUnit` / `UnitBuilder` / `CrossTreeRef` / `CompilationUnitId`, CU-scoped `SymbolId`, `UnitAttribute<T>` and the import resolver all live under `src/analysis/compilation_unit/`. | See [`08-compilation-unit-plan - tbd.md`](./08-compilation-unit-plan - tbd.md) — phase #7.5 between parser and semantic. ⚠ That plan's own Status row has read "CU1 + CU2 + CU3 + CU4 shipped" since 2026; this row contradicted it until 2026-08-04. |
...
| ID    | Gap | Notes |
|-------|-----|-------|
| G-520 | C runtime linkage on Windows. MSVCRT? UCRT? | **ANSWERED: UCRT — and the migration is IN FLIGHT, tracked by `D-FFI-PE-CRT-UCRT-MIGRATION` (phased P0–P5).** The question is no longer open; what remains is finishing the move. History: pe shipped against legacy `msvcrt.dll`, which is a C89 runtime and was accumulating C99 gaps one surgical patch at a time (math, then `strftime`), so the honest fix is the whole CRT. Landed: P0 pre-flight (measured that `ucrtbase` self-initializes heap/stdio/locale on DLL-attach, so no synthesized CRT init is needed), P1 stateless descriptor flips (ctype/memory/tgmath/fcntl), P2 the `_time64` family. ★ NOTE the note above is wrong on one point: no import library is involved — DSS binds a DLL's EXPORT table directly (`--resolve-library` reads `.edata`), so `ucrtbase.dll` is named straight in the descriptor's `library.pe`. ★ The load-bearing constraint this gap's one-line framing hides is the **CO-STATE GROUP**: a heap pointer / `FILE*` / CRT fd / `errno` / static `tm` produced by one CRT and read by the other corrupts SILENTLY (proven the hard way in P2 — flipping `sys/stat.json` alone passed a fully green 720/720 3-leg gate and still crashed sqlite at `vtabH-3.5`, because `fstat()` was handed an fd minted by the *other* CRT). So the stateful descriptors must flip TOGETHER, and `tests/ffi/test_pe_crt_costate_binding.cpp` now fails loudly in 0.03s if one drifts. Blocks nothing else; see the registry row for the live phase state. |
...
| G-711 | **Exhaustive-switch enforcement (warnings-as-errors latch).** No `-Wswitch`/`-Werror` is configured today, so the codebase-wide open-core dispatch idiom — an exhaustive `switch` over an enum with **no `default`** + a trailing "unreachable" return (`HirKind`/`TypeKind`/`HirOpKind` dispatch, `requiresValidType`, `arityOf`, `opName`, every `*Name()` / lowering switch) — is **convention-only, not compiler-enforced**. A future core enum member added without updating a switch silently returns a plausible-but-wrong value: e.g. `requiresValidType(NewExprKind)` → `false` makes the HIR verifier silently skip a genuinely untyped node; `arityOf(newBinaryOp)` → `Binary` mis-classifies. This is the one class of regression the "make every regression visible" posture currently can't see. | **✅ CLOSED 2026-08-15 (AP6 close-out).** Surfaced by the HR2 5-agent review (silent-failure-hunter, conf 72; 2026-05-26) and **pending ~3 months while the narrow form was implemented FIVE separate times** — per-file `set_source_files_properties` ratchets in `src/asm`, `src/opt`, `src/lir`, `src/core` and `src/mir/lowering`, covering 8 hand-listed files. ★ **The file that shipped the defect fell in the gap between them**, which is the argument for the tree-wide latch this row asked for, made by the codebase rather than by a reviewer. **Action taken, exactly as stated here:** `-Werror=switch` (GNU/Clang) and `/we4062` (MSVC) at ONE chokepoint — `CMakeLists.txt:220-227`, after `FetchContent_MakeAvailable` — and the five per-file ratchets RETIRED (`set_source_files_properties` with switch flags: 0 remaining), their anchor-bearing comments left pointing at the chokepoint with an explicit "do not add a sixth". ✔**COVERAGE PROVEN:** 434/434 of our TUs, 0/4 googletest TUs. ✔**MSVC MECHANISM PROVEN SEPARATELY** — `/we4062` maps to `TreatSpecificWarningsAsErrors`, a different element than the retired `AdditionalOptions`, and C4062 is off-by-default, so an isolated probe confirmed `error C4062` + rc=1 against an rc=0 negative control. ✔**RED-ON-DISABLE ON A REAL DEFECT:** `asm_text_to_lir.cpp(2577,9): error C4062: enumerator 'CfClass::Switch' … is not handled` — an unhandled control-flow enumerator previously visible only on macOS is now a build error on the local MSVC gate. ✔**ZERO FALLOUT:** gcc full-tree rc=0 over 703 targets, 257 warnings = baseline exactly (delta 0); `-Wswitch` tree-wide = 0 occurrences. ⛔ **THE STRETCH GOAL IS MEASURED INFEASIBLE AND IS HEREBY WITHDRAWN, NOT DEFERRED:** `-Wswitch-enum` / `/we4061` produces **45,272** warnings tree-wide, dominated by `src/mir/mir_opcode.hpp` re-warning once per inclusion (18,666). It cannot be adopted incrementally at that volume and nothing should re-propose it without a per-TU scoping design. ⓘ Also measured while here, and deliberately NOT taken in this PR: `-Wall` costs +87 (24 files, 13 in `src/`) and `-Wall -Wextra` +782 (686 of them one category in 3 files) — a very doable dedicated follow-up, declined only because a build-flags PR should not churn 24 files while sibling lanes are mid-edit. **Cross-ref:** [[D-BUILD-CLANG-ONLY-WARNINGS-INVISIBLE-TO-THE-MSVC-AND-GCC-LEGS]] — the row that forced it, and whose own "clang-only" diagnosis this disproved: the tree passed no `-Wall`/`-Wextra`/`/W4` at all, so nobody had asked gcc or MSVC the question. |
...
| ID    | Gap | Notes |
|-------|-----|-------|
| G-730 | `tests/corpus/c/` — ~5 real programs covering pointer ops, function calls, arrays, control flow, struct (if struct lands in c-subset; otherwise plain functions + locals). | parser-plan PA4 prereq. |
...
| G-810 | ✅ **The C PREPROCESSOR ITSELF SHIPPED** — `src/analysis/preprocess/preprocessor.cpp` + `pp_if_eval.cpp`: `#include`, object- and function-like macros, conditionals, `#line`, `_Pragma`. It is what compiles full upstream SQLite (189 TUs). ⚠ **RE-SCOPED 2026-08-04 — what stays reserved is the FFI HEADER-MODE CONSUMER only:** running the preprocessor *inside* `readCHeader`. ~~(post-v1) C preprocessor for header mode (`#include` + function-like macros).~~ | ~~Reserved.~~ Pre-reduced headers under `src/dss-config/ffi-headers/` were removed OPT2 cycle 1 (2026-06-03, commit `8bae225`) as dead-code; ⚠ the "(no production caller)" half is now STALE — `src/ffi/ingest.cpp` calls `readCHeader` / `readCHeaderDirectory` on the live ingestion path. `src/ffi/c_header_parser.cpp` states in-code that it resolves data-model differences "downstream against the real format rather than through the preprocessor", which is exactly the residual this row still owns. |
