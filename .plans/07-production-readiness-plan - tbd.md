...
| G-005 | ✅ **Project-config format SHIPPED (AP2 + AP4, 2026-06-09).** ~~No project-config format. Driver layer is unspecified; users can't actually invoke the compiler against a real project.~~ `dss-code-prime --project <file.dss-project.json>` is a real entry point (`src/program/cli_args.cpp`, `src/program/project_config.{hpp,cpp}`) declaring language / artifactProfile / targets / sources — it is the manifest that builds full-source SQLite (`D-AP2-SOURCES-GLOB` source globs, `D-AP2-OUTPUT-ROUTING` output routing). | Without this, every other artifact has no entry point — ✅ satisfied. |
