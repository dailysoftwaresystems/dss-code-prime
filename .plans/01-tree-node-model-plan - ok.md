...
tree.hpp/.cpp                ← detail::TreeData, Tree (immutable, schema- & diagnostics-aware accessors, cursor()/astCursor() entry points)
...
| T8  | ✅ done | `tree-attrs`       | `NodeAttribute<T>` side-tables | `tree_attrs.hpp`, `tests/core/test_tree_attrs.cpp` | Header-only `NodeAttribute<T>` bound to a `Tree const&` (stashes `&tree` + `tree.id()`). Storage is `std::variant<unordered_map<NodeId,T>, vector<optional<T>>>` — starts sparse, auto-promotes to dense when coverage ≥ 50% AND `nodeCount() ≥ 16`. No demotion; `clear()` resets to sparse. Public API: `set`/`erase`/`clear`/`reserve`, `has`/`get`/`tryGet` with both const and mutable overloads (in-place mutation supported), `size`/`empty`/`isDense`/`tree`. Forward iterator template `Iterator_<bool IsConst>` over both backings via internal `std::variant<MapIter, (vec*, idx)>`; yields `std::pair<NodeId, T&>` by value (the reference inside the by-value pair aliases live storage, so `for (auto kv : attr) kv.second.mutate();` works); order unspecified. Bounds check (`id.valid() && id.v < nodeCount()`) on every entry → `attrFatal` aborts on violation. **Cross-tree guard — closed in SH3**: NodeId now carries a `treeTag`, `validateId_` aborts on tag mismatch via `crossTreeFatal(boundTreeId, idTreeTag)` (both ids in the message). Move-only with custom move ops that reset the source's `denseCount_` + variant so the moved-from observable state is internally consistent (rather than relying on the std-lib's "valid but unspecified" contract). **67 test cases** (40 functional + 13 original `EXPECT_DEATH` + 14 SH3 cross-tree death tests) covering: two-tree-id disambiguation, const-overload return-type `static_assert`, sparse + dense modes, promotion floor boundary at exactly nc=16 (and nc=15 stays sparse), 50% threshold at even (nc=22) and odd (nc=17) node counts, promotion preserves values + survives mid-life mutation, dense-mode new-entry tracking, erase-below-threshold does NOT demote, promotion is set-only (never triggered by has/get/erase), mutation through get/tryGet/iteration in both modes, dense iteration with internal `nullopt` gaps, sparse iteration after erase, move-only `T = unique_ptr<int>` across promotion, move-construct + move-assign of the attribute leave source with `size()==0`, reserve is a safe no-op in dense mode, every bounds-violation path on every API, absent-NodeId fatals in both backings, empty-tree fatals on every entry point, AND cross-tree set/get/has/tryGet/erase/iterator all abort with both TreeIds in the message. |
| T9  | ✅ done | `tree-views`       | Initial typed views | `tree_views.hpp`, `well_known_names.hpp`, `tests/core/test_tree_views.cpp` | Seven header-only views: token-level **IdentifierView** + **LiteralView** (with cached `Kind` enum: Int/Float/String/Char/Bool/Null), plan-named rule-level **BinaryExprView** (`lhs`/`op`/`opNode`/`rhs`), **BlockView** (`statementCount`/`statementAt`/`statements`), **FunctionDeclView** (`name`/`paramList`/`body` — chains to `IdentifierView::name()`), plus toy-aligned **VarDeclView** (`name`/`value`) and **ExprStmtView** (`expression`). Each view: trivially copyable (Tree* + NodeId + maybe 1 byte), public unchecked ctor, `static std::optional<View> from(tree, id)` factory that rule/token-kind-checks via `tree.rules().find(name)` / `tree.schema().schemaTokens().find(name)` (returns `nullopt` on mismatch — incl. cleanly on schema-less trees via the new `Tree::hasSchema()` accessor + `tokenKindFor`'s short-circuit). Structural accessors use `detail::views::nthVisibleChild` which skips `NodeFlags::EmptySpace` children, so whitespace between role positions doesn't break indexing. Rule + token names live in `well_known_names.hpp` (single source of truth, shared with future engine code). Lifetime contract matches `TreeCursor`/`NodeAttribute`. **36 test cases** covering: every `from()` accept/reject path including `FromOnInvalidNode` and `FromOnSchemaLessTree` symmetry across all 7 views, all 6 LiteralView kinds + rejection of both Identifier-kind and operator-kind tokens, unchecked LiteralView on non-literal aborts via `viewFatal`, BinaryExprView/FunctionDeclView/VarDeclView/ExprStmtView accessor correctness WITH and WITHOUT EmptySpace tokens interspersed, BlockView with 0/1/3 statements + whitespace, `span()` assertions on every rule-level view (pins span-of-rule vs span-of-child), the chained-unchecked-`IdentifierView` hazard documented for `FunctionDeclView::name()` (proves the wrong-shape tree returns garbage text via the unchecked path AND that `IdentifierView::from(nameNode())` correctly rejects), VarDeclView/ExprStmtView via the real `toy.lang.json` schema, `static_assert` view triviality + size bounds, `std::optional` reassignment. Also adds `Tree::hasSchema()` / `Tree::hasDiagnostics()` as the canonical "probe before dereference" accessors. |
| T10 | ✅ done | `tree-end-to-end`  | Toy parser + walker demo + broken-input demo | `tests/core/test_tree_end_to_end.cpp` | End-to-end integration test for the full core stack: `GrammarSchema::loadShipped("toy")` → `SourceBuffer` → `TreeBuilder` driven by a `TokenSeq` helper → `Tree` → `walkPreOrder` in AST mode → indented `prettyPrint(rule:<name>` / `tok:"<text>"`) → string-equality assertion against an inline expected literal. **9 tests**: 3 happy-path full-print assertions (`varDecl`, `exprStmt`, multi-statement — each also asserts `diagnostics().all().empty()`; the multi-statement test additionally scans `t.children(root)` and asserts exactly 2 EmptySpace tokens to lock whitespace attachment), 1 views-resolve-on-real-parse (chains `VarDeclView::from` / `ExprStmtView::from` using `detail::views::nthVisibleChild` for forward-compatible role indexing; asserts captured names `[varDecl:x, exprStmt:y, varDecl:w]`), and 5 broken-path flavors: (a) `BrokenPath_UnknownTokenRecovered` — pushes `@` mid-`varDecl`, asserts ≥1 `P_UnknownToken` + `hasError(root)` + ≥1 Error-flagged descendant + tree still walks; (b) `BrokenPath_UnclosedScopesAtEof` — finishes with 3 frames still open (heap-held `OpenScope` guards dropped post-finish), asserts exactly 3 `P_PrematureEndOfInput` + `hasError(root)`; (c) `BrokenPath_TruncatedAfterKeyword` — single-token input, asserts exactly 3 `P_PrematureEndOfInput`; (d) `BrokenPath_PushErrorRecovered` — exercises the builder's explicit `pushError` API, asserts ≥1 `P_UnexpectedToken` + `hasError(root)` + ≥1 Error-flagged descendant; (e) `BrokenPath_PopScopeUnderflow` — pushes `}` token with no opener, exercises the schema's `closesScope` effect → empty-stack `popScope` → `P_BuilderInvariant`. NB: `P_UnmatchedClose` / `P_UnclosedScope` codes exist in the enum but are NOT emitted by the current T5 implementation — reserved for a future parser enhancement. |
| T11 | ✅ done (rolling) | `tree-cmake-wireup` | Per-checkpoint `src/core/CMakeLists.txt` + `tests/core/CMakeLists.txt` updates | `src/core/CMakeLists.txt`, `tests/core/CMakeLists.txt` | Folded into each chunk as files land. Final audit at end of T10: 11/11 `src/core` .cpp files registered, 16/16 `tests/core` test .cpp files registered, 22/22 headers consumed (by sources, tests, or other headers — header-only T7/T8/T9 additions consumed only by tests, expected until parser/IR-gen lands), DSS_EXPORT applied to every cross-DLL-boundary class/struct with out-of-line members, templates/inline-only types/enums correctly omit it. Zero orphan files. |
| T12 | ✅ done | `tree-docs`        | Header docs + design docs | `docs/tree-model.md` + `docs/language-config-spec.md` + cookbook-pin test in `tests/core/test_grammar_schema.cpp` | Two onboarding-focused markdown docs (~5 pages each), copy-pasteable cookbook examples for both "add a typed view" and "author a `.lang.json`" workflows. The language-config-spec cookbook (a complete "Calc" language) is pinned by `GrammarSchema.DocsCookbookCalcExampleLoadsCleanly` so the doc's "loads cleanly" claim is verified by CI. Inline doxygen was punted: the codebase uses regular C++ `// ...` comments throughout, no doxygen build wiring exists, and the acceptance criterion is the markdown docs — the existing header comments are already WHY-focused per the repo convention. |
