...
| 12 | ~~Arrays `int a[10]; a[0] = 1;` — no index-postfix shape.~~ **Closed by parser-plan PA4** for the expression side. c-subset added postfix `[` with `endsAt: "]"` + `bodyRule: expression`; the walker routes the body through `prattWalker->walkExpression` so operator climb runs inside the brackets (`a[i + j * k]` builds `binaryExpr` correctly). Array-declarator side (`int a[10];`) still needs a `typeRef`-suffix shape — that's c-subset authoring work, not a new mechanism. | `operators` postfix `[` (parser side) | n/a                                                | **parser-plan PA4 ✅** (expression side); declarator side defers to c-subset shape extension | `a[0]`, `a[i + j * k]` (expression side ✅); `int a[10];` ⏳ |
