...
|---|--------------------------------------------------------------|---------------------------------------------|-----------------------------------------------------------------------|---------------------------------------|-----------------------------------------|
| 1 | ~~`a + b * c` parses left-fold.~~ **Schema-side data landed in PR1**: `c-subset.lang.json` declares the full C precedence table via `operators.groups`. **Driver shipped in PA1 ✅** but the flat-fold `PrattWalker` interface is the stub — tree shape is still wrong on mixed-op expressions because PA1 doesn't invoke a precedence-climbing walker (PA2 wires it). Test `CSubsetEndToEnd.ExpressionWithMixedOpsIsLeftFolded` stays pinned until then; `CSubsetEndToEnd.OperatorTableMatchesCPrecedence` proves the data is in place. | `shapes/expression`, `shapes/binaryOp`, `operators.groups` | none — loader accepts | **§5.1 (PR1) — data ✅**; **PA1 driver ✅**; tree-shape flip awaits PA2 Pratt walker | `1 + 2 * 3;`, `a = b = c;` |
...
| 26 | c-subset has no function calls (row 7 above) and no array indexing (row 12). The Pratt walker's postfix arity (parser-plan PA2) closes both — but the c-subset config also needs `tokens` + postfix operator declarations. | parser-plan PA4 + operator-table postfix arity. Schema mechanism is already there from PR1. | G-103, G-104 |
...
| 29 | No `#include` / module system. c-subset deliberately omits the preprocessor; full C99 is post-v1. But c-subset still needs *some* way to declare external functions (e.g., the C runtime's `printf`) — at minimum, an `extern` declaration shape. | c-subset shape extension. `extern` keyword + decl-without-body shape. Likely lands parser-plan PA4. | G-110 |
...
|-------|---------------------------------------------|--------------------------------------------------|
| 1     | Row 1 (precedence tree shape)               | parser-plan PA2 — Pratt walker lands             |
| 2     | Row 6 (`*` prefix-on-lvalue / postfix-after-typeRef) | parser-plan PA2 — prefix arity for `*`     |
| 3     | Row 7 (function calls `f(x,y)`)             | parser-plan PA2 — postfix arity for `(`          |
| 4     | Row 12 (array indexing `a[0]`)              | parser-plan PA2 — postfix arity for `[`          |
| 5     | Row 8 (postfix `x++` `x--`)                 | parser-plan PA2 — postfix arity for `++`/`--`    |
| 6     | Row 16 (compound assignment `+= -= *= /=`)  | parser-plan PA2 — right-assoc binary slot        |
| 7     | Row 29 (extern decl in c-subset)            | parser-plan PA4 — c-subset shape extension       |
| 8     | Row 25 (struct/union/enum in c-subset)      | parser-plan PA4 — c-subset shape extension       |
