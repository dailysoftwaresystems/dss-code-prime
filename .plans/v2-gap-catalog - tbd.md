...
| 19 | **Body-lexed literal VALUES (char/string) cannot be captured as a CST subtree, so they cannot be lowered to a typed literal.** Surfaced in HR9 (HIR plan §HR9, 2026-05-27): a body mode (`charBody`/`string`) emits its `defaultToken.kind` (`CharLiteral`/`StringChar`) **per byte**, and those kinds are **off-grammar by design** — the loader rejects referencing them from a `shape` (`C_BodyDefaultKindInShape`, 0xC029, see TZ3), and the builder's `bodyDefaultTokenKinds_` skips them in the schema cursor. So `operand`'s `CharStart`/`StringStart` alternative matches only the opener; the body + closer **spill into the enclosing frame as off-grammar trailing tokens** (verified: for `return 'a';` the `CharLiteral` body tokens are children of `returnStmt`, not `operand`). Reaching into the parent is ambiguous (`f('a','b')` interleaves bodies). The char/string **TYPE** resolves fine (`char`→Char); only the **value** is blocked. | `lexerModes.*.defaultToken`, `shapes/operand`, tokenizer body-mode loop | n/a — fails loud (`H_UnsupportedLoweringForKind`); never a silent miscompile | **tokenizer follow-up TZ4** (plan 04 §"Deferred follow-ups"): coalesce a body-mode run into ONE token spanning the literal, OR emit a distinct close delimiter (e.g. `CharEnd`/`StringEnd`) on mode-pop so `operand` can capture `[Start, body…, End]` as a subtree. Then HR9-style lowering reads a clean subtree. | `'a'`, `'\n'`, `"hello"` |
