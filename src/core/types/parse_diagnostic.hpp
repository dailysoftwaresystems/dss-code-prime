...
enum class DiagnosticCode : std::uint16_t {
    ...
    // D-PP-DEFINED-VIA-MACRO-EXPANSION (C 6.10.1): the `defined` operator in an
    // `#if`/`#elif` controlling expression was PRODUCED BY MACRO EXPANSION
    // rather than written in the directive. Warning severity, and translation
    // CONTINUES with the operator evaluated — DSS answers the question, it does
    // not refuse it.
    //
    // ★ WHY DIAGNOSE SOMETHING WE DELIBERATELY SUPPORT. C 6.10.1 makes the
    // result UNDEFINED when the `defined` token is generated by macro
    // replacement, so a program relying on it is relying on a consensus rather
    // than on the standard. ✔MEASURED 2026-08-27, all three references warn and
    // continue, on every shape: gcc 13.3.0 `-Wexpansion-to-defined` ("this use
    // of \"defined\" may not be portable"), clang 18.1.3 `-Wexpansion-to-defined`
    // ("macro expansion producing 'defined' has undefined behavior"), MSVC 19.51
    // `C5105` (same wording as clang), the last in BOTH the traditional and
    // `/Zc:preprocessor` preprocessors. Evaluating without saying so is the one
    // behaviour NONE of the three has.
    //
    // ★ SUPPRESSABLE, DELIBERATELY, and the closed table's own rule is what puts
    // it here rather than there: this is advice about a construct that is legal
    // by consensus and that real system headers use ON PURPOSE (Apple's
    // `secure/_string.h` `__is_modern_darwin`). Suppressing it ships no wrong
    // bytes and hides no build failure — `hasErrors()` is untouched by a warning
    // — so it fails BOTH prongs of the unsuppressable membership rule and is
    // pinned there as a NEGATIVE, beside `P_PreprocessorWarningDirective` and
    // `S_DeprecatedSymbolUsed`. `--warnings-as-errors` still promotes it, which
    // is the correct lever for a project that wants the construct out.
    //
    // ★ REACHABILITY, the `#error`/`#warning`/`#pragma` invariant, and it is not
    // optional here either: the operator is only ever evaluated for a LIVE
    // `#if`/`#elif`, so a macro-produced `defined` inside a NOT-TAKEN branch is
    // entirely silent (C 6.10p1). The SDK headers park these inside
    // unsupported-configuration branches by the hundred.
    P_PreprocessorDefinedFromExpansion = 0x0023,

    // D-PP-IF-LARGE-DECIMAL-LITERAL-HAS-NO-WARNING. A DECIMAL, UNSUFFIXED
    // integer literal in an `#if`/`#elif` controlling expression whose magnitude
    // exceeds INTMAX_MAX. C 6.10.1p4 evaluates phase 4 in intmax_t/uintmax_t, so
    // the decimal-unsuffixed ladder (int/long/long long — every candidate
    // SIGNED) runs out of candidates and the literal is taken as UNSIGNED. That
    // silently flips the sense of any comparison against a negative operand, and
    // both references say so out loud rather than let it pass.
    //
    // ✔MEASURED 2026-08-27, gcc 13.3.0 and clang 18.1.3 probed SEPARATELY at
    // `-std=c2x`, no `-W` flags and no `-pedantic` — this is ON BY DEFAULT in
    // both:
    //   gcc   "integer constant is so large that it is unsigned"
    //   clang "integer literal is too large to be represented in a signed
    //          integer type, interpreting as unsigned"
    //          [-Wimplicitly-unsigned-literal]
    // Both then take the SAME (unsigned) branch and exit 0. Unanimous ⇒ the
    // union rule makes the WARNING required, not merely permitted, and DSS's
    // silence was the gap.
    //
    // ⚠ THE THREE CONDITIONS ARE ALL LOAD-BEARING AND EACH IS PINNED BY A
    // MEASURED NEGATIVE, because a warning DSS invents where neither reference
    // warns is as much a divergence as one it omits:
    //   * SUFFIXED — `18446744073709551615u`: neither warns (the `u` rule's
    //     candidates are unsigned, so nothing was reinterpreted).
    //   * NON-DECIMAL — `0xFFFFFFFFFFFFFFFF`, `0x8000000000000000`: neither
    //     warns. C 6.4.4.1 gives a HEX literal unsigned candidates outright, so
    //     again nothing is reinterpreted.
    //   * AT OR BELOW INTMAX_MAX — `9223372036854775807`: neither warns. The
    //     boundary is exact: `9223372036854775808` DOES warn in both.
    //
    // ⚠ NOT the >64-bit-literal case, which is a settled and DIFFERENT split
    // (gcc warns and substitutes a truncated value, clang refuses; DSS refuses
    // with clang). That literal never reaches here — `decodeInteger` has already
    // failed loud.
    //
    // ★ SUPPRESSABLE, by the closed table's own rule: it is advice about a
    // construct both references accept and compile, it ships no wrong bytes on
    // its own, and it hides no build failure. Same membership answer as
    ...
    // C 6.5.16p2: "An assignment operator shall have a modifiable lvalue as its
    // left operand." The LEFT operand of `=` (or of a compound assignment) is a
    // manifest RVALUE — an integer/character literal, an arithmetic result, a
    // call, a cast, a comparison — so there is no object to store into.
    // D-CSUBSET-ASSIGNMENT-TO-A-NON-LVALUE-REFUSED-BY-THE-LOWERING-TIER
    //
    // ★ A DISTINCT CODE FROM `S_IncDecNeedsModifiableLvalue` (0xE024), on this
    // header's own stated grounds — the message must match reality. That code
    // names `++`/`--` and C 6.5.2.4/6.5.3.1; a reader who wrote `8 = 5;` and is
    // told about an increment operator has been sent to the wrong clause.
    //
    // ⚠ WHAT IT REPLACED IS THE POINT, AND IT WAS MEASURED, not inferred. Before
    // this code existed the assignment path had NO lvalue-shape guard at all:
    // `classifyLvalue`'s fall-through arm took `AddressOf(<whatever lowered>)`
    // and deferred the question to MIR, where it surfaced as an internal
    // lowering complaint about whichever node kind happened to arrive.
    // ✔MEASURED 2026-08-27 (P42 lane AI) on x86_64:pe64-x86_64-windows-exec:
    // `8 = 5;` reported `H_UnsupportedLoweringForKind: string-literal
    // materialization: the Literal pool entry is not a string arm` — a message
    // about STRING literals, for an integer one; `(a+b) = 5;` and `f() = 5;`
    // reported `lvalue kind 'BinaryOp' (ordinal …)` / `'Call' (ordinal 27)`,
    // naming a raw internal ordinal. All three were REFUSED (never a
    // miscompile), which is why this is a diagnostic-quality defect and not a
    ...
    // D-PROGRAM-PROJECT-WIDE-PARSE-GATE-MASKS-CENSUS: THE RUN IS TELLING YOU
    // WHAT IT DID NOT MEASURE.
    //
    // ★★ THE FACT THIS CODE EXISTS TO CARRY: a diagnostic that is ABSENT from a
    // run is absent for one of two reasons that look identical on a terminal —
    // the condition did not occur, or the phase that would have reported it
    // never ran. Nothing in the output used to distinguish them, and an entire
    // arc read "no S0006 in the project build" as "DSS supports __uint128_t"
    // when the truth was "semantic analysis did not run for any of 189
    // translation units". This code makes the second reading impossible.
    //
    // Emitted at every altitude where the driver stops short of the full
    // pipeline, and the MESSAGE names the altitude, the counts, and which
    // phases did not run — because a reader who cannot tell a pre-CU-build stop
    // from a pre-analysis stop cannot tell which diagnostic families are
    // missing.
    //
    // ⚠ SEVERITY IS `Info`, AND THAT IS A CONTRACT, not a softening. The build
    // that triggers it is ALREADY failing on its own errors; this is a fact
    // ABOUT the diagnostic stream, not one more complaint in it. At Error it
    // would gate `hasErrors()` and so re-enter the very gates it describes; at
    // Warning `--warnings-as-errors` would fail a build for its own honesty.
    // Same reasoning, same shape, as `P_DiagnosticsElided`.
    //
    // ⚠ DELIVERY IS `Guaranteed` (the volume caps must not drop the notice that
    // the stream is incomplete) but the code is deliberately NOT a member of
    // the closed unsuppressable table: applying that table's own two-prong rule,
    // suppressing this ships no wrong bytes and hides no build failure — the
    // parse errors that caused it are still reported and the build still fails.
    // An operator who names the code has been told the fact exists and chosen
    // not to see it, which is exactly what `--suppress` is for. The defect this
    // closes was a silence NOBODY asked for.
    D_LaterPhasesNotRun          = 0xD02C,
