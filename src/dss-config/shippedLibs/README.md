...
`when: {dataModel: "ILP32"}` the LP64 text again, because DSS's C vocabulary
still gives `long` 64 bits there (the declared-only wasm32 and spirv formats,
where these descriptors are available too; whether DSS ships an ILP32 target
is `D-FFI-STDINT-PTR-WIDTH-ILP32`, and the arm moves with the vocabulary) —
see the `symbols` row above — and NO `default`, so a data model no arm names
is refused rather than handed another model's text. The active object format's `dataModel` picks one —
`pe` is LLP64, `elf`/`macho` are LP64 — and the reader parses EVERY arm on
...
**P68 round 9 (D-C-LIMITS-H-DEFINES-NINE-OF-THE-STANDARD-MACROS).** A `constants`
row may name a TYPE and a LIMIT instead of stating a number:

```jsonc
{ "name": "LONG_MAX",   "of": "long",    "limit": "max"   }   // per pair: 2147483647 (pe) or 9223372036854775807
...
value, and both are per (target, format) pair — `long` is 32 bits on pe and 64 on
ELF and Mach-O, plain `char` is unsigned on aarch64 Linux alone. A fixed row would
be right on some pairs and silently wrong on the rest, so the ONE constants decode
realizes a derived row from the pair (`ffi::ShippedPairFacts`, built by both seams
from the same `applyTargetFormatPair` inputs):
...
2. Add the symbol with its hir-text signature (see `docs/ir-type-text.md` for
   the vocabulary). If it involves `long`, give its `signature` an arm per data
