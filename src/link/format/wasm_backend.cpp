...
// is WASM — that is the whole reason the seam exists. What it must never
...
// `ObjectFormatBackend`, and they cannot even SPELL `ObjectFormatKind::Wasm`
...
// `if (kind == ObjectFormatKind::Wasm)` in the schema triple — same rules,
...
char const* const kRejectedRootFields[] = {
            ...
            // EXPRESS A WEAK DEFINITION. That claim is the capability shape
            // the anchor above refuses to encode in config; this row says only
            // that no reader of this key exists on this backend. If a WASM
            // weak-definition encoder lands, it arrives WITH its dialect row
            // and this entry goes — the same direction the list's other
            // entries move in.
            "weakDefinition",
};

class WasmBackend final : public ObjectFormatBackend {
public:
    [[nodiscard]] std::string_view configName() const noexcept override {
        return "wasm";
    }
    [[nodiscard]] ObjectFormatKind kind() const noexcept override {
        return ObjectFormatKind::Wasm;
    }
    // No identity block: this format declares nothing under a root key of its
    // own. The engine + JSON vocabulary arrive with plan 18.
    ...
    void validateIdentity(detail::ObjectFormatData const&,
        ...
        // No identity block to validate. Every WASM coherence rule that
...
ObjectFormatBackend const& wasmBackend() noexcept {
    static WasmBackend const instance{};
    return instance;
}
