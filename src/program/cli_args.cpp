...
std::string cliHelpText() {
    ...
    static std::string const text =
        ...
        "  --resolve-library <path>[=<import-name>]  point the build at a "
            "library it may resolve against (repeatable; typically a "
            "DSS-built one). The path is dispatched by MAGIC BYTES, never "
            "by extension, and the two kinds are treated differently. A "
            "DYNAMIC library (.so/.dll/.dylib): its export table is read to "
            "resolve + validate this build's externs against it, and a "
            "matching extern records a runtime import. A STATIC `ar` "
            "archive (.a/.lib): the members defining otherwise-unresolved "
            "symbols are PULLED and MERGED into the image, so their code "
            "ships inside the artifact instead of being imported. A dynamic "
            "library named alongside an archive ALSO re-binds the imports "
            "of the members that get pulled: an object file records an "
            "undefined symbol's NAME and nothing else, so a member built "
            "against a library reads back with no owner, and the binding is "
            "re-derived here — a library named on this flag outranks the "
            "platform default. The optional `=<import-name>` STATES the "
            "runtime identity to record (DT_NEEDED / LC_LOAD_DYLIB / PE "
            "import name) instead of the binary's own embedded soname — for "
        ...
        "\n"
        "Examples:\n"
        "  dss-code-prime --compile hello.c --language c "
            "--target x86_64:elf64-x86_64-linux\n"
        "  dss-code-prime --directory src/ --language c "
            "--target x86_64:elf64-x86_64-linux --config=release\n"
        "  dss-code-prime --dump-predefined-macros --language c "
