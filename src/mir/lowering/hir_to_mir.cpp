...
#include "mir/mir_literal_pool.hpp"   // forEachLiteralNode — the ONE heap literal walk
...
// → `MirAggregateValue`, D5.3) copies field-by-field so a folded aggregate
// `constInit` carries the same shape on the MIR side.
//
// ★★ D-MIR-HIRTOMIR-LITERAL-AND-AGGREGATE-CONST-WALKS-RECURSE-PER-BRACE-LEVEL
// (P55, lane `ag`). This was one host frame per BRACE LEVEL, uncapped — a
// TRANSFORM, so it could not simply borrow `forEachLiteralNode` (which visits
// one tree; this one BUILDS a second). It is an explicit frame stack instead:
// each frame is a source aggregate, the destination aggregate being filled, and
// how far through the fields it has got — the same activation record the
// recursion kept.
//
// ⚠ THE DESTINATION POINTERS ARE ONLY VALID BECAUSE EVERY `fields` VECTOR IS
// RESERVED TO ITS FINAL SIZE BEFORE ANYTHING IS PUSHED INTO IT. `reserve` then
// `emplace_back` up to capacity cannot reallocate, so a parked
// `MirAggregateValue*` stays live for as long as its frame does. Drop the
// reserve and this silently becomes a use-after-free.
//
// ⓘ NOT FIXED HERE, AND IT IS THE REAL REMAINING CEILING FOR A DEEP LITERAL:
// `~MirLiteralValue` is itself one host frame per level (`~variant` →
// `~MirAggregateValue` → `~vector` → `~MirLiteralValue`) and is GENERATED BY THE
// STANDARD LIBRARY, so nothing in this project can flatten it. Building the
// value on the heap is necessary but not sufficient; see the note in
// `tests/mir/test_deep_nesting_costs_heap.cpp`.
[[nodiscard]] MirLiteralValue toMirLiteral(HirLiteralValue const& src) {
    struct Frame {
        HirAggregateValue const* srcAgg;
        MirAggregateValue*       dstAgg;
        std::size_t              next;
    };
    std::vector<Frame> work;

    // Convert ONE node in place. An aggregate seeds an empty, fully-reserved
    // destination and pushes a frame; every other arm is a leaf.
    auto convertOne = [&work](HirLiteralValue const& s, MirLiteralValue& d) {
        d.core = s.core;
        std::visit([&](auto const& arm) {
            ...
            if constexpr (std::is_same_v<T, HirAggregateValue>) {
                ...
                // directly (NEVER degrade to monostate, which would silently drop
                // the value). The narrow-literal MIR lowering extracts the
                // container int from this arm; the wide path fills limbs; the
                // globals emitter fails loud on it.
                d.value = arm;
            } else if constexpr (std::is_same_v<T, WideFloatValue>) {
                // LD-3 (D-CSUBSET-LONG-DOUBLE-CONSTFOLD-PRECISION): the folded
                // F80/F128 wide-float value is the SAME host type in both pools —
                // copy it directly (the generic `else` below would also carry it,
                // but the explicit branch documents the new arm's round-trip
                // contract, mirroring BitIntValue). The globals byte-emitter
                // encodes it via `appendWideFloatBits`.
                d.value = arm;
            } else {
                d.value = arm;
            }
    ...
    convertOne(src, dst);
    while (!work.empty()) {
        std::size_t const top = work.size() - 1;
        if (work[top].next >= work[top].srcAgg->fields.size()) {
            work.pop_back();
            continue;
        }
        HirLiteralValue const& child =
            work[top].srcAgg->fields[work[top].next++];
        MirAggregateValue* const dstAgg = work[top].dstAgg;
        dstAgg->fields.emplace_back();
        // `convertOne` may push, which is why `dstAgg` was read out first.
        convertOne(child, dstAgg->fields.back());
    }
    return dst;
}
...
struct Lowerer {
    ...
    // (P55 lane `ag`). THE WALK IS ITERATIVE, AND REMOVING THE DEPTH CAP IS THE
    // FIX RATHER THAN A SIDE EFFECT OF IT.
    //
    // The recursive form carried `if (depth > 64) return 0`, and 0 is not a
    // neutral "don't know" here: `atomicAlignPayload` stamps it into `payload2`,
    // and `mir_to_lir`'s `atomicAccessIsUnderAligned` reads
    // `if (provable == 0) return false` — i.e. 0 means TREAT AS ALIGNED. So an
    // `_Atomic` member of a packed struct reached through a member/index chain
    // deeper than 64 stopped being recognized as under-aligned and got the
    // NATIVE inline form instead of the generic `__atomic_*` call: `rc 135, Bus
    // error` on a native arm64 target, silently, with no diagnostic. The
    // truncation failed toward UNSAFE, which is the direction the bar ranks
    // worse than a crash.
    //
    // ⇒ There is nothing to truncate now. `min` is associative and commutative,
    // so the chain's per-level `offsetAlign` terms accumulate into ONE running
    // value while the walk descends `kids[0]` in a plain loop — O(1) host
    // frames per level, exactly the operator's ruling of 2026-09-02. The
    // arithmetic is unchanged term for term: `kUnconstrainedAlign` is `min`'s
    // identity, and the terminal `naturalAlignOfType` is still the innermost
    // factor, so every answer this returns is the answer the recursion returned
    // at depths it survived.
    //
    // ⚠ THE ONLY BOUND LEFT IS A STRUCTURAL ONE AND IT FAILS LOUD. HIR is a
    // tree (`HirBuilder::addParent` aborts on a double-attach), so a descent
    // that visits more nodes than the module HAS is a cyclic — i.e. malformed —
    // tree, an internal invariant break rather than a deep input. It is
    // reported, never silently answered.
    [[nodiscard]] std::uint32_t provableLvalueAlign(HirNodeId node) {
        std::uint32_t acc  = kUnconstrainedAlign;   // min's identity
        HirNodeId     cur  = node;
        std::size_t   step = 0;
        ...
        for (;;) {
            ...
            if (++step > limit) {
                unsupported(node, "lvalue alignment walk exceeded the module's "
                                  "own node count — the HIR lvalue chain is "
            ...
            switch (hir.kind(cur)) {
                case HirKind::MemberAccess: {
                    ...
                    auto const off = fieldByteOffset(aggTy, hir.payload(cur));
                    if (!off.has_value()) return 0;
                    acc = std::min(acc, offsetAlign(*off));
                    cur = kids[0];
                    continue;
                }
                case HirKind::Index: {
                    ...
                    // power-of-two factor is provable. A pointer base restarts
                    // the walk at the pointee's guaranteed alignment (the Deref
                    // rule) and therefore ENDS the descent.
                    TypeId const baseTy = hir.typeId(kids[0]);
                    TypeId const elemTy = hir.typeId(cur);
                    auto const stride = elementStride(elemTy);
                    if (!stride.has_value()) return 0;
                    acc = std::min(acc, offsetAlign(*stride));
                    if (baseTy.valid()
                        && interner.kind(baseTy) == TypeKind::Ptr) {
                        std::uint32_t const base = naturalAlignOfType(elemTy);
                        if (base == 0) return 0;
                        return std::min(base, acc);
                    }
                    cur = kids[0];
                    continue;
                }
                case HirKind::Deref: {
                    ...
                    std::uint32_t const base = naturalAlignOfType(pointee[0]);
                    if (base == 0) return 0;
                    return std::min(base, acc);
                }
                default: break;
            }
            // Every other lvalue root — a named object, a global, a temporary —
            // is stored at its own type's natural alignment (a packed struct's
            // is 1, which is exactly the term that makes the member arm above
            // answer correctly at offset 0).
            std::uint32_t const base = naturalAlignOfType(hir.typeId(cur));
            if (base == 0) return 0;
            return std::min(base, acc);
        }
    }
    ...
    // ── THE AGGREGATE-INIT WORK STACK ───────────────────────────────────────
    //
    // D-MIR-AGGREGATE-INIT-INTO-SLOT-RECURSES-PER-INITIALIZER-LEVEL (P55, lane
    // `ag`). `lowerAggregateInitIntoSlot` ⇄ `lowerArrayInitIntoSlot` ⇄
    // `lowerBitfieldAggregateInitIntoSlot` were ONE mutual host recursion, one
    // call frame per BRACE LEVEL, with no cap of any kind. The operator's
    ...
    // explode the stack"* — makes depth cost HEAP instead, so all three forms
    // now share ONE explicit frame stack. A frame IS the activation record the
    // recursion kept: the container node, its destination slot, its type, the
    // volatility flag, which of the three forms it is, and how far through its
    // children it has got.
    //
    // ★★ THE EMISSION ORDER IS THE RECURSION'S, INSTRUCTION FOR INSTRUCTION,
    // AND THAT IS LOAD-BEARING. An initializer element may CALL A FUNCTION
    // (`struct S s = { f(), g() };`), and in the recursive form a nested brace's
    // ENTIRE subtree is emitted BETWEEN its left and right siblings' stores. So
    // this is deliberately NOT "emit every scalar, then walk the nested ones":
    // a frame SUSPENDS at the child it descended into and RESUMES at `next`,
    // which is the same left-to-right pre-order the call stack produced.
    // Reordering would move observable side effects, not just instructions.
    //
    // ⓘ Parking a `StructLayout const*` in a frame is safe: `layoutCache_` is a
    // `std::unordered_map`, whose element references survive a rehash.
    enum class AggInitForm : std::uint8_t { Struct, Array, Bitfield };

    // What a level's prologue decided: walk its children, the level is already
    // complete (a whole-object zero-fill has no children to visit), or it was
    // refused (diagnostic already reported).
    enum class AggInitPrologue : std::uint8_t { Walk, Done, Failed };
    ...
    struct AggInitFrame {
        ...
        TypeId              ty;
        MirInstFlags        vf     = MirInstFlags::None;
        AggInitForm         form   = AggInitForm::Struct;
        std::size_t         next   = 0;
        std::size_t         count  = 0;
        StructLayout const* layout = nullptr;   // Bitfield form only
    };

    // Drive the stack to completion. Returns false at the FIRST failure, whose
    // diagnostic the failing arm has already reported (the fail-loud contract
    // the recursive form had).
    [[nodiscard]] bool runAggregateInitWork(std::vector<AggInitFrame>& work) {
        while (!work.empty()) {
            AggInitFrame& top = work.back();
            if (top.next >= top.count) {
                work.pop_back();
                continue;
            }
            std::size_t const i = top.next++;
            // BY VALUE: the child handler may push, which can reallocate the
            // vector and dangle `top`.
            AggInitFrame const f = top;
            bool ok = false;
            switch (f.form) {
                case AggInitForm::Struct:
                    ok = aggInitStructChild(work, f, i);
                    break;
                case AggInitForm::Array:
                    ok = aggInitArrayChild(work, f, i);
                    break;
                case AggInitForm::Bitfield:
                    ok = aggInitBitfieldChild(work, f, i);
                    break;
            }
            if (!ok) return false;
        }
    ...
    // The ONE decision every child of every form makes, so the three forms
    // cannot drift apart: a nested brace DESCENDS (pushes a frame), an
    // aggregate VALUE copies from its lvalue address, a scalar stores.
    // `dstPtr` is the child's own destination pointer, already Gep'd.
    [[nodiscard]] bool aggInitMemberIntoSlot(std::vector<AggInitFrame>& work,
                                             HirNodeId diagAt, HirNodeId child,
                                             MirInstId dstPtr, TypeId memberTy,
                                             MirInstFlags vf) {
        TypeKind const mk = interner.kind(memberTy);
        ...
        if (isByValueClass(interner, memberTy)) {
            ...
            // from an aggregate VALUE (`{ existingStruct, … }`, not a nested
            // brace) is a copy from that value's lvalue address into the slot.
            MirInstId const srcPtr = lowerLvalueAddress(child);
            if (!srcPtr.valid()) return false;
            return lowerAggregateCopy(diagAt, srcPtr, dstPtr, memberTy, vf);
        }
        ...
        std::array<MirInstId, 2> stOps{v, dstPtr};
        // c27: flag if the destination aggregate is volatile (`vf`) OR this
        // MEMBER's own type is volatile (`struct { volatile int m; }`, and
        // `volatile int va[]` — which distributes the qualifier to the ELEMENT
        // type, so `vf` alone would miss it).
        mir.addInst(MirOpcode::Store, stOps, InvalidType, /*payload=*/0,
                    vf | volatileFlagForType(memberTy));
        return true;
    }

    // Push the frame for one aggregate level, running whatever PROLOGUE that
    // level's form owes first (the bit-field unit pre-zero, the union arity
    // refusal, the overlapping-struct gate). Some prologues COMPLETE the level
    // outright — an all-zero overlapping struct is one whole-object zero-fill
    // with no children to walk — in which case nothing is pushed and the level
    // is simply done. Returns false on a reported failure.
    [[nodiscard]] bool pushAggregateInitFrame(std::vector<AggInitFrame>& work,
                                              HirNodeId aggNode, MirInstId slot,
                                              TypeId aggTy, MirInstFlags vf,
                                              bool asArray) {
        AggInitFrame f;
        f.node = aggNode;
        f.slot = slot;
        ...
        f.vf   = vf;
        if (asArray) {
            f.form  = AggInitForm::Array;
            f.count = hir.children(aggNode).size();
            work.push_back(f);
            return true;
        }
        ...
        // co-resident neighbours. Routed to the unit-aware form.
        if (hasBitfieldMember(aggTy))
            return pushBitfieldAggregateInitFrame(work, f);
        switch (aggregateInitStructPrologue(aggNode, slot, aggTy, vf)) {
            case AggInitPrologue::Failed: return false;
            case AggInitPrologue::Done:   return true;   // level completed whole
            case AggInitPrologue::Walk:   break;
        }
        f.form  = AggInitForm::Struct;
        f.count = hir.children(aggNode).size();
        work.push_back(f);
        return true;
    }

    // FC8 D-CSUBSET-BITFIELD-INIT: the bit-field form's PROLOGUE — PASS 1, which
    // zeroes every bit-field allocation unit BEFORE any field value is written.
    [[nodiscard]] bool pushBitfieldAggregateInitFrame(std::vector<AggInitFrame>& work,
                                                      AggInitFrame fr) {
        HirNodeId const aggNode   = fr.node;
        MirInstId const allocaPtr = fr.slot;
        TypeId    const aggTy     = fr.ty;
        MirInstFlags const vf     = fr.vf;
        ...
        // PASS 2 is the frame's own walk — every field is written from
        // `aggInitBitfieldChild` below, in the same declaration order, now that
        // all zeroing is done and no write can clobber another.
        fr.form   = AggInitForm::Bitfield;
        fr.layout = layout;
        fr.count  = fieldCount;
        work.push_back(fr);
        return true;
    ...
    // FC8 D-CSUBSET-BITFIELD-INIT — PASS 2, one field. Bit-fields read-modify-
    // write their (pass-1-zeroed) unit; ordinary fields store or descend;
    // zero-width markers carry no storage and are skipped.
    [[nodiscard]] bool aggInitBitfieldChild(std::vector<AggInitFrame>& work,
                                            AggInitFrame const& fr,
                                            std::size_t f) {
        StructLayout const* layout = fr.layout;
        auto kids = hir.children(fr.node);
        bool const isBitfield =
            f < layout->bitFields.size() && layout->bitFields[f].unitBytes != 0;
        bool const isZeroWidthMarker =
            !isBitfield && interner.fieldBitWidth(fr.ty, f).has_value();
        if (isZeroWidthMarker) return true;   // no storage; child is a zero-fill
        ...
        if (!isBitfield) {
            // Ordinary field: Gep to its byte offset, then store/descend exactly
            // as the non-bit-field form does (a struct/union/array field
            // descends; a scalar field is a plain Store).
            std::array<MirInstId, 2> gepOps{fr.slot, offK};
            MirInstId const fieldPtr = mir.addInst(
                MirOpcode::Gep, gepOps, interner.pointer(fieldTy));
            if (!fieldPtr.valid()) return false;
            return aggInitMemberIntoSlot(work, fr.node, child, fieldPtr, fieldTy,
                                         fr.vf);
        }
        ...
        return emitBitfieldInsert(unitPtr, v, layout->bitFields[f], fieldTy, fr.vf);
    }
    ...
    // ⇒ P55 lane `ag`: BOTH halves walk an explicit HEAP stack. The predicate is
    // unchanged term for term — it is still a conjunction over the leaves, and a
    // conjunction short-circuits identically whichever order the tree is walked,
    // so flattening cannot move the answer. Depth follows the user's BRACE
    // NESTING, which is input, so under the operator's ruling of 2026-09-02 it
    // may not follow the host call stack.
        ...
        while (!work.empty()) {
            ...
            switch (hir.kind(n)) {
                ...
                case HirKind::Cast: {
                    ...
                    work.push_back(kids[0]);
                    continue;
                }
                case HirKind::Literal: {
                    if (!isAllZeroLiteral(literals.at(hir.payload(n))))
                        return false;
                    continue;
                }
                default:
    ...
    // Also its own heap walk: a folded aggregate literal nests as deep as the
    // brace nesting that produced it.
    [[nodiscard]] static bool isAllZeroLiteral(HirLiteralValue const& root) {
        std::vector<HirLiteralValue const*> work;
        work.push_back(&root);
        while (!work.empty()) {
            ...
            if (auto const* d = std::get_if<double>(&v.value)) {
                // rejects -0.0: numerically zero, but its sign bit is SET, so its
                // object representation is NOT all-zero bytes.
                if (*d != 0.0 || std::signbit(*d)) return false;
                continue;
            }
            if (auto const* a = std::get_if<HirAggregateValue>(&v.value)) {
                for (HirLiteralValue const& f : a->fields) work.push_back(&f);
                continue;
            }
            return false;
        }
    ...
    // Returns false (fail-loud already reported) on any failure.
    // ⇒ P55 lane `ag`: the ENTRY POINT. It seeds the shared work stack and
    // drives it; the per-level body is `aggregateInitStructPrologue` +
    // `aggInitStructChild`. `vf` (c27, D-CSUBSET-VOLATILE-POINTEE) is Volatile
    // when the DESTINATION aggregate is `volatile`-qualified (`volatile struct S
    // s = {…}` — C 6.7.3p5: every member write of a volatile object is a
    // volatile access). The VarDecl site passes the object's volatility;
    // structural descent propagates it to nested fields/elements through the
    // frame, exactly as the recursive parameter did.
    [[nodiscard]] bool lowerAggregateInitIntoSlot(HirNodeId aggNode,
                                                  MirInstId allocaPtr,
                                                  TypeId aggTy,
                                                  MirInstFlags vf = MirInstFlags::None) {
        std::vector<AggInitFrame> work;
        if (!pushAggregateInitFrame(work, aggNode, allocaPtr, aggTy, vf,
                                    /*asArray=*/false))
            return false;
        return runAggregateInitWork(work);
    }

    // One struct/union level's PROLOGUE — everything the recursive body did
    // BEFORE its field loop. `Done` means the level is finished outright (the
    // all-zero overlapping-struct fill writes the whole object and has no
    // children to visit); `Walk` means the field loop follows.
    [[nodiscard]] AggInitPrologue
    aggregateInitStructPrologue(HirNodeId aggNode, MirInstId allocaPtr,
                                TypeId aggTy, MirInstFlags vf) {
        ...
        //   * `prepareUnionBraceInit` and its one-slot level yield a 1-child `ConstructAggregate` (its
        ...
        if (interner.kind(aggTy) == TypeKind::Union) {
            ...
            return lowerByteWiseZeroFill(allocaPtr, layout->size, vf)
                       ? AggInitPrologue::Done
                       : AggInitPrologue::Failed;
        }
        return AggInitPrologue::Walk;
    }

    // FC7 D-FC7-NESTED-STRUCT-FIELD: one struct/union field. A struct/union- or
    // array-TYPED field whose initializer is itself a brace-init lowers to a
    // nested ConstructAggregate and DESCENDS element-wise into the field's
    // sub-slot at `fieldPtr` (there is no aggregate-width Store, so the scalar
    // path cannot realize it). A field initialized from an aggregate VALUE (not
    // a brace-init) is a field-into-field copy (D-FC7-AGGREGATE-COPY-MEMCPY).
    [[nodiscard]] bool aggInitStructChild(std::vector<AggInitFrame>& work,
                                          AggInitFrame const& fr, std::size_t i) {
        auto kids = hir.children(fr.node);
        auto const off = fieldByteOffset(fr.ty, static_cast<std::uint32_t>(i));
        if (!off.has_value()) {
            unsupported(fr.node, "aggregate initializer field-offset "
        ...
        MirInstId const fieldPtr = mir.addInst(
            MirOpcode::Gep, gepOps, interner.pointer(fieldTy));
        if (!fieldPtr.valid()) return false;
        return aggInitMemberIntoSlot(work, fr.node, kids[i], fieldPtr, fieldTy,
                                     fr.vf);
    }

    // D-MIR-ARRAY-FIELD-AGGREGATE-INIT: lower an ARRAY ConstructAggregate
    // initializer ELEMENT-WISE into the slot at `basePtr` — one Store (or a
    // nested descent) per element at the CONSTANT byte offset `j·stride` (init
    // indices are compile-time, so the offset is a const-disp Gep, already
    // lowered). `lowerBraceInit` has zero-filled unspecified elements, so the
    // child list is positional + complete. Returns false (fail-loud already
    // reported) on any failure. Wired at both array-init sites — the array field
    // descent guard AND the VarDecl array-local arm — so every array brace-init
    // funnels through this one entry point (§A.5 by-construction coverage).
    //
    // ⇒ P55 lane `ag`: it seeds the SAME work stack the struct form uses, so a
    // struct-of-array-of-struct nest costs one heap frame per level and no host
    // frames at all. c27: `vf` is Volatile when the destination array is
    // `volatile`-qualified (`volatile int va[] = {…}`); the frame propagates it
    // to nested element inits.
    [[nodiscard]] bool lowerArrayInitIntoSlot(HirNodeId arrNode,
                                              MirInstId basePtr,
                                              TypeId arrTy,
                                              MirInstFlags vf = MirInstFlags::None) {
        std::vector<AggInitFrame> work;
        if (!pushAggregateInitFrame(work, arrNode, basePtr, arrTy, vf,
                                    /*asArray=*/true))
            return false;
        return runAggregateInitWork(work);
    }
    ...
    // One array element.
    [[nodiscard]] bool aggInitArrayChild(std::vector<AggInitFrame>& work,
                                         AggInitFrame const& fr, std::size_t j) {
        auto kids = hir.children(fr.node);
        TypeId const elemTy = hir.typeId(kids[j]);
        std::optional<std::uint64_t> const stride = elementStride(elemTy);
        if (!stride.has_value()) {
            unsupported(fr.node, "array initializer element type has no "
                                 "computable size");
            return false;
        }
        ...
        return aggInitMemberIntoSlot(work, fr.node, kids[j], elemPtr, elemTy,
                                     fr.vf);
    }
    ...
    // ── ONE OWNER FOR "WALK EVERY NODE UNDER `root`", ON THE HEAP ────────────
    //
    // ★★★ D-MIR-HIRTOMIR-WHOLE-BODY-PRE-PASSES-RECURSE-PER-HIR-LEVEL
    //
    // ⚠⚠ PLAN-24 FLATTENED THE LOWERING DRIVERS AND LEFT THESE SIX PRE-PASSES
    // BEHIND. `lowerExpr` / `lowerLvalueAddress` / `lowerStmt` are explicit heap
    // work-stack drivers (Stage 4 / 4-address / 4b) — but the whole-body SCANS
    // that run BEFORE them (`collectAddressTakenSymbols`,
    // `collectSehFilterReferencedSymbols`, `collectRefSymbols`,
    // `collectAddressTakenLabels`, `collectLabelNodes`, `collectLocalDecls`) were
    // each their own host recursion, one frame per HIR level, with NO CAP OF ANY
    // KIND. The flattened driver was therefore never what decided the ceiling.
    //
    // ✔MEASURED (P55, lane `rc`) through `ctest` on the ORDINARY gtest main
    // thread, walking the depth through the real entry point:
    //   * nested `if` statements — 2048 lowers with rc 0, **2560 SEGFAULTs**,
    //     and the last stage marker printed is HIR, so the death is inside
    //     `lowerToMir`, not the parser and not the semantic pass;
    //   * a left-deep non-foldable operator spine (`argc+argc+…`) — 4096 lowers
    //     with rc 0, **8192 SEGFAULTs**, same stage.
    // ⓘ The CLI survives both because `program.cpp` builds every CU inside
    // `substrate::callOnLargeStack` — the mitigation the operator's ruling
    // rejects, since the depth still follows the input.
    //
    // ⇒ DEPTH NOW COSTS HEAP. One walker, one explicit `std::vector<HirNodeId>`
    // work stack, O(1) host frames per level, shared by all six passes so the
    // next pre-pass cannot re-introduce the recursion by being written in the
    // obvious style.
    //
    // ★ THE VISIT ORDER IS UNCHANGED AND THAT IS LOAD-BEARING: children are
    // pushed in REVERSE so they pop left-to-right, giving the identical
    // source-ordered pre-order DFS the recursive form produced.
    // `collectLabelNodes` writes `labelNodeByOrdinal_[ord] = node` — a last-wins
    // map assignment — so a reordered walk would silently pick a different label
    // node for a duplicated ordinal.
    template <typename Visit>
    void forEachHirNodeUnder(HirNodeId root, Visit&& visit) {
    ...
    void collectAddressTakenSymbols(HirNodeId root,
    ...
    void collectSehFilterReferencedSymbols(
        HirNodeId root, std::unordered_set<std::uint32_t>& out) {
        forEachHirNodeUnder(root, [&](HirNodeId node) {
            if (hir.kind(node) == HirKind::SehTryExcept) {
                // `collectRefSymbols` is itself a heap walk over the FILTER
                // subtree — a second walk, not a nested frame per body level.
                collectRefSymbols(hir.sehTryFilter(node), out);
            }
        });
    }
    ...
    void collectRefSymbols(HirNodeId root,
                           std::unordered_set<std::uint32_t>& out) {
        forEachHirNodeUnder(root, [&](HirNodeId node) {
    ...
    void collectAddressTakenLabels(HirNodeId root,
                                   std::unordered_set<std::uint32_t>& out) {
        forEachHirNodeUnder(root, [&](HirNodeId node) {
    ...
    void collectLabelNodes(HirNodeId root) {
        forEachHirNodeUnder(root, [&](HirNodeId node) {
    ...
    void collectLocalDecls(HirNodeId root, std::vector<HirNodeId>& out) {
        // ★ ORDER IS OBSERVABLE HERE: `out` is a VECTOR the caller walks in
        // order to emit allocas, so the pre-order-preserving push is not a
        // nicety — a reordered walk changes the emitted slot order.
        forEachHirNodeUnder(root, [&](HirNodeId node) {
    ...
    // heap walker in `mir/mir_literal_pool.hpp` the merge, the summary and the
    // lazy-import pass already route through. The walker visits EVERY node,
    // aggregates included, which changes nothing here: an aggregate node is not
    // a `MirSymbolAddrValue`, so the screen does not fire on it, and a
    // `MirSymbolAddrValue` node has no children for the old early `return` to
    // have pruned. Extending the existing owner rather than flattening a
    // seventh private walk is the point — a private one can go missing again.
    ...
    // written down. Arm (b) is tried by recursing, and when that recursion
    // returns nullopt the caller FALLS THROUGH to arms (c)-(f) FOR THE SAME
    // CHILD: a nested `{…}` of plain integers is not a link-time-constant
    // aggregate, fails arm (b), and is then folded whole by `evaluateConstant`.
    // A flattening that simply pushed a frame and failed on the way out would
    // silently lose that fallback and demote every such global to a runtime
    // store chain. So the descent is SPECULATIVE: a failed subtree pops back to
    // its parent, truncates the field it speculatively appended, and re-tries
    // the same child past the nested arm — exactly what the unwinding call did.
    //
    // ⚠ The parked `MirAggregateValue*` is valid only because every `fields`
    ...
    tryClassifyAggregateConst(HirNodeId initNode, EvalEnvironment const& env,
        ...
        MirLiteralValue out;
        seedAggregate(initNode, out);

        struct ClassifyFrame {
            HirNodeId          node;
            MirAggregateValue* dst   = nullptr;
            std::size_t        next  = 0;
            // Set while a child is under speculative descent; on resume it
            // means "arm (b) has already been tried and failed for this child".
            bool               retry = false;
        };
        std::vector<ClassifyFrame> work;
        work.push_back(ClassifyFrame{
            initNode, std::get_if<MirAggregateValue>(&out.value), 0, false});

        while (!work.empty()) {
            std::size_t const top = work.size() - 1;
            auto kids = hir.children(work[top].node);
            if (work[top].next >= kids.size()) {
                // This level classified completely. Its speculative field in the
                // parent is KEPT, and the parent moves on to its next child.
                work.pop_back();
                if (!work.empty()) {
                    work.back().next++;
                    work.back().retry = false;
                }
                continue;
            }
            ...
            // (a) D-C-LABEL-ADDRESS-IN-A-STATIC-INITIALIZER-REFUSED: a `&&label`
            // MEMBER (`static void *tbl[] = {&&L0, &&L1};`). FIRST, because a
            // label address is a link-time constant that const-eval provably
            // cannot fold and no other arm recognizes — leaving it to the
            // fallback is exactly how the whole aggregate used to bail to a
            // runtime store-chain.
            if (!skipNested) {
                if (auto la = tryClassifyAsLabelAddr(child)) {
                    dst->fields.push_back(std::move(*la));
                    work[top].next++;
                    continue;
                }
                ...
                if (hir.kind(child) == HirKind::ConstructAggregate) {
                    dst->fields.emplace_back();
                    seedAggregate(child, dst->fields.back());
                    work[top].retry = true;
                    work.push_back(ClassifyFrame{
                        child,
                        std::get_if<MirAggregateValue>(&dst->fields.back().value),
                        0, false});
                    continue;
                }
            }
            ...
            if (auto cx = tryClassifyComplexConst(child, hir.typeId(child),
                                                  env, opts)) {
                dst->fields.push_back(std::move(*cx));
                work[top].next++;
                continue;
            }
            ConstEvalResult const r =
            ...
            if (r.value.has_value()) {
                ...
                dst->fields.push_back(std::move(folded));
                work[top].next++;
                continue;
            }
            // One un-foldable member: this LEVEL does not classify. At the root
            // that is the whole answer (never partially classify); inside, the
            // parent rolls its speculative field back and re-tries this child
            // with the arms after the nested one.
            work.pop_back();
            if (work.empty()) return std::nullopt;
            work.back().dst->fields.pop_back();
            work.back().retry = true;
        }
        return out;
    }
    ...
    //
    // ★★ P55 lane `ag`: EXPLICIT POST-ORDER STACK, NOT HOST RECURSION. The
    // depth here follows an OPERATOR CHAIN (`double _Complex z = a + b + c +
    // …;` at file scope), and `rc`'s corpus census says a left-deep chain — not
    // brace nesting — is the shape real input makes long. Two stacks: `work`
    // holds nodes to EXPAND and nodes to COMBINE, `vals` holds finished
    // component pairs. A binary node pushes its combine step first and its two
    // operands after, right operand FIRST, so the left one pops first and the
    // values land in operand order. Arms and arithmetic are unchanged
    // expression for expression — ⚠ floating `+` and `*` are not associative,
    // so the operand order and the per-op rounding below are the answer, not a
    // formatting choice.
    //
    // ⓘ Both operands are now folded before the pair is checked, where the
    // recursion could stop after the first failed. `foldRealComponent` and
    // `evaluateConstant` are pure and emit no diagnostics, so that is wasted
    // work on a doomed fold and nothing else.
    struct CxStep {
        HirNodeId node;
        bool      combine = false;
    };
    ...
    foldComplexComponents(HirNodeId root, EvalEnvironment const& env,
                          EvalOptions const& opts) {
        std::vector<CxStep>                    work;
        std::vector<std::pair<double, double>> vals;
        work.push_back(CxStep{root, false});
        while (!work.empty()) {
            CxStep const step = work.back();
            work.pop_back();
            if (!foldComplexStep(step.node, step.combine, work, vals, env, opts))
                return std::nullopt;
        }
    ...
    // ONE step of the complex fold: expand `n` into its operands (pushing a
    // combine step behind them) or combine the operand values already on
    // `vals`. False means the whole fold fails, exactly as a `nullopt` return
    // did — the caller then falls through to its pre-existing loud refusal,
    // never to a partial image.
    [[nodiscard]] bool
    foldComplexStep(HirNodeId n, bool combine, std::vector<CxStep>& work,
                    std::vector<std::pair<double, double>>& vals,
                    EvalEnvironment const& env, EvalOptions const& opts) {
        ...
        if (!isComplex(interner, ty)) {
            ...
            vals.emplace_back(roundToComponent(*v, rk), 0.0);
            return true;
        }
        auto const ekOpt = complexComponentKind(ty);
        if (!ekOpt.has_value()) return false;          // F80/F128 element → wall
        TypeKind const ek = *ekOpt;
        auto const pair = [&vals, ek](double re, double im) {
            vals.emplace_back(roundToComponent(re, ek), roundToComponent(im, ek));
            return true;
        };
        auto const kids = hir.children(n);
        // EXPAND: push the combine step, then the operands (right first, so the
        ...
        if (!combine) {
            switch (hir.kind(n)) {
                ...
                case HirKind::UnaryOp: {
                    if (kids.size() != 1) return false;
                    work.push_back(CxStep{n, true});
                    work.push_back(CxStep{kids[0], false});
                    return true;
                }
                case HirKind::BinaryOp: {
                    if (kids.size() != 2) return false;
                    work.push_back(CxStep{n, true});
                    work.push_back(CxStep{kids[1], false});
                    work.push_back(CxStep{kids[0], false});
                    return true;
                }
                case HirKind::BuiltinCall: {
                    if (static_cast<BuiltinLowering>(hir.payload(n))
                        == BuiltinLowering::ComplexConj) {
                        if (kids.size() != 1) return false;
                        work.push_back(CxStep{n, true});
                        work.push_back(CxStep{kids[0], false});
                        return true;
                    }
                    // ComplexMake's operands are REAL folds, never complex ones,
                    // so it is a leaf here — fall through to the combine arms,
                    // which consume nothing from `vals` for it.
                    break;
                }
                default:
                    return false;
        ...
        switch (hir.kind(n)) {
            case HirKind::Cast: {
                ...
                if (kids.size() != 1 || vals.empty()) return false;
                std::pair<double, double> const inner = vals.back();
                vals.pop_back();
            ...
            case HirKind::BinaryOp: {
                ...
                if (kids.size() != 2 || vals.size() < 2) return false;
                std::pair<double, double> const b = vals.back();
                vals.pop_back();
                std::pair<double, double> const a = vals.back();
                vals.pop_back();
            ...
            case HirKind::BuiltinCall: {
                switch (static_cast<BuiltinLowering>(hir.payload(n))) {
                    case BuiltinLowering::ComplexMake: {
                        // A LEAF: both operands are REAL folds, so nothing was
                        // pushed for them and nothing is popped here.
                        if (kids.size() != 2) return false;
                        auto const re = foldRealComponent(kids[0], env, opts);
                        auto const im = foldRealComponent(kids[1], env, opts);
                        if (!re.has_value() || !im.has_value()) return false;
                        return pair(*re, *im);
                    }
                    case BuiltinLowering::ComplexConj: {
