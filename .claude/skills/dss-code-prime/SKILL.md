...
| `ToyHarness` | `tests/core/toy_harness.hpp` | `make(sourceText, configText)` loads an inline JSON config + source buffer; `tok(text, kind)` synthesizes tokens by substring lookup. For loadShipped paths, construct manually with the loaded schema. |
| `TokenSeq` | `tests/core/test_tree_end_to_end.cpp` (inline) | Sequential token emitter — finds next occurrence at-or-after the cursor. Use for sources with repeated lexemes (`;`, `=`) instead of per-call startHints. |
