...
| Whether one specific defect is open | that row's own `Status` cell — `bash scripts/anchors/read-anchor.sh <ANCHOR>` prints it, and finds the row in whichever registry holds it (two since 2026-09-16: production and the archive) | any list, queue or summary that names the row |
