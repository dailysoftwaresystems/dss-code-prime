...
- **OPT7 / inlining — `G-406` (plan 07) + its cross-CU sub-anchor (plan 22) — hard stop.** If an
...
- **Trigger-gated anchors are not TODOs.** A row that DECLARES a trigger must remain open until that
  ...
  `python .harness-config/runner/actions/check-anchor-balance/check-anchor-balance.py` says which are still open. This bullet
  USED TO name two particular rows
  as its exemplars, and both HAVE BEEN CLOSED since — so a rule that was correct was routing
  auditors to closed rows. A named exemplar is a status claim with a shelf life — true when typed
  and false once the closing commit landed; the rule is what belongs
