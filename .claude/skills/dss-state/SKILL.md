...
- **Per-format features flip green per HOST (TLS arc example).** The battery compiles probes
  ...
  (thread_local: ELF x86_64 landed TLS C1, arm64-ELF landed C2; PE lands C3; Mach-O C4)
  therefore shows `c11_thread_local` GREEN on a Linux host but 0x8015-RED on the Windows
  host until its C3 leg lands — the red is CORRECT (fail-loud on the un-landed leg), not a
  regression. Judge such probes on the leg that owns them.
