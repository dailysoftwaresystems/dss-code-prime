...
#       python tools/profile-compile-support.py kit \
...
#       bash tools/profile-compile.sh --kit build/perf/kit --label win-x86_64 \
#            --target x86_64:elf64-x86_64-linux-exec
#  2. The other three legs, from WSL (it owns the ssh carriage):
#       bash tools/profile-compile-dispatch.sh wsl
#       bash tools/profile-compile-dispatch.sh vps
#       bash tools/profile-compile-dispatch.sh mac
#  3. The yardstick, on any host with gcc (the same TUs, the same manifest):
#       python3 tools/profile-compile-support.py gcc-reference \
...
if [[ $rc -ne 0 ]]; then
  ...
  # is what tools/run-gate.sh exists to refuse. The failure token deliberately
