...
Each keyword's lexeme (`word`) is matched as a complete word — `letme` is an `Identifier`, not `LetKeyword` + `me`. This is what makes a pair like `__typeof__`/`__typeof` or `__volatile__`/`__volatile` safe to declare together, and the reason is worth being precise about: there is **no prefix or longest-first matching** involved. The tokenizer consumes the whole word-shaped run and then looks that complete string up in the lexeme table by exact match (`GrammarSchema::lookupLexeme` is a hash lookup on the full lexeme), so `__volatile__` is looked up as `__volatile__` and can never be read as `__volatile` followed by a stray `__`. Declaring both spellings is therefore two independent table entries, not an ambiguity to be resolved. The loader interns each kind into `schema.schemaTokens()` alongside the `tokens` declarations.

⚠ **The keyword ROW has no closed key vocabulary.** Unlike almost every other block in this schema, the `keywords` pass validates only that `word` and `kind` are strings (and rejects `modeOp`/`modeArg`/`stringStyle` as belonging on `tokens`); it does **not** run the typo discriminator. So a misspelled per-row key is silently ignored rather than rejected — most consequentially `contextual`, where a typo means the keyword loads as a HARD reserved word instead of a soft one. Spell `contextual` carefully, and put longer per-row rationale in a top-level `$…Comment` (the checked, explicitly-exempted documentation channel) rather than in a `$comment` inside the row.
...
| `C_RedundantField` (warning) | `lookahead` without `speculative: true`; `modeArg` with `popMode`; `endsAtLongestMatch` with a 1-character `endsAt`; a mode declaring **both** `defaultToken` and `tokens` (the body branch is entered on `defaultToken` and `continue`s on every exit, so the `tokens` table can never be consulted). The rule still loads. ⚠ A case-folded duplicate mode name is **not** in this set — it emits `C_ConflictingField` (`grammar_schema_json.cpp`, the `lexerModes` fold guard). ⚠ The INVERSE pairing — a mode with only `defaultToken` and no `tokens` — used to be listed here and is **not** a trigger: that is the canonical body-scanner shape (14 of 14 corpus modes), and warning on it was the inverted predicate this row's own trigger replaced. ⓘ These are visible on a load that SUCCEEDS as of `D-CONFIG-WARNINGS-DISCARDED-ON-SUCCESSFUL-LOAD` / `D-CONFIG-WARNINGS-DISCARDED-BY-DUMP-PREDEFINED-MACROS`: every driver route now forwards `GrammarSchema::loadDiagnostics()`. |
