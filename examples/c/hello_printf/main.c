// D-LANG-VARIADIC-CALL-SUBSTRATE (step 13.4, 2026-06-02): the first DSS-emitted
// binary that calls a C variadic function. printf is declared with
// `...`; the c grammar admits the call with the fixed param
// (`fmt`) plus one variadic int (`42`).
//
// End-to-end substrate exercised:
//
//   * D-LANG-VARIADIC-CALL-SUBSTRATE grammar — `extern int printf(const char* fmt, ...);`
//     parses via the new `paramOrEllipsis` alt + `ellipsisParam`
//     wrapper around `EllipsisOp`.
//
//   * D-LANG-VARIADIC-CALL-SUBSTRATE semantic — the externDecl's params subtree
...
//
//   * D-LANG-VARIADIC-CALL-SUBSTRATE call admission — `printf("hello\n")` passes 1
...
//
//   * D-LANG-VARIADIC-CALL-SUBSTRATE ABI — MIR Call's payload carries
