...
// arc. One compiled c artifact that drives the full
...
// Types: true`).
//
// Why the strict arm and not the char-exception arm: c is
// C, so `charTypesAliasAll: true` is the semantically-correct
// language config. An example under examples/c/ inherits
...
// c so a non-char distinct-primitive pair (`int*` vs
...
// Under c's default config (`strictAliasingOnDistinctTypes:
